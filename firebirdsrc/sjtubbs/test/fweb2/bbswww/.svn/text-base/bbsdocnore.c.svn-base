...
int bbsdocnore_main()
{
    ...
    if (strcmp(p->owner, currentuser.userid)
    ...
    if (num >= 0 && num < total)
    {
    	...
    	{
		if (x.accessed[1] & FILE_NOREPLY2 || x.accessed[0] & FILE_NOREPLY)
		{
			x.accessed[1] &= ~FILE_NOREPLY2;
			x.accessed[0] &= ~FILE_NOREPLY;
		}
		else if (!strcmp(p->owner, currentuser.userid))
		{
			x.accessed[0] |= FILE_NOREPLY;	
		}
		else
	...
	else if (!(x.accessed[1] & FILE_NOREPLY2))
	{
    ...
    printf("<center>\n");
    printf
        ("设置成功.<br><a href=\"bbsdoc?board=%s&page=%d\">返回本讨论区</a>",
         board, num / PAGE_SIZE);
