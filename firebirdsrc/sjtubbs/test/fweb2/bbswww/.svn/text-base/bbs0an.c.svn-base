...
int bbs0an_main()
{
...
#ifdef STATIC_URL
        printf("[<a href=bbs0an,path,%s.html>返回上一页</a>]\n", buf);
        if (!strncasecmp(board, "p_", 2))
...
#else
        printf("[<a href=bbs0an?path=%s>返回上一页</a>]\n", buf);
        if (!strncasecmp(board, "p_", 2))
    ...
    else
    {
        ...
        pre++;
#ifdef STATIC_URL
        printf("[<a href=bbsdoc,board,%s.html>本讨论区</a>] ", pre);
#else
        printf("[<a href=bbsdoc?board=%s>本讨论区</a>] ", pre);
#endif
    }    
    printf("\n</html>");
    return 0;
