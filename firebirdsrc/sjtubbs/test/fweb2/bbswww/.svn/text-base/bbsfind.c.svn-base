...
static int
search(char *id, char *pat, char *pat2, char *pat3, int dt, int dt2, int pagesize, int page)
{
    FILE *fp = 0;
    char board[256], dir[256];
    int total, now = time(0), i, sum = 0, n, t; //total: 板面内帖子数; sum: 总帖子数
    struct fileheader x;
    ...
    for (i = 0; i < MAXBOARD; i++)
    {
        ...
        while (1)
        {
			...
			{
				printf("<tr><td>%d</td><td><a href=bbsqry?userid=%s>%s</a></td>", n,
					   x.owner, x.owner);
				printf("<td>%6.6s</td>", Ctime(atoi(x.filename + 2)) + 4);
				printf
					("<td><a href=\"bbscon?board=%s&file=%s&num=%d\" target=\"_blank\">%s</a></td></tr>",
...
int bbsfind_main()
{
    ...
    else
    {
        ...
        if (day2 > 9999)
            day2 = 9999;
        search(user, title, title2, title3, day * 86400, day2 * 86400, pagesize, page);
    }
    printf("</html>");
    return 0;
}
