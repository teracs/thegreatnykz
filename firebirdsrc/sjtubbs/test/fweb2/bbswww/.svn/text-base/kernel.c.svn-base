...
int hhprintf(char *fmt, ...)
{
    ...
    if (!strcasestr(s, "http://") && !strcasestr(s, "https://") && !strcasestr(s, "ftp://")
        && !strcasestr(s, "mailto:") && !strcasestr(s, "[["))
        return hprintf("%s", buf);
    while (s[0])
    {
        ...
        else if (s[0] == '[' && s[1] == '[')
        {
            ...
            if (endpos == NULL || endpos - linktype > STRLEN - 2)
                break;
            ...
            *dest++ = '\0';            
            ...
            if (!strcmp(linktype, "User"))
                printf("<a target='_self' href='/bbsqry?userid=%s'>%s</a>", dest, display);
            else if (!strcmp(linktype, "Post"))
            {
                ...
                {
                    *filename++ = '\0';
                    printf("<a target='_self' href='/bbscon?board=%s&file=%s'>%s</a>", dest, filename, display);
                }
                else
                    break;
            }
            else if (!strcmp(linktype, "Digest"))
            {
                ...
                if (filename != NULL)
                {
                    *filename++ = '\0';
                    ...
                    printf("%%2F%s.html\">%s</a>", url_quote(filename), display);
                }
            }
            ...
            {	
                ...
                printf("<a target='_self' href='/wiki/%s'>%s</a>", dest, display);
            else if (!strcmp(linktype, "TeX"))
                printf("<img src=\"/cgi-bin/mimetex.cgi?%s\">",dest);
            else
                break;
...
int hhfprintf(FILE * stream, char *fmt, ...)
{
    ...
    if (!strcasestr(s, "https://") && !strcasestr(s, "http://") && !strcasestr(s, "ftp://")
    ...
    while (s[0])
    {
        ...
        else if (s[0] == '[' && s[1] == '[')
        {
            ...
            *dest++ = '\0';            
            ...
            if (!strcmp(linktype, "User"))
                fprintf(stream, "<a target='_self' href='/bbsqry?userid=%s'>%s</a>", dest, display);
            else if (!strcmp(linktype, "Post"))
            {
                filename = strchr(dest, ',');
                if (filename != NULL)
                {
                    *filename++ = '\0';
                    fprintf(stream, "<a target='_self' href='/bbscon?board=%s&file=%s'>%s</a>", dest, filename, display);
                }
                else
                    break;
            }
            else if (!strcmp(linktype, "Digest"))
            {
                ...
                if (filename != NULL)
                {
                    *filename++ = '\0';
                    ...
                    fprintf(stream, "%%2F%s.html\">%s</a>", url_quote(filename), display);
                }
            }
            else if (!strcmp(linktype, "Digdir"))
            {
                ...
                fprintf(stream, "<a target='_self' href='/wiki/%s'>%s</a>", dest, display);
            else if (!strcmp(linktype, "TeX"))
                fprintf(stream, "<img src=\"/cgi-bin/mimetex.cgi?%s\">",dest);
            else if (!strcmp(linktype, "Url"))
                fprintf(stream, "<a target='_blank' href='%s'>%s</a>", dest, display);
            else
                break;
...
int has_trainer_perm(struct userec *user, char *board)
{
    struct shortfile *x;
    char buf[256], *bm;

    if (strncmp(board, "BMTraining", 10))
        return 0;
    x = getbcache(TRAINER_BOARDNAME);
    if (x == 0)
        return 0;
...
int has_BM_perm(struct userec *user, char *board)
{
    ...
    if (!user_perm(user, PERM_BOARDS))
        return 0;
    if (has_trainer_perm(user, board))
        return 1;
...
int has_post_perm(struct userec *user, char *board)
{
	...
	if (!strcasecmp(board, "BBSHelp"))
		return 1;
	if (!strcasecmp(board, "Court"))
		return 1;
	if (!strcasecmp(board, "delation"))
		return 1;
