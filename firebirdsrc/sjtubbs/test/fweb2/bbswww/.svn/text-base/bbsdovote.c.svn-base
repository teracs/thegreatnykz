...
static int append_ip(char *filename, char *ip)
{
    int fd;
    struct voteip tmp;
...
static int append_mail(char *filename, char *email)
{
...
static int search_ip(char *filename, char *userip)
{
    int fd;
    int size = sizeof(struct voteip);
    struct voteip tmp;
...
static int search_mail(char *filename, char *email)
{
	return search_ip(filename, email);
}

static int get_real_mail( char firstchar, char *userid, char *real_mail)
{
...
int bbsdovote_main()
{
    ...
    char flagfile[STRLEN], ipfile[STRLEN], uipfile[STRLEN], emailfile[STRLEN];
    ...
    if (strcasecmp(cgiRequestMethod, "POST"))
        http_quit_print("Big Brother is watching!");
    ...
    printf("<body>");
    printf
        ("<form name=form1 action='bbsdovote' method=post enctype='multipart/form-data'>\n<input type=hidden name=board value='%s'>",
         board);
    ...
    sprintf(emailfile, "vote/%s/mail.%ld", board, opendate);
    sprintf(uipfile, "vote/%s/uip.%ld", board, opendate);
    ...
    while (read(fd, &tmpvote, sizeof(struct ballot)) ==
    ...
    if (voted_flag ==NA && get_real_mail(toupper(currentuser.userid[0]), currentuser.userid,real_mail) >= 0 && search_mail(emailfile, real_mail) >= MAXVOTEPEREMAIL)
    {
        printf("<tr><td>由于您的email已经超过投票次数,您将不能再投票.</td>");
        close(fd);
        goto out;
    }

    if (voted_flag == NA
        && search_ip(ipfile, currentuser.lasthost) >= MAXVOTEPERIP)
    {
    ...
    {
        if (type == 1)
            sprintf(valuestring, "%s", "SF");
        else if (type == 2)
            sprintf(valuestring, "%s", "DX");
    ...
    else if (type == 3)
    {
        ...
        for (i = 0; i <= totalitems - 1; i++)
        {
            ...
            if (strlen(getparm(valuestring)) == 0)
                continue;
            value = atoi(getparm(valuestring));
            uservote.voted ^= (1 << value);
        }
    ...
    {
	    append_mail(emailfile, real_mail);
    }
    close(fd);

    printf("您的投票已记入投票箱中，谢谢您的参与！");
  out:
    printf("<hr><center><a href=bbsdoc?board=%s>返回</a></center> ",
           board);
    printf("</form>\n");
    printf("</body>\n");
    return 0;
