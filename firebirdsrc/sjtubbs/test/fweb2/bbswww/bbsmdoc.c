...
int bbsmdoc_main()
{
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        printf("<td><a href=bbscon?board=%s&file=%s&num=%d>%s%s%36.36s </a>",
               board, x.filename, start + i, ((x.accessed[0] & FILE_NOREPLY) || (x.accessed[1] & FILE_NOREPLY2)) ?
               "<font color = blue><u>x</u></font>" : "", strncmp(x.title,
               "Re: ", 4) ? "○ " : "", void1(nohtml(x.title)));
    }
    printf("</table>\n");
    ...
    printf("<a href=bbsmnote4?board=%s>编辑板面关键字</a> ", board);
