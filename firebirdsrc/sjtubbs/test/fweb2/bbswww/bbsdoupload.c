...
static int get_unique_file_name(char *name)
{
...
int bbsdoupload_main()
{
    ...
    if (flag == 1 || flag == 2)
    {
        ...
        {
            printf
                ("<script src=\"/xml/index.js\" language=\"javascript\" type=\"text/javascript\"></script>");
        }
        else
        {
            printf
    ...
    {
        printf("<script src=\"/file/bbs/swfobject.js\" type=\"text/javascript\"></script>");
        printf("<script type=\"text/javascript\">");
        printf("function copySuccess(){alert(\"复制成功\");}");
        printf("var params={menu:\"%s\",scale:\"%s\",allowFullscreen:\"%s\",allowScriptAccess:\"%s\",bgcolor:\"%s\",wmode:\"transparent\",FlashVars:\"text=%s&data=%s/%s/%s\"};",
                "false",
                "noScale",
                "false",
                "always",
                "#FFFFFF",
                "复制",
                UPLOAD_FILE_URL_HEADER,
                x.board,
                x.showname
                );
        printf("var attributes={id:\"clipboard\",style:\"margin:0 0 -5px 10px;\"};");
        printf("swfobject.embedSWF(\"/file/bbs/clip.swf\", \"altContent\", \"30px\", \"20px\", \"9.0.0\", \"\", null, params, attributes);");
        printf("</script>");


        printf("附件上载成功, ");
        printf("URL为 :<P><font color=green>" UPLOAD_FILE_URL_HEADER
               "/%s/%s</font><span id=\"altContent\"></span><br>\n", x.board, x.showname);
        printf
        ...
        printf("文件说明: %s<br>\n", nohtml(x.exp));
        printf
            ("<hr color=green><a href='javascript:history.go(-1)'>返回</a>\n");
    }
    printf("\n</html>");
    return 0;
}
