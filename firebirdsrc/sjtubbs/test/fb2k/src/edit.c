...
int join(register struct textline *line)
{
    ...
    if (!line->next || line->next == can_edit_end)
        return YEA;
    if (!strncmp(line->next->data, "¡¾ ÔÚ", 5))
        return YEA;
...
void write_header(FILE * fp, int mode, int iscross)
{
    ...
    else
    {
        ...
        fprintf(fp, "·¢ÐÅÈË: %s (%s), ÐÅÇø: %s\n",
                ...
                (header.chk_anony && !iscross) ? "ÎÒÊÇÄäÃûÌìÊ¹" : uname, currboard);
    }
    fprintf(fp, "±ê  Ìâ: %s\n", save_title);
    getdatestring(now, NA);
...
char *format_pushbuf(char *pushbuf, char *bufformatted)
{
    ...
    *(tempbuf + j) = '\0';
    ...
    snprintf(buf, 256,
             "[1;32mÍÆ [m[1;33m%-12.12s:[m[33m%-40s[m [37m%15s[m [37m%2d/%2d[m",
    ...
    {
        snprintf(buf, 256,
                 "\n[32m¡ú [m[1;33m%-12.12s:[m[33m%-40s[m [37m%15s[m [37m%2d/%2d[m",
           ...
           unsigned int reid)
{
    struct textline *p;
    FILE *fp = NULL, *tmpfp;
    char abort[6], p_buf[100], *temp;
    int aborted = 0, savemode, qmd=0;
    int index;
    char userid[256], tmpfname[128];
    ...
    while (p != NULL)
    {
        ...
        if (aborted != -1)
        {
            if (uinfo.mode != EDIT || p != can_edit_end->prev ||
                (ADD_EDITMARK
                 && strncmp(p->data, "[1;36m¡ù ÐÞ¸ÄÄÚÈÝ:¡¤", 17)))
            {
		...
		qmd = 1;    
            ...
            if( qmd == 0 && uinfo.mode != SMAIL && (temp = strstr(p->data,"@")))
	    {
	        while( temp ){
		        temp++;
			index = 0;
			...
			sprintf(tmpfname, "tmp/%s.%s.%s", BBSID, currentuser.userid,userid);
			savemode = uinfo.mode;
			tmpfp = fopen(tmpfname,"w"); 
			if (tmpfp != NULL){
			    fprintf(tmpfp,"[[User:%s]]¸Õ¸ÕÔÚÒ»¸öÌû×ÓÖÐÌáµ½ÁËÄã\n"
					  "Ìû×ÓµÄÁ´½ÓÎª[[Post:%s,%s|%s]]\n" ,
					  currentuser.userid,
					  currboard,
					  strrchr(filename, '/') + 1,
					  save_title);
			    fclose(tmpfp);
			    strcpy(save_title2, save_title);
			    sprintf(save_title, "%s¸Õ¸ÕÌáµ½ÁËÄã", currentuser.userid);
			    mail_file(tmpfname, userid, save_title);
			    strcpy(save_title, save_title2);
			    unlink(tmpfname);
			    uinfo.mode = savemode;
			}
    ...
    if ((uinfo.mode == EDIT) && (aborted != -1))
    {
        ...
        fprintf(fp,
            "\n[1;36m¡ù ÐÞ¸Ä:¡¤%s ÓÚ %s ÐÞ¸ÄÄÚÈÝ¡¤[FROM: %-.46s][m\n",
...
int vedit(char *filename, int write_header_to_file, int modifyheader)
{
    ...
    if (DEFINE(DEF_AUTOWRAP) && (uinfo.mode == POSTING || uinfo.mode == SMAIL))
        linelen = 79;
    else
#endif
