...
int x_denypostqz(void)
{
	...
	int ch;
	char *denydesc[2] = {"发帖", "基本"}; //deny description
    ...
    while (1)
    {
        ...
        if (currentuser.userlevel & PERM_BLEVELS)
        {
            num[0] = 0;
            num[1] = 0;
			sprintf(msgbuf, "你要编辑在哪一区被封%s权限的名单.", denydesc[denytype]);
            ...
            if ((num[0] == 'a') || (num[0] == 'A'))
            {
                whichdist = 10;
                clear();
                prints("设定 A 区的封禁 %s 权限名单\n", denydesc[denytype]);
            }
            else if ((num[0] == 'b') || (num[0] == 'B'))
            {
                whichdist = 11;
                clear();
                prints("设定 B 区的封禁 %s 权限名单\n", denydesc[denytype]);
            }
            ...
            else if (num[0] == '\0' || num[0] < '1' || num[0] > '9')
            {
                whichdist = 12;
                clear();
                prints("设定全站的封禁 %s 权限名单\n", denydesc[denytype]);
            }
            else
            {
                whichdist = num[0] - '0';
                clear();
                prints("设定 %d 区的封禁 %s 权限名单\n", whichdist, denydesc[denytype]);
            }
        }
        else
        {
        ...
        {
            getdata(1, 0,
                    "(A)增加 (D)删除 (C)改变 (J)下一页 (K)上一页 (E)离开[E]: ",
                    ans, 7, DOECHO, YEA);
        }
        else
        {
            getdata(1, 0, "(A)增加 (E)离开 (J)下一页 (K)上一页 [E]: ", ans,
                    7, DOECHO, YEA);
        }
        if (*ans == 'A' || *ans == 'a')
        {
            ...
            {
                move(3, 0);
                prints("错误的使用者代号,或者你无权封此ID%s权限", denydesc[denytype]);
                pressanykey();
                clear();
                return 0;
            }
        ...
        else if ((*ans == 'D' || *ans == 'd') && count)
        {
			...
			{
				usercomplete("请输入要恢复全站基本权者 ID: ", uident);
			}
            if (*uident == '\0')
            {
                clear();
