...
int chk_trainer(void)
{
    struct shortfile *bp;
    char *ptr;
    char BMstrbuf[BM_LEN - 4];

    if ((bp = getbcache(TRAINER_BOARDNAME)) == NULL)
        return NA;
    memcpy(BMstrbuf, bp->BM, BM_LEN - 4);
    ptr = strtok(BMstrbuf, ",: ;|&()\0\n");
    while (1)
    {
        if (ptr == NULL)
...
//      return NA;
    ...
    if (!HAS_PERM(PERM_BOARDS))
        return NA;
    if (!strncmp(currboard, "BMTraining", 10) && chk_trainer() == YEA)
        return YEA;
...
*/
    if (!(ent->accessed[0] & FILE_DELETED) && (digestmode == 9)
        && (HAS_PERM(PERM_SPECIAL7)
            || (!strncmp(currboard, "BMTraining", 10) && chk_trainer() == YEA)))
    {
        if (brc_unread(ent->filename))
            type = 'D';
        else
