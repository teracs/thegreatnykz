...
int do_copyfav(char *boardfile,char *fromdir, char * todir)
{
	int i;
	char buf [STRLEN*2],next[STRLEN];
	struct boardcollection currentdir;
	if (f_get_file(boardfile, &currentdir, 1) != 0)
		    return -1;
	for(i = 0; i < currentdir.subnum; i++)
	{
		snprintf(buf,STRLEN*2,"cp %s%s %s%s",fromdir,currentdir.subdirsfile[i],todir,currentdir.subdirsfile[i]);
		system(buf);
		snprintf(next,STRLEN,"%s%s",fromdir,currentdir.subdirsfile[i]);
		do_copyfav(next,fromdir,todir);		
	}
	return 0;
}
...
int CopyMainHck()
{
    ...
    char srcfilename[STRLEN],destfilename[STRLEN],buf[STRLEN*2];
	char touser[STRLEN];
	clear ();
	prints("¸´ÖÆÊÕ²Ø¼Ð¸ø:\n<ÊäÈëÊ¹ÓÃÕß´úºÅ, °´¿Õ°×¼ü¿ÉÁÐ³ö·ûºÏ×Ö´®>\n");
	move(0,14);
	usercomplete(NULL, touser);
	if (touser[0] == '\0')
            return 0;
    snprintf(srcfilename, STRLEN, "home/%c/%c/%s/main.hck",
             toupper(currentuser.userid[0]),
             toupper(currentuser.userid[1]), currentuser.userid);
    snprintf(destfilename,STRLEN, "home/%c/%c/%s/main.hck.%s",
             toupper(touser[0]),
             toupper(touser[1]),touser,currentuser.userid);
    snprintf(buf,STRLEN*2,"cp %s %s",srcfilename,destfilename);
    system(buf);
    return 0;    
}
int PasteMainHck()
{
    char srcfilename[STRLEN],destfilename[STRLEN],buf[STRLEN*2];
	char fromuser[STRLEN],tmp[STRLEN];
	clear ();
	prints("½ÓÊÜÊÕ²Ø¼Ð×Ô:\n<ÊäÈëÊ¹ÓÃÕß´úºÅ, °´¿Õ°×¼ü¿ÉÁÐ³ö·ûºÏ×Ö´®>\n");
	move(0,14);
	usercomplete(NULL, fromuser);
	if (fromuser[0] == '\0')
            return 0;
    snprintf(destfilename, STRLEN, "home/%c/%c/%s/",
             toupper(currentuser.userid[0]),
             toupper(currentuser.userid[1]), currentuser.userid);
    snprintf(srcfilename,STRLEN, "home/%c/%c/%s/",
             toupper(fromuser[0]),
             toupper(fromuser[1]), fromuser);
    snprintf(tmp,STRLEN,"%smain.hck.%s",destfilename,fromuser);         
    if (!dashf(tmp))
    {
    	prints("\n\n\n       Ê¹ÓÃÕß%sÎ´¸øÄú¸´ÖÆÊÕ²Ø¼Ð£¬¸´ÖÆÊÕ²Ø¼ÐÊ§°Ü ",fromuser);
    	pressanykey();
    	return 0;
    }
    snprintf(buf,STRLEN*2,"cp %smain.hck.%s %smain.hck",destfilename,fromuser,destfilename);
    system(buf);
    do_copyfav("main.hck",srcfilename,destfilename);
    return 0;    
}
