...
//added by heack 2005-05-07
char * get_author(struct posttop ptop)
{
	 
	 FILE *fp;
    	char dir[80];
	struct fileheader f;
	sprintf(dir, "boards/%s/.DIR", ptop.board);
	fp = fopen(dir, "r");
	while (1)
	{
...
struct posttop *t;
{
    ...
    while (p && (!found))
    {
        if (!strncmp(p->title, t->title, 40)
                && !strcmp(p->board, t->board))
            found = 1;
        else
        {
            q = p;
...
int mytype;
{
    ...
    {
        fprintf(fp,
                "                [1;34m-----[37m=====[41m ±æ%s¥Û»»√≈ª∞Ã‚ [40m=====[34m-----[0m\n\n",
                mytitle[mytype]);
        ...
        for (i = 0; i < j; i++)
        {
            ...
            fprintf(fp,
                    ...
                    top[i].board, p, top[i].number, get_author(top[i]),
                    top[i].title);
        }
        fclose(fp);
    }
