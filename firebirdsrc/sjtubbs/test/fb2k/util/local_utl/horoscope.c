/* $Id: horoscope.c,v 1.2 2004/06/29 14:59:23 rwzmm Exp $ */
...
main()
{
    ...
    for (i=0; ; i++)
    {
        if (fread(&aman, sizeof( aman ), 1, fp ) <= 0)
            break;
        ...
        {
            if(aman.gender=='M')
                mhoro[aman.birthmonth-1]++;
            else
                fhoro[aman.birthmonth-1]++;
        }
        else
        {
            ...
            {
                if(aman.gender=='M')
                    mhoro[aman.birthmonth]++;
                else
                    fhoro[aman.birthmonth]++;
            }
        }
    }
    fclose(fp);
    ...
    fprintf(fp,"   [37m0©¸¡ª¡ª¡ª Ä¿Ç°±¾Õ¾×¢²áÊ¹ÓÃÕßÐÇ×ùÍ³¼Æ¡ª¡ª¡ª%s¡ª¡ª©¼\n"
