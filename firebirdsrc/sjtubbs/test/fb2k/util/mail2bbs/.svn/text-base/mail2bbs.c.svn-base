 ...
 *
 * $Id: mail2bbs.c,v 1.2 2004/06/29 14:59:23 rwzmm Exp $
 */
...
char *uid, *sender1, *sender, *title, *received;
{
    ...
    if (!strcmp(userid, "SYSOP") && strstr(conv_buf, " mail check."))
    {
        ...
        if ((!strstr(sender, "bbs")) && (strchr(conv_buf, '@')))
        {
            ...
            sprintf(fff, "%s/home/%c/%c/%s/mailcheck",
                    BBSHOME, toupper(sender1[0]), toupper(sender1[1]), sender1);
            if ((dp = fopen(fff, "r")) != NULL)
            {
                ...
                if (dosearchuser(sender1) && strstr(conv_buf, fff2))
                {
                    ...
                    sprintf(buf, "%s/home/%c/%c/%s/register",
                            BBSHOME, toupper(sender1[0]), toupper(sender1[1]), sender1);
                    if (dashf(buf))
                    {
                        sprintf(buf, "%s/home/%c/%c/%s/register.old",
                                BBSHOME, toupper(sender1[0]), toupper(sender1[1]), sender1);
                        rename(buf, conv_buf);
                    }
    ...
    bzero(&newmessage, sizeof(newmessage));
    sprintf(fname, "M.%d.A", time(0));
    sprintf(genbuf, "%s/%c/%s/%s", MAILDIR, toupper(userid[0]), userid,
            fname);
    ...
    else
    {
        ...
        else
        {
            ...
            while (fgets(genbuf, 255, fin) != NULL)
            {
                if (!strncmp(genbuf, "From: SYSOP.bbs", 15)
                        && passcheck == 0)
                    passcheck = 1;
                ...
                else if (!strncmp(genbuf, "认证暗码         :", 14)
                         && passcheck == 3)
                {
                    fclose(fout);
                    fclose(fin);
                ...
                spam_a =
                    spam_a +
                    genbuf[strlen(genbuf) / ((spam_b % RULE) + 1)];
                spam_c = spam_c + genbuf[strlen(genbuf) / RULE];
            }
        }
        fclose(fout);
...
int main(int argc, char *argv[])
{
    ...
    if (ptr == NULL)
    {
        ...
        //      toboard = 0;
        uidshm =
            (void *) attach_shm("UCACHE_KEY", (UCACHE_KEY),
                                sizeof(*uidshm));
    }
    /*	remove by alt@
