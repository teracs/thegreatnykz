    ...
    for (ptr1 = server; ptr1 && *ptr1;)
    {
        ...
        *ptr2 = savech;
        ...
        if (nl->host && *nl->host)
        {
            ...
            {
                nl->feedfp =
                    fopen(fileglue("%s/%s.link", INNDHOME, nl->node), "a");
                if (nl->feedfp == NULL)
                {
                    bbslog("<save outgoing> append failed for %s/%s.link",
                           INNDHOME, nl->node);
                }
            }
            if (nl->feedfp != NULL)
            {
                ...
                fprintf(nl->feedfp, "%s\t%s\t%s\t%ld\t%s\t%s\n",
                        sover->board, filename, group, mtime, FROM,
                        sover->subject);
                fflush(nl->feedfp);
...
soverview_t *sover;
{
    ...
    FN = fopen(fileglue("%s/boards/%s/%s/%s", BBSHOME, board, getfolderstr(filename), filename), "w");
    if (FN == NULL)
    {
...

    {
        ...
        utime(fileglue("%s/boards/%s/%s/%s", BBSHOME, board, getfolderstr(filename), filename),
              &times);
...
char *board, *filename, *userid, *nickname, *subject;
{
    ...
    filepath = fileglue("%s/boards/%s/%s/%s", BBSHOME, board, getfolderstr(filename), filename);
    if (isfile(filepath))
    {
        linkoverview_t lover;
        ...
        if (read_article(&lover, filepath, userid))
        {
...
#ifndef MapleBBS
            strncpy(POSTER_BUF,
                    fileglue("%s@%s (%s)", userid, MYBBSID, nickname),
                    sizeof POSTER_BUF);
            POSTER = POSTER_BUF;
#endif
#endif
            ...
            strncpy(FROM_BUF,
                    fileglue("%s.bbs@%s (%s)", userid, MYADDR, nickname),
                    sizeof FROM_BUF);
            FROM = FROM_BUF;
            sover.from = FROM;
            sover.board = board;
    ...
    *msgid, *path;
    ...
    else
    {
        ...
        *CONTROL = '\0';
        ...
        BODY = "";
        FD = open(fileglue("%s/boards/%s/%s/%s", BBSHOME, board, getfolderstr(filename), filename),
                  O_RDONLY);
        if (FD < 0)
        {
            if (Verbose)
                printf(" !! can't open %s/boards/%s/%s/%s\n", BBSHOME, board, getfolderstr(filename),
                       filename);
            else
                fprintf(stderr, "can't open %s/boards/%s/%s/%s\n", BBSHOME,
                        board, getfolderstr(filename), filename);
            return -1;
        }
        FD_SIZE =
            filesize(fileglue
                     ("%s/boards/%s/%s/%s", BBSHOME, board, getfolderstr(filename), filename));
        if (FD_BUF == NULL)
        {
...
nodelist_t *node;
{
    ...
    {
        node->feedfp =
            fopen(fileglue("%s/%s.link", INNDHOME, node->node), "a");
    }
}
...
char *board, *filename, *from, *subject;
{
    ...
    if (strncmp(result, "post to ", 8) == 0)
    {
        /* try to remove it */
        strncpy(filepath,
                fileglue("%s/boards/%s/%s/%s", BBSHOME, board, getfolderstr(filename), filename),
                sizeof filepath);
        if (isfile(filepath))
        {
        ...
        fprintf(FN, "发信人: %s, 信区: %s\n", from, board);
        fprintf(FN,
                "标  题: <article cancelled and mailed to the moderator\n");
        fprintf(FN, "发信站: %s (%s)\n", MYSITE, DATE);
        fprintf(FN, "转信站: %s\n", MYBBSID);
        fprintf(FN, "\n");
        fputs("\n", FN);
    ...
    if (Verbose)
        printf("%s/bbspost cancel %s %s %s moderate\n", INNDHOME, BBSHOME,
               board, filename);
    system(fileglue
           ("%s/bbspost cancel %s %s %s moderate", INNDHOME, BBSHOME,
            board, filename));
    return 1;
}
        ...
        if (isfile(feedfile) && !iszerofile(feedfile))
        {
            if (!NoAction)
            {
                        ...
                        && bbslink_get_lock(feedingfile))
                {
                    ...
                    system(fileglue
                           ("%s/ctlinnbbsd reload > /dev/null", INNDHOME));
                    send_nntplink(node, site, nntphost, protocol, port,
                                  feedingfile, nlcount);
...
char *argv;
{
    ...
    fprintf(stderr,
            "       -N (no visit, and only process batch queue)\n");
    fprintf(stderr,
                ...
                && bbslink_get_lock(bbslink_lockfile))
        {
            ...
            while (fgets(result, sizeof result, NEWPOST))
            {
                ...
                *ptr++ = '\0';
                ...
                process_article(board, filename, userid, nickname,
                                subject);
            }
            fclose(NEWPOST);
            unlink(OUTING);
    ...
    for (nlcount = 0; nlcount < NLCOUNT; nlcount++)
    {
        ...
        if (localsendout || remotesendout || localfailed || remotefailed)
            ...
            ("<bbslink> [%s]%s lsend:%d rsend:%d lfail:%d rfail:%d\n",
             ...
             localsendout, remotesendout, localfailed, remotefailed);
        if (NODELIST[nlcount].feedfp != NULL)
        {
            fclose(NODELIST[nlcount].feedfp);
            NODELIST[nlcount].feedfp = NULL;
