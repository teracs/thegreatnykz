...
char *outgoing;
{
    ...
    *NODELIST_BUF = '\0';
    ...
    {
        NODELIST =
            (nodelist_t *) mymalloc(sizeof(nodelist_t) * (count + 1));
        NODELIST_BYNODE =
            (nodelist_t **) mymalloc(sizeof(nodelist_t *) * (count + 1));
    }
    else
    {
        NODELIST =
            (nodelist_t *) myrealloc(NODELIST,
                                     sizeof(nodelist_t) * (count + 1));
        NODELIST_BYNODE =
            (nodelist_t **) myrealloc(NODELIST_BYNODE,
                                      sizeof(nodelist_t *) * (count + 1));
    }
    ...
    lastcount = count;
    NLCOUNT = 0;
    for (ptr = NODELIST_BUF;
            (nodelistptr = (char *) strchr(ptr, '\n')) != NULL;
        ...
        *nptr = '\0';
        ...
        if (*nptr == '+' || *nptr == '-')
        {
            ...
            if (NODELIST[NLCOUNT].feedtype == '+')
                if (outgoing != NULL)
                {
                    NODELIST[NLCOUNT].feedfp =
                        fopen((char *)
                              fileglue("%s/out.going/%s.%s", BBSHOME,
                                       NODELIST[NLCOUNT].node, outgoing),
                              "a");
                }
            nptr++;
        }
...
char *inndhome;
{
    ...
    *FILTER_BUF = '\0';
    ...
    {
        FILTERLIST =
            (filter_t *) myrealloc(FILTERLIST,
                                   sizeof(filter_t) * (count + 1));
    }
    FILTERCOUNT = 0;
    for (ptr = FILTER_BUF;
            (filterptr = (char *) strchr(ptr, '\n')) != NULL;
            ptr = filterptr + 1, FILTERCOUNT++)
    {
        char *nptr, *bptr, *pptr;
...
char *inndhome;
{
    ...
    *NEWSFEEDS_BUF = '\0';
    ...
    {
        NEWSFEEDS =
            (newsfeeds_t *) mymalloc(sizeof(newsfeeds_t) * (count + 1));
        NEWSFEEDS_BYBOARD =
            (newsfeeds_t **) mymalloc(sizeof(newsfeeds_t *) * (count + 1));
    }
    else
    {
        NEWSFEEDS =
            (newsfeeds_t *) myrealloc(NEWSFEEDS,
                                      sizeof(newsfeeds_t) * (count + 1));
        NEWSFEEDS_BYBOARD =
            (newsfeeds_t **) myrealloc(NEWSFEEDS_BYBOARD,
                                       sizeof(newsfeeds_t *) * (count +
                                                                1));
    }
    NFCOUNT = 0;
    for (ptr = NEWSFEEDS_BUF;
    ...
    for (i = 0; i < NFCOUNT; i++)
    {
        ...
        nfptr = NEWSFEEDS + i;
        for (found = 0, ngptr = nfptr->newsgroups, ptr =
                    strchr(ngptr, ','); ngptr && *ngptr && !found;
                ptr = strchr(ngptr, ','))
        {
            if (ptr)
                *ptr = '\0';
            ...
            if (fptr)
            {
                ...
                nfptr->rfilter =
                    (FuncPtr) search_filtercmd(fptr->rcmdfilter);
                nfptr->sfilter =
                    (FuncPtr) search_filtercmd(fptr->scmdfilter);
                found = 1;
            }
            else
            {
...
char *board;
{
    ...
    nftptr = &nft;
    find =
        (newsfeeds_t **) bsearch((char *) &nftptr, NEWSFEEDS_BYBOARD,
                                 NFCOUNT, sizeof(newsfeeds_t *),
                                 nf_byboardcmp);
    if (find != NULL)
        return *find;
    return NULL;
...
char *node;
{
    ...
    nltptr = &nlt;
    find =
        (nodelist_t **) bsearch((char *) &nltptr, NODELIST_BYNODE, NLCOUNT,
                                sizeof(nodelist_t *), nl_bynodecmp);
    if (find != NULL)
        return *find;
    return NULL;
}
...
char *identuser;
{
    ...
    nlt.host = site;
    find =
        (nodelist_t *) bsearch((char *) &nlt, NODELIST, NLCOUNT,
                               sizeof(nodelist_t), nlcmp);
...
char *newsgroup;
{
    ...
    find =
        (newsfeeds_t *) bsearch((char *) &nft, NEWSFEEDS, NFCOUNT,
                                sizeof(newsfeeds_t), nfcmp);
    return find;
}
