...
int num;
{
	...
	undercontrol = 0;
	setbfile(ctrlpath, currboard, "board.allow");
	if(dashf(ctrlpath))
		undercontrol = 1;
	...
	else if(undercontrol == 0)
	{
		Postfile (nname, "vote", title, 1);
	}
	
//    if (currboard != "vote")
	if(strncmp(currboard, "vote", 4)) //not vote board itself
	{
...
user_vote (int num)
{
    ...
    else
    {
        ...
        prints ("\nÒÑ¾­°ïÄúÍ¶ÈëÆ±ÏäÖĞ...\n");
		if(!strncmp(currboard, "sysop", 5)) /* È«Õ¾Í¶Æ± */
		{
			char votereport[STRLEN];
			sprintf(votereport, "%s ²Î¼ÓÁËÈ«Õ¾Í¶Æ±", currentuser.userid);
			securityreport(votereport);
		}
    }
    pressanykey ();
    return;
...
#define maxrecp 300
int
ListFileEdit (char *fname, char *msg, int response, int allow_reject )
{
    ...
    while (1)
    {
		...
		if(allow_reject==1){
        getdata (1, 0,
                 "(A)Ôö¼Ó (D)É¾³ı (I)ÒıÈëºÃÓÑÃûµ¥ (C)Çå³ıÄ¿Ç°Ãûµ¥ (E)È·¶¨ [E]: ",
				 ans, 2, DOECHO, YEA);
		}
		else{
        getdata (1, 0,
                 "(A)Ôö¼Ó (D)É¾³ı (I)ÒıÈë½ûÈëÕßÃûµ¥ (C)Çå³ıÄ¿Ç°Ãûµ¥ (E)È·¶¨ [E]: ",
				 ans, 2, DOECHO, YEA);
		}
...
vote_key (int ch, int allnum, int pagenum)
{
    ...
    case Ctrl ('E'):
        ...
        ListFileEdit (genbuf, "±à¼­¡ºÍ¶Æ±Ãûµ¥¡»", 0,1);
        break;
    case 'E':
        if (!chk_currBM (currBM))
            return YEA;
...
//	int i = 0;
	int callforvoted = 0;
	struct votebal vote_rec = {0};
	struct ballot vote_ballot = {0};
	char path[STRLEN];
...
//	for(i = 0; i < num_of_vote; i++)
	...
	{
		sprintf(path, "vote/%s/flag.%d", currboard, vote_rec.opendate);
