...
{
    FILE *stream;
    char bm_file_path[32],buffer[55];
    sprintf(bm_file_path,"home/%c/%s/real_email",firstchar,userid);
...
//	struct rpg u_rpg;
    ...
    char planid[IDLEN + 2], buf[64], buf2[64];
    time_t now;
...
//    int dist = 12;
    ...
    if ((uinfo.mode != LUSERS && uinfo.mode != LAUSERS && uinfo.mode!= ADMIN
    ...
    if (strcasecmp (lookupuser.userid, "guest") != 0)
        sprintf (buf, "[\033[1;3%dm%s座 %s\033[m] ", clr,
		horoscope (lookupuser.birthmonth, lookupuser.birthday),
		n_horoscope (lookupuser.birthmonth, lookupuser.birthday)
    ...
    if (strcmp (lookupuser.userid, "guest") == 0)
        ...
        sprintf (buf2, "网龄[\033[1;32m%d\033[0;1m]天",
		(time (0) - lookupuser.firstlogin) / 86400);
	
    prints ("\033[1;37m%s \033[m(\033[1;33m%s\033[m) 共上站 \033[1;32m%d\033[m 次  %s  %s\n",
    ...
    prints ("上 次 在: [\033[1;32m%s\033[m] 从 [\033[1;32m%s\033[m] 到本站一游。\n",
		...
		datestring, hideip?BBSIP:lookupuser.lasthost);
    if (num)
    {
        sprintf (genbuf,
			"目前在线: [\033[1;32m讯息器: (\033[36m%s\033[32m) 呼叫器: (\033[36m%s\033[32m)\033[m] ",
	...
	/*
    prints
    ("生命力:[\033[1;32m%d\033[m] 文章:[\033[1;32m%d\033[m] 信箱:[\033[1;5;32m%2s\033[m]\n",
	...
	*/
	   ...
	   if (HAS_PERM (PERM_SPECIAL8)|| (strcmp(lookupuser.userid,currentuser.userid)==0)){
		   ...
		   /* 查看个人信息的时候刷新银行的存款 add by pax */
		   sprintf (fname,"bank/%s.bank", lookupuser.userid);
		   if(fd=open(fname,O_RDWR )!=-1)
		   {
			   fd=open(fname,O_RDWR);
			   read(fd,&cBill,sizeof(struct callbill));//读取原有信息
			   cBill.interest=cBill.principal*(0.0005*(time(0)-cBill.date))/86400;
			   //allmoney=(int)ceil(cBill.interest);
			  // cBill.principal=cBill.principal+allmoney;
			   lseek(fd,0,SEEK_SET);//定位到文件头部
			   write(fd,&cBill,sizeof(struct callbill));//重写文件
			   close(fd);		
			   allmoney=lookupuser.inbank+(int)cBill.interest;		
		   }
		   
		   /*	end by pax */
		   prints("自由资金: [\033[1;32m%d元\033[m] 银行存款: [\033[1;32m%d元\033[m](\033[1;33m%s\033[m) 经验值: [\033[1;32m%d\033[m](\033[1;33m%s\033[m)。\n",
			   lookupuser.money,allmoney,
	   ...
	   */
	   ...
	   {
		   prints("职务:\033[0;1m[\033[1;32m银行行长\033[0;1m]");
		   ...
		   if (lookupuser.userlevel & PERM_XEMPT)
		   {
			   if(IsGoodBm(lookupuser.userid))
				   prints (" [\033[1;32m优秀板主\033[0;1m]");
		   }
		   		   		   if(lookupuser.userlevel & PERM_REALESTATE)
			   prints (" [\033[1;32m居委会主任\033[0;1m]");
	   ...
	   {
		   prints ("职务:\033[0;1m[\033[1;32m账号管理员\033[0;1m]");
		   if ((lookupuser.userlevel & PERM_ANNOUNCE) ||
			   (lookupuser.userlevel & PERM_OBOARDS) ||
			   (lookupuser.userlevel & PERM_OVOTE))
			   prints (" [\033[1;32m本站总管\033[0;1m]");
		   ...
		   (lookupuser.userlevel & PERM_OVOTE))
	   {
		   prints ("职务:\033[0;1m[\033[1;32m本站总管\033[0;1m]");
		   if (lookupuser.userlevel & PERM_CHATCLOAK)
			   prints (" [\033[1;32m本站智囊\033[0;1m]");
		   if (lookupuser.userlevel & PERM_SPECIAL6)
			   prints (" [\033[1;32m技术智囊\033[0;1m]");
	   ...
	   {
		   prints ("职务:\033[0;1m[\033[1;32m南洋总管\033[0;1m]");
		   if (lookupuser.userlevel & PERM_SPECIAL6)
			   prints (" [\033[1;32m技术智囊\033[0;1m]");
		   if ((lookupuser.userlevel & PERM_SPECIAL7) && !(lookupuser.userlevel & PERM_OBOARDS))
			   show_dm( lookupuser.userid  );
	   ...
	   {
		   prints ("职务:\033[0;1m[\033[1;32m美工总管\033[0;1m]");
		   if (lookupuser.userlevel & PERM_SPECIAL6)
			   prints (" [\033[1;32m技术智囊\033[0;1m]");
		   if ((lookupuser.userlevel & PERM_SPECIAL7) && !(lookupuser.userlevel & PERM_OBOARDS))
			   show_dm( lookupuser.userid  );
	   ...
	   {
		   prints ("职务:\033[0;1m[\033[1;32m本站智囊\033[0;1m]");
		   if ((lookupuser.userlevel & PERM_SPECIAL7) && !(lookupuser.userlevel & PERM_OBOARDS))
			   show_dm( lookupuser.userid  );
		   //show_dm(toupper(lookupuser.userid[0]),lookupuser.userid);
		   /*prints (" [\033[1;32m本站区长\033[0;1m]");*/
	   ...
	   {
		   prints ("职务:\033[0;1m[\033[1;32m技术智囊\033[0;1m]");
		   if (lookupuser.userlevel & PERM_XEMPT)
		   {
			   if(IsGoodBm(lookupuser.userid))
				   prints (" [\033[1;32m优秀板主\033[0;1m]");
	   ...
	   else if ((lookupuser.userlevel & PERM_SPECIAL7) && !(lookupuser.userlevel & PERM_OBOARDS))
	   {
		   ...
		   show_dm( lookupuser.userid );
		   //show_dm(toupper(lookupuser.userid[0]),lookupuser.userid);
		   /*prints ("职务:\033[0;1m[\033[1;32m本站区长\033[0;1m]");*/
		   if (lookupuser.userlevel & PERM_XEMPT)
		   {
			   if(IsGoodBm(lookupuser.userid))
				   prints (" [\033[1;32m优秀板主\033[0;1m]");
	   ...
	   else if(lookupuser.userlevel & PERM_XEMPT)
	   {
		   ...
		   if(IsGoodBm(lookupuser.userid))
		   {
			   prints ("职务:\033[0;1m[\033[1;32m优秀板主\033[0;1m]");
			   tmp = 1;
		   }
		   		   		   if(lookupuser.userlevel & PERM_REALESTATE)
			   prints (" [\033[1;32m居委会主任\033[0;1m]");
	   ...
	   else if(lookupuser.userlevel & PERM_REALESTATE)
	   {
		     prints (" 职务:[\033[1;32m居委会主任\033[0;1m]");
	   ...
	   if (uinfo.mode != LUSERS && uinfo.mode != LAUSERS /*&& uinfo.mode != READING*/
		   ...
		   {
			   //add by rwzmm @ sjtubbs. 增加查询使用者的时候的发信，讯息，好友名单功能
			   //  update_time = 0;
			   //  choose (YEA, 0, 0, deal_key3, 0, do_query3);
			   move (t_lines - 1, 0);
			   prints("\033[0;1;37;44m聊天[\033[1;32mt\033[37m] 寄信[\033[1;32mm\033[37m] 送讯息[\033[1;32ms\033[37m] 加,减朋友[\033[1;32mo\033[37m,\033[1;32md\033[37m] 亲戚关系查询[\033[1;32mq\033[37m]                                       \033[m");
			   ...
			   case 'G':
				   ...
				   {
				   sprintf (buf, "%s 还没有申请个人文集", lookupuser.userid);
				   msgflag = YEA;
				   }
				   break;
				   */
...
struct user_info *uentp;
{
	...
	i < (ulistpage + 1) * pageusers && i < range; i++)
    {
        ...
        if (i < numf || friendmode)
            ...
            ovv = NA;
        sprintf (ubuf, "%s%-12.12s %s%-10.10s\033[m",
...
char *modestr;
{
    ...
    if (uinfo.mode == MONITOR)
    {
        ...
        sprintf (genbuf,
			"\033[1;44;33m  目前有 \033[32m%3d\033[33m %6s上线, 时间: \033[32m%22.22s \033[33m, 目前状态: \033[36m%10s   \033[m",
...
struct user_info* t_search (sid, pid)
char *sid;
int pid;
{
    int i;
...
int
deal_key3 (char ch, int allnum, int pagenum)	//ctrl a按键处理
{
...
int star, curr;
{
    if (user_record[curr]->userid != NULL)
    {
        ...
        move (t_lines - 1, 0);
        prints("\033[0;1;37;44m聊天[\033[1;32mt\033[37m] 寄信[\033[1;32mm\033[37m] 送讯息[\033[1;32ms\033[37m] 加,减朋友[\033[1;32mo\033[37m,\033[1;32md\033[37m] \033[m");
    }
}
...
{ 
	...
	for(y=1;y<=19;y++) 
		...
		move (3,43); 
		prints("CHESS_GO v1.0 for firebird BBS2000\n"); 
		move (5,43); 
		prints("        思源湖 2005 07"); 
		move(12,43); 
		if(chesscolor==1) 
		...
		if(chessmode==2) 
			prints("        MODE: 其他棋类"); 
		move (20,0); 		
		prints("左移:'a', 右移:'s', 上移:'w', 下移:'z', 落子:' [Space]'\n, 模式切换:'#', EXIT:^D.\n"); 
...
{	
	...
	{	
		...
		{ 
			...
			if(a[y][x]==chesscolor)
				strcpy(tmp,"●");
			else if(a[y][x]==3-chesscolor)
				strcpy(tmp,"○");
			else
				strcpy(tmp,chesstyle(x,y));
			strcat (chesslog, tmp);
		} 
		strcat (chesslog, "\n");
	}
	strcat (chesslog, "  A B C D E F G H I J K L M N O P Q R S");
	if (chessturn==chesscolor)
		sprintf(title,"%s vs %s 第%d手 白棋 %c%2d ",currentuser.userid,lookupuser.userid,chesshand-1,chessx+64,20-chessy);
	else
		sprintf(title,"%s vs %s 第%d手 黑棋 %c%2d ",currentuser.userid,lookupuser.userid,chesshand-1,chessx+64,20-chessy);

	strcpy (currboard, "WeiQi");
	autoreport(title,chesslog,NULL, NULL);
	return 1;
}