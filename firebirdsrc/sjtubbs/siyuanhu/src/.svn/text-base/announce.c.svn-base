...
/*
    ...
    case 'S':
        {
			...
			getdata(  t_lines - 1, 0,"ÇëÊäÈë²Ù×÷Ë¿Â·±àºÅ 1-4(Ä¬ÈÏÎª×î½üÒ»´ÎÉèÖÃµÄË¿Â·):", msg, 2, DOECHO,  YEA);
    ...
    case 'f':
        {
			getdata(  t_lines - 1, 0,"ÇëÊäÈë²Ù×÷Ë¿Â· 1-4:", msg, 2, DOECHO,  YEA);
			opt = atoi(msg);
				if (msg[0] == '\n' || msg[0] == '\0')
	msg[0] = '0';
	opt = atoi(msg);
		...
		presskeyfor ("ÊäÈë´íÎó, Çë°´<Enter>¼ÌĞø...",t_lines - 1);
			...
			else
			{
				...
				if (fn)
				{
					...
					fclose (fn);
					presskeyfor ("ÒÑ½«¸ÃÂ·¾¶ÉèÎªË¿Â·, Çë°´<Enter>¼ÌĞø...",
						t_lines - 1);
				}
				sethomefile (genbuf, currentuser.userid, ".announcepath");
				fn = fopen (genbuf, "w+");
