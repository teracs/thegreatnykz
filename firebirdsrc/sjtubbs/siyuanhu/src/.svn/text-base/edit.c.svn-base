...
write_posts ()
{
	...
	if (no_top_ten () || normal_board (currboard) != 1)	
        return;
    now = time (0);
...
/*add by pax 2005-10-9 Ä£°æ·¢ÎÄ¹¦ÄÜ  ¶ÁÈ¡Ä£°æÎÄ¼þ*/
void
addmodel (FILE * fp)
{
    FILE *sigfile;
   ...
   post_word = 0;
    ...
    if (uinfo.mode == POSTING || uinfo.mode == EDIT)
    {
        ...
        if (len < 20 || lines < 1)
        {
            ...
            prints
            ("      ±»ÏµÍ³ÅÐ¶¨Îª¹àË®µÄÎÄÕÂ£¬½«×Ô¶¯¼ÓÉÏ 'X' ±ê¼Ç¡£\n      ÏµÍ³¿ÉÄÜ¶¨Ê±É¾³ý±» 'X' ±ê¼ÇµÄÎÄÕÂ¡£°åÖ÷¿ÉÒÔÓÃ w ½â³ý¸Ã±ê¼Ç\n");
            markXflag = 1;
            y += 3;
            w = YEA;
        }
        ...
        {
            strcpy (pmt,
                    "E.ÔÙ±à¼­, S.×ªÐÅ, L.±¾Õ¾·¢±í, A.È¡Ïû or T.¸ü¸Ä±êÌâ?[L]: ");
        }
    }
#ifdef ALLOWGAME   
   post_word = len;
...
/*
int
log2db(char *user_id, char *ip_address,char *filename,char *board_name,char *content,unsigned int reid)
{
char query_string[65535], content_ok[65535],title_ok[256],table_name[64];
MYSQL mysql;
time_t timestamp;
struct tm *tm;
timestamp = time(NULL);
...
mysql_real_escape_string(&mysql,content_ok, content,strlen(content));
mysql_real_escape_string(&mysql,title_ok, save_title,strlen(save_title));
sprintf(query_string,"insert into %s (user_id,ip_addr,time,filename,board,title,content,reid) values ('%s','%s',NOW(),'%s','%s','%s','%s','%d')",table_name,user_id,ip_address,filename,board_name,title_ok,content_ok,reid);
...
write_file (char *filename, int write_header_to_file, int addfrom, int sure,unsigned int reid)
{
    struct textline *p;
    FILE *fp;
    char abort[6], p_buf[100],buf[65535]={0};
    int aborted = 0;
...
/*        }*/
    ...
    while (p != NULL)
    {
        ...
        if (aborted != -1)
        {
            if (uinfo.mode != EDIT || p != can_edit_end->prev ||
                    (ADD_EDITMARK && strncmp (p->data, "[1;36m¡ù ÐÞ¸Ä:¡¤", 17)))
            {
            ...
            {
				if(HAS_DEFINE(currentuser.userdefine, DEF_NOTHIDEIP))
                {
					sprintf (buf,
                         "%s\n[1;36m¡ù ÐÞ¸Ä:¡¤%s ÓÚ %16.16s ÐÞ¸Ä±¾ÎÄ¡¤[FROM: %-.20s][m\n",
                         buf,
            ...
            && addfrom != 0 && aborted != -1 && write_header_to_file)
    {
		...
		{
			sprintf (buf, "%s\n\033[m\033[1;%2dm¡ù À´Ô´:¡¤%s %s¡¤[FROM: %-.20s]\033[m\n",
			buf,
                 color, BoardName, BBSHOST,
                 (noidboard) ? "ÄäÃûÌìÊ¹µÄ¼Ò" : BBSIP);
		}
    }
	    
	if (aborted != -1) {
//		log2db( currentuser.userid,  currentuser.lasthost,(strrchr(filename,'/')+1),currboard,buf,reid);
		fprintf(fp,"%s",buf);
	        buf[0]='\0';
...
vedit_reid (char *filename, int write_header_to_file, int modifyheader,unsigned int reid)
{
    int ans, t = showansi;
