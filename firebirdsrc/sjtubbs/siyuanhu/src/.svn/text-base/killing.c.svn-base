...
struct action condition_data[] = {
	/*modified by pax@sjtubbs 05-7-6*/
    {":D", "Ğ¦µÃºÏ²»Â£×ì", NULL},
    {":)", "Î¢Ğ¦", NULL},
    {":P", "Ğ¦ÍáÁË×ì", NULL},
    {":(", "¾õµÃ²»Ë¬", NULL},
...
//    if(r->level&currentuser.userlevel!=r->level) return 0;
    if(r->style!=1) return 0;
    if(r->flag&ROOM_SECRET&&!HAS_PERM(PERM_SYSOP)) return 0;
    return 1;
}
...
//    if(r->level&currentuser.userlevel!=r->level) return 0;
    if(r->style!=1) return 0;
    if(r->flag&ROOM_LOCKED&&!HAS_PERM(PERM_SYSOP)) return 0;
    return 1;
}
...
void start_killergame()
{
    ...
    for(i=0;i<MAX_PEOPLE;i++)
        ...
        if(!(inrooms[myroom].peoples[i].flag&PEOPLE_SPECTATOR)) 
            total++;
    if(total<6) {
        send_msg(me, "[31;1mÖÁÉÙ6ÈË²Î¼Ó²ÅÄÜ¿ªÊ¼ÓÎÏ·[m");
        end_change_inroom();
        refreshit();
        return;
    }
    ...
    for(i=0;i<totalk;i++) {
        ...
        send_msg(j, "ÓÃÄãµÄ¼âµ¶([31;1mCtrl+S[m)Ñ¡ÔñÄãÒª²Ğº¦µÄÈË°É...");
    }
    for(i=0;i<totalc;i++) {
        do{
            j=rand()%MAX_PEOPLE;
        ...
        send_msg(j, "°×ÌìÇëÓÃÄãµÄ¾¯°ô([31;1mCtrl+T[m)Ñ¡ÔñÄã»³ÒÉµÄÈË...");
    }
    for(i=0;i<MAX_PEOPLE;i++) 
    if(inrooms[myroom].peoples[i].style!=-1)
    if(!(inrooms[myroom].peoples[i].flag&PEOPLE_SPECTATOR))
        ...
        do{
            ...
            if(ch==KEY_UP) {
                ...
                if(inrooms[myroom].peoples[me].flag&PEOPLE_TESTED) {
                    send_msg(me, "[31;1m±¾ÂÖÄãÒÑ¾­Õì²é¹ıÁË[m");
                    refreshit();
                    continue;
                }
                if(inrooms[myroom].peoples[sel].flag&PEOPLE_SPECTATOR)
                    ...
                    send_msg(me, "[31;1m¾­¹ıÄãµÄÕì²â, ·¢ÏÖ´ËÈËÊÇ»µÈË!!![m");
                }
                else {
                    inrooms[myroom].peoples[me].flag|=PEOPLE_TESTED;
                    send_msg(me, "[31;1m¾­¹ıÄãµÄÕì²â, ·¢ÏÖ´ËÈËÊÇºÃÈË[m");
                }
                refreshit();
            }
            else if(ch==Ctrl('S')) {
                ...
                if(inrooms[myroom].peoples[me].flag&PEOPLE_ALIVE&&
                    ...
                    if(inrooms[myroom].peoples[sel].flag&PEOPLE_ALIVE && 
                        ...
                        sel!=me) {
                        int i,j,t1,t2,t3;
                        sprintf(buf, "[32;1m%d %sÍ¶ÁË%d %sÒ»Æ±[m", me+1, inrooms[myroom].peoples[me].nick,
                                ...
                                if(a>0&&a>=b-1&&inrooms[myroom].status==INROOM_DAY) {
                                    ...
                                    for(i=0;i<MAX_PEOPLE;i++) 
                                        ...
                                        if(inrooms[myroom].peoples[i].flag&PEOPLE_KILLER&&inrooms[myroom].peoples[i].flag&PEOPLE_ALIVE)
                                            send_msg(i, "Çë×¥½ôÄãµÄ±¦¹óÊ±¼äÓÃ[31;1mCtrl+S[mÉ±ÈË...");
                                }
                                else {
                                    for(i=0;i<MAX_PEOPLE;i++)
...
static int room_list_key(struct _select_def *conf, int key)
{
    ...
    case 'J':
        ...
        return SHOW_DIRCHANGE;
    case 'K':
        if(!HAS_PERM(PERM_SYSOP)) return SHOW_CONTINUE;
