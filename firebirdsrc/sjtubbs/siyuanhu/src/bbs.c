...
struct person currentpker;//pax @sjtubbs pk
int     usernum=0;
...
char   *buf, *boardname;
{
    ...
    case 6: /* 同作者 */
    case 7: /* 标题关键字 */
    case 10: /*十大*///20070310 add by pax
    }
    dir[STRLEN - 1] = '\0';
    ...
    else if ( digestmode == 10) //20070310 modify by pax
        sprintf(buf, "boards/%s/TOPTEN.%s.DIR",boardname,currentuser.userid);
...
no_top_ten()
{
   return seek_in_file("etc/no_top_ten",currboard);
}
...
struct fileheader *ent;
{
    ...
    if ((ent->accessed[0] & FILE_VISIT && chk_currBM(currBM)) /* && HAS_PERM(PERM_MARKPOST) */ )
    {
		strcpy(degist,DEGISTFLAG);
	}
	else{
		degist[0]='\0';
	}
    ...
    if (FFLL == 0)
    {
        ...
        {
            sprintf(buf, " %4d %s%c\033[m %-12.12s %s%6.6s\033[m %s%-.48s ", num, degist,type,
                    ent->owner, color, date, noreply?"\033[0;1;4;33mx\033[m":" ",TITLE);
        }
        else
        {
            sprintf(buf, " %4d %s%c\033[m %-12.12s %s%6.6s\033[m %s◆ %-.45s ", num, degist,type,
                    ent->owner, color, date, noreply?"\033[0;1;4;33mx\033[m":" ",TITLE);
        }
    }
    else
    {
        if (!strncmp("Re: ", ent->title, 4))
        {
			if(reply_id != 0)
			{
				...
				{
					sprintf(buf,
							" \033[1;36m%4d\033[m %s%c\033[m %-12.12s %s%6.6s.%s\033[1;36m%-.48s\033[m ",
							num, degist,type, ent->owner, color, date,
        ...
        else
        {
			if(reply_id != 0)
			{
				...
				{
					sprintf(buf,
						" \033[1;32m%4d\033[m %s%c\033[m %-12.12s %s%6.6s.%s\033[1;32m◆ %-.45s\033[m "
						,num, degist,type, ent->owner, color, date,
						noreply?"\033[0;1;4;33mx\033[m":" ",TITLE);
				}
				else
...
//if(!HAS_PERM(PERM_SYSOP))
	...
	//modified by pax @ sjtubbs 061003
setbfile(genbuf, bname, "board.urgent");
if(!(dashf(genbuf)&& (!strcmp (currentuser.userid, "SYSOPs"))))
    {
...
if(!(dashf(genbuf)&& (!strcmp (currentuser.userid, "SYSOPs"))))
    {
		//setbfile(genbuf, currboard, "board.reject");
		setbfile(genbuf, bname, "board.reject");
        if(dashf(genbuf)&&seek_in_file(genbuf,currentuser.userid))
        {
            clear();
            move(5,10);
...
/*end by pax*/
	...
	if(0==strcmp("Admin",currboard)) 
	{
		char sysbuf[STRLEN];
		sprintf(sysbuf, "enter board Admin with ip %s", currentuser.lasthost);
		report(sysbuf);
	}
...
char   *direct;
{
	...
	//write to log pax@070101
	#ifdef BM_LOG
		write_to_log(c_log_name[LOG_G],currboard,fhdr->title,currentuser.userid);
	#endif
...
void do_quote(char *filepath, char quote_mode,int mode)
{
    FILE   *inf, *outf;
    char   *qfile, *quser;
    char    buf[256], *ptr;
...
//add by pax	
	setbfile(tempbuffer, currboard, "board.reject");
	if(dashf(tempbuffer)&&seek_in_file(tempbuffer,currentuser.userid) && !mode)
	{
        prints("\n\n您不是 %s 板成员，取消转载\n", currboard);
        return -1;
...
int     mode;
{
    ...
    if ((fp = fopen(filepath, "a")) == NULL)
        return;
	if(HAS_DEFINE(currentuser.userdefine, DEF_NOTHIDEIP))//show ip
    {
	...
	{
		fprintf(fp, "--\n\033[m\033[1;%2dm※ 转%s:·%s %s·[FROM: %-.20s]\033[m\n"
            ,color, (mode == 1) ? "载" : "寄", BoardName, BBSHOST, BBSIP);
	}
    fclose(fp);
    return;
}
...
int ipinrange(unsigned long ipaddr, unsigned long network,unsigned long mask)
{
...
int insjtunetwork(char *ip)
{
	...
	ip_range[0].network = 3396861952; /* 202.120.0.0   255.255.192.0/18   64  */
	ip_range[0].mask = 4294950912;
	ip_range[1].network = 3545243648; /* 211.80.32.0   255.255.224.0/19   32  */
	ip_range[1].mask = 4294959104;
	ip_range[2].network = 3545255936; /* 211.80.80.0   255.255.240.0/20   16  */
	ip_range[2].mask = 4294963200;
	ip_range[3].network = 3396972544; /* 202.121.176.0 255.255.248.0/21    8  */
	ip_range[3].mask = 4294965248;
	ip_range[4].network = 3670126592; /* 218.193.176.0 255.255.240.0/20   16  */
	ip_range[4].mask = 4294963200;
	ip_range[5].network = 3689177088; /* 219.228.96.0  255.255.224.0/19   32  */
	ip_range[5].mask = 4294959104;
	ip_range[6].network = 3396344320; /* 202.112.26.0  255.255.255.0/24    1  */
	ip_range[6].mask = 4294967040;
	
	for(i=0;i<=6;i++) {
		if( ipinrange(ipintonum(ip), ip_range[i].network, ip_range[i].mask) ) {
			return 1;
		} 
...
post_article(char* postboard, char* mailid, unsigned int* p_reid)
{
	...
	int post_money,model;//add by pax 
    time_t  now = time(0);
	...
	#ifdef NO_POST
	#define NO_POST_START	1
	#define NO_POST_END	8
	if(!HAS_PERM(PERM_SYSOP|PERM_SPECIAL6|PERM_SPECIAL7|PERM_SPECIAL8)) {
		if(tm->tm_hour >= NO_POST_START && tm->tm_hour < NO_POST_END) {
			move(5,10);
			clrtobot();
			prints("本站于每日%d时至%d时全站只读。",NO_POST_START,NO_POST_END);
			pressreturn();
        		clear();
        		return FULLUPDATE;	
			}
	...
	/*if(tm->tm_hour >= START_TIME && tm->tm_hour < END_TIME) {*/
	if(1) {
		if( !insjtunetwork(fromhost) ) {
			move(5, 10);
        		clrtobot();
			prints("本站于每日%d时至%d时暂停校外用户的发文权限。\n\n          如果有疑问请在 BBSHelp 或者 sysop 板查阅相关通知与说明。",START_TIME,END_TIME);
			pressreturn();
        		clear();
        		return FULLUPDATE;			
		}		
    ...
    modify_user_mode(POSTING);
    do_quote(filepath, header.include_mode,model);
...
//	    post_word /= 10;
       ...
       // currentuser.money += post_word;
		//modified by pax
		if(post_time<=0 || post_word<=0){
			post_money=0;
		}
		else
		{
			post_money=(int)sqrt((double)post_time*post_word);
			//sprintf(buf, "%d %d %d",post_money,post_time,post_word);
			if(post_time/(2*post_word)>9) post_money=post_money/(int)(sqrt((double)post_time/(2*post_word))+1);
			if((2*post_word)/post_time>9) post_money=post_money/(int)(sqrt((double)(2*post_word)/post_time)+1);
			if(post_money>50) post_money=50;
		}
		//sprintf(buf, "post_money=(int)sqrt((double)post_time*post_word) %d",post_money);
		currentuser.money += post_money;
		//end by pax
...
/*	char output[STRLEN]= {0};*/
	...
	sprintf(filepath, BBSHOME"/boards/%s/%s", currboard, fileinfo->filename);
	sprintf(url, "http://bbs.sjtu.edu.cn/bbscon?board=%s&file=%s", currboard, fileinfo->filename);
...
char   *direct;
{
	...
	//write to log pax@070101
	#ifdef BM_LOG
		write_to_log(c_log_name[LOG_M],currboard,fileinfo->title,currentuser.userid);
	#endif
...
int
recommend_post(ent, fileinfo, direct)
int     ent;
struct fileheader *fileinfo;
char   *direct;
{
	char query_string[65535],title_ok[256];
	MYSQL mysql;
    // 回收站, nextline add by mudboy
    // if (digestmode==9) return DONOTHING;
    if (!HAS_PERM(PERM_SYSOP))
	...
	mysql_init(&mysql);
	if (!mysql_real_connect(&mysql,DB_HOST,DB_USER,DB_PASSWD,DB_NAME,DB_PORT,DB_SOCK,0)) {    
		report(mysql_error(&mysql));
		}
	
    if (fileinfo->accessed[1] & FILE_RECOMMENDED) {
        fileinfo->accessed[1] &= ~FILE_RECOMMENDED;
	/ *del post from recommend table* /
	sprintf(query_string,"DELETE FROM `recommend` WHERE `board`='%s' AND `reid`='%d' AND filename = '%s'",currboard,fileinfo->reid,fileinfo->filename);
	if(mysql_real_query (&mysql, query_string, strlen (query_string)))  {
			report(mysql_error(&mysql));
			}
    }
    else
    {
		...
		mysql_real_escape_string(&mysql,title_ok, fileinfo->title,strlen(fileinfo->title));
		sprintf(query_string,"INSERT INTO `recommend` ( `id` , `board` , `user_id` , `reid` , `title` ,`filename`,  `height` ) VALUES ('', '%s', '%s', '%d', '%s', '%s', '0')",currboard,fileinfo->owner,fileinfo->reid,title_ok, fileinfo->filename);
		if(mysql_real_query (&mysql, query_string, strlen (query_string)))  {
			report(mysql_error(&mysql));
			}
    }
...
sprintf(query_string,"insert into junk (del_user,time,filename,board) values ('%s',NOW(),'%s','%s')",user_id,filename,board_name);
...
{
	/*modified by pax 多个丝路*/
	int opt;
	char msg[2];
	char fname[80];
	getdata(  t_lines - 1, 0,"请输入操作丝路编号 1-4(默认为最近一次设置的丝路):", msg, 2, DOECHO,  YEA);

	if (msg[0] == '\n' || msg[0] == '\0')
	msg[0] = '0';
	opt = atoi(msg);
	if(!isdigit(msg[0]) || opt <0 || opt > 4 )
	{
	...
	//write to log pax@070101
	#ifdef BM_LOG
		write_to_log(c_log_name[LOG_I],currboard,fileinfo->title,currentuser.userid);
	#endif
...
//        '+',recommend_post,
		...
		Ctrl('H') , sjtuschedule,//pax@sjtubbs 05-07-11
        'h', mainreadhelp,
        Ctrl('J'), mainreadhelp,
        KEY_TAB, show_b_note,
        'z', show_b_secnote,
		...
		'*', show_file_info, /* show file info, add by rwzmm @ sjtubbs */
		 Ctrl('Q'), model_edit,/*编辑模版 add by pax*/
                   '\0', NULL
               };
...
//if(!HAS_PERM(PERM_SYSOP))
	...
	//modified by pax @ sjtubbs 061003
setbfile(genbuf, currboard, "board.urgent");
if(!(dashf(genbuf)&& (!strcmp (currentuser.userid, "SYSOPs"))))
    {
...
if(!(dashf(genbuf)&& (!strcmp (currentuser.userid, "SYSOPs"))))
    {
        setbfile(notename, currboard, "board.reject");
        if(dashf(notename)&&seek_in_file(notename,currentuser.userid))
        {
            clear();
            move(5,10);
...
op_userid 操作人
*/
int write_to_log(op_type,op_board,post_title,op_userid)
char *op_type,*op_board,*post_title,*op_userid;
{
	char genbuf[50];
	char bmlogname[50];
    FILE *bmfp;
    ...
    sprintf(genbuf, "boardslog/%s", op_board);
	if ((!dashd (genbuf) && mkdir (genbuf, 0777) == -1))
		return -1;

    nowtime = time (0);
    getdatestring (nowtime, NA);

    setlogfile (genbuf, currboard, ".bm_log");
    bmfp = fopen (genbuf, "a+");
    if (bmfp)
    {
        fprintf (bmfp, "%s  %-10s%-24s%-44s%-16s\n", datestring,op_type,op_board,post_title,op_userid);
        fclose (bmfp);
    }
	return 0;
}
...
//get the path of log
char   *
setlogfile(buf, boardname, filename)
char   *buf, *boardname, *filename;
{
    sprintf(buf, "boardslog/%s/%s", boardname, filename);
    return buf;
}
/* end by pax*/