...
*/
/*
$Id: userinfo.c,v 1.2 2004/06/29 14:59:22 rwzmm Exp $
*/
...
void
disply_userinfo (struct userec *u)
{
    int num;
...
//#endif
    ...
    {
        //modified by rwzmm@sjtubbs, ft°¡£¬ÓĞÈË¿´×ÅÕâ¸ö¸ºÊı²»Ë³ÑÛ
    ...
    prints ("Ë½ÈËĞÅÏä     : %d ·â\n", u->nummails);
#ifdef ALLOWGAME
   prints("¾­¼Ã×´¿ö     : ÏÖ½ğ %d Ôª / ½±ÕÂ %d  / ´æ¿î %d Ôª (%s)\n",
      u->money,u->medals,u->inbank,cmoney(u->money+u->inbank));
#endif
...
void
uinfo_change1 (int i, struct userec *u, struct userec *newinfo)
{
    ...
    sprintf (genbuf, "ÉÏÕ¾×ÜÊ±Êı[%dÌì]: ", u->stay / 86400);
    getdata (i++, 0, genbuf, buf, 10, DOECHO, YEA);
    if (buf[0] != '\0' && atoi (buf) >= 0)
        newinfo->stay = atoi (buf) * 86400;
#ifdef ALLOWGAME
   sprintf(genbuf, "×ÔÓÉ×Ê½ğ [%d]: ", u->money);
   getdata(i++, 0, genbuf, buf, 8, DOECHO, YEA);
   if (buf[0]!='\0' && atoi(buf) >= 0)
      newinfo->money = atoi(buf);

   sprintf(genbuf, "ÒøĞĞ´æ¿î [%d]: ", u->inbank);
   getdata(i++, 0, genbuf, buf, 8, DOECHO, YEA);
   if (buf[0]!='\0' && atoi(buf) >= 0)
  	newinfo->inbank = atoi(buf);

   sprintf(genbuf, "½±ÕÂÊı [%d]: ", u->medals);
   getdata(i++, 0, genbuf, buf, 10, DOECHO, YEA);
   if (buf[0]!='\0' && atoi(buf) >= 0)
       newinfo->medals = atoi(buf);
#endif
...
check_uinfo (struct userec *u, int MUST)
{
    ...
    {				// ¼ì²éêÇ³Æ
        ...
        move (pos++, 0);
        prints ("ÇëÊäÈëÄúµÄêÇ³Æ (Enter nickname) [1;31mÎğÈ¡²»ÑÅÄØ³Æ![m £º");
    ...
    {				// ¼ì²éÕæÊµĞÕÃû
        ...
        move (pos, 0);
        prints ("ÇëÊäÈëÄúµÄÕæÊµĞÕÃû (Enter realname) [1;31mÇëÊ¹ÓÃÖĞÎÄ[m£º\n");
    ...
    {				// ¼ì²é³öÉúÈÕ
        changeIT = MUST || (u->birthday < 1) || (u->birthday > 31)
                   ...
                   && u->birthday > 30 || u->birthmonth == 2 && u->birthday > 29;
        if (!changeIT)
        {
            if (changed)
            {
...
//    if ((fn = fopen ("new_register", "a")) != NULL)
	if ((fn = fopen ("register_sjtubbs", "a")) != NULL)
    {
		...
		strncpy(form.realname, rname, NAMELEN);
		strncpy(form.dept, dept, STRLEN);
		strncpy(form.addr, addr, STRLEN);
		strncpy(form.phone, phone, STRLEN);
		form.status = 0;
