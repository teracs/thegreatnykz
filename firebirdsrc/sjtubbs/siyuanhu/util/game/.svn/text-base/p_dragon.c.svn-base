...
#define setutmpmode modify_user_mode

int tx(x){ return x*11-8; } /* 算座标 */
...
start:
 ...
 break;
       ...
       else if (y!=max[x]+1) break; /* 贴上 */
       if((max[x]&&(da2(dd[x][max[x]])==da2(buffer)+1)&&(da1(dd[x][max[x]])+
da1(buffer))%2)
        ||(max[x]==0 && da2(buffer)==13) )
  if(buffery==1) /* 从上面贴下来的 */
  {
