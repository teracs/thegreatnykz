// by zhch
// Last Modified by soff bitbbs.org on 12/10/2000 ·ÖÊý¼ÆËã
...
checklines()
{
   ...
   for(y1=0;y1<=19;y1++)
   {
      ...
      s=1;
      t++;
      printf("[37m[24;1HÐÐÊý£º[32m%3d", lines+1);
      if(lines==0) printf("                         ");
      for(y2=y1;y2>=1;y2--)
      for(x1=1;x1<=10;x1++)
        a[y2][x1]=a[y2-1][x1];  
      for(x1=1;x1<=10;x1++) 
        a[0][x1]=0;
      if((++lines)%30==0) 
        {delay*=.9; level++; bell();printf("[37m[24;23HµÈ¼¶£º[32m%3d", level);} 
   }
   if(t==0) getscore = 0;
   if(t==1) getscore = 1;
   if(t==2) getscore = 3;
   ...
   if(t==4) getscore = 13;
   score += getscore;
   printf("[37m[24;12H·ÖÊý£º[32m%3d", score);
...
intr()
{
  clear();
  oflush();
  sprintf(tmp,"»¶Ó­¹âÁÙ %s. ÄúÊÇ×Ô2000Äê9ÔÂ13ÈÕ±¾ÓÎÏ·µÄµÚ [1;33m%d[m Î»·ÃÎÊÕß.\r\n\r\n", userid, count());
  prints(tmp);
  prints("¼üÅÌÉèÖÃ: \r\n");
  prints("×ó: '[1;32ma[m', '[1;32m¡û[m';\r\n");
  prints("ÓÒ: '[1;32ms[m', '[1;32m¡ú[m';\r\n");
  prints("ÏÂ: '[1;32mz[m', '[1;32m¡ý[m';\r\n");
  prints("Ë³Ê±Õë×ª¶¯: '[1;32mb[m', <[1;32mCR[m>; ÄæÊ±Õë×ª¶¯: '[1;32mh[m', '[1;32m¡ü[m'; 180¶È×ª¶¯: '[1;32mj[m';\r\n");
  prints("¿ì½µ: ' ', ÔÝÍ£: <[1;32mESC[m>»ò<[1;32mp[m>.\r\n");
  prints("ÍË³ö: '[1;32mCTRL+C[m', '[1;32mCTRL+D[m'.\r\n\r\n");
  oflush();
  getchar();
  clear();
  oflush();
...
int win_showrec()
{
  ...
  win_loadrec();
  clear();
  prints("[44;37m                         -Ë¼Ô´ºþ  TETRIS  ÅÅÐÐ°ñ-                                 \r\n[m");
  prints("[41m No.          ID        ÐÐÊý          ·ÖÊý                 À´×Ô                      [m\n\r");
  for(n=0;n<=19;n++)
  {
