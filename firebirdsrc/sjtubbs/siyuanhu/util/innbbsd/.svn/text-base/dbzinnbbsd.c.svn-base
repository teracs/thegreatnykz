...
static daemoncmd_t cmds[] =
    ...
    { {"help", "help [cmd]", 1, 2, 99, 100, CMDhelp},
      ...
      {"addhist", "addhist <mid> <path>", 3, 3, NNTP_ADDHIST_BAD,
       NNTP_ADDHIST_OK, CMDaddhist},
      {"grephist", "grephist <mid>", 2, 2, NNTP_GREPHIST_BAD, NNTP_GREPHIST_OK,
       CMDgrephist},
      {"midcheck", "midcheck [on|off]", 1, 2, NNTP_MIDCHECK_BAD,
       NNTP_MIDCHECK_OK, CMDmidcheck},
    ...
    if (Channel)
        for (i = 0, j = 0; i < Maxclient; ++i)
        {
            ...
            fprintf(argv->out, " %d) in->used %d, in->left %d %s@%s\r\n",
                    i, Channel[i].in.used, Channel[i].in.left,
