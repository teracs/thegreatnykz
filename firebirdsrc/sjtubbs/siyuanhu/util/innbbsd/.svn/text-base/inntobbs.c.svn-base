    ...
    if (nf->path != NULL)
    {
        ...
        for (ptr1 = nf->path; ptr1 && *ptr1;)
        {
            ...
            *ptr2 = '\0';
            ...
            /*
             * bbslog("search node node %s, host %s fp
            ...
            *ptr2 = savech;
            ...
            /* to conform to the bntplink batch file */
            {
                ...
                fprintf(nl->feedfp, "%s\t%s\t\t%s\t%s\t%c\t%s\t%s!%s\n",
                        ...
                        GROUPS, FROM, SUBJECT, type, MSGID, MYBBSID,
                        HEADER[PATH_H]);
                if (slash != NULL)
                    *slash = '/';
            }
...
int type;
{
    ...
    if (bbsfeedsfp != NULL)
    {
        fprintf(bbsfeedsfp, "%s %c %s %s %s %s!%s %s\n", datebuf, type,
                REMOTEHOSTNAME, GROUPS, MSGID, MYBBSID, HEADER[PATH_H],
                filepath == NULL ? "" : filepath);
        fflush(bbsfeedsfp);
    }
}
...
char *indata, **filterbuffer, *cmd;
{
    ...
    fp = (FILE *) popen((char *)
                        fileglue("%s > /tmp/.innbbsdfilter%d%d", cmd,
                                 getpid(), getuid()), "w");
    if (fp == NULL)
    {
    ...
    tmpfile =
        (char *) fileglue("/tmp/.innbbsdfilter%d%d", getpid(), getuid());
                        ...
                        ngp && *ngp; ongp = strchr(ngp, ','))
                {
                    ...
                    if (arp)
                    {
                        ...
                        if (ptr)
                            *ptr = '\n';
                        outdata =
                            filterdata(arp, direction, indata,
                                       filterbuffer);
                        if (outdata)
                            filterflag = 1;
                        if (ongp)
...
char *inputheader;
{
    ...
    findkey = (header_t *) bsearch((char *) &key, (char *) headertable,
                                   sizeof(headertable) / sizeof(header_t),
                                   sizeof(key), headercmp);
    if (findkey != NULL)
        return findkey->id;
    return -1;
}
...
article_init()
{
    ...
    qsort(headertable, sizeof(headertable) / sizeof(header_t),
          sizeof(header_t), headercmp);
    for (i = 0; i < LASTHEADER; i++)
        HEADER[i] = NULL;
}
