...
ActiveBoard_Init (void)
{
    ...
    if (movieshm == NULL)
        movieshm =
            (void *) attach_shm ("ACBOARD_SHMKEY", 4123, sizeof (*movieshm));
    ...
    while (i <= max && j < ACBOARD_MAXLINE)
    {
        ...
        sprintf (buf, "boards/Bulletin/%s", fh.filename);
...
int
seek_nth_line (int fd, int no)	// ´ÓÎÄ¼þÍ·¶Áµ½µÚ no ÐÐ( ¶¨Î»µ½ no ÐÐ )
{
    ...
    while (numbytes)
    {
        if (linesread <= numlines || numlines == 0)
        {
            ...
            if (i == t_lines - 1)
            {
                ...
                clrtoeol ();
                prints
                ("[1;44;32mÏÂÃæ»¹ÓÐà¸ (%d%%)[33m   ©¦ ½áÊø ¡û <q> ©¦ ¡ü/¡ý/PgUp/PgDn ÒÆ¶¯ ©¦ ? ¸¨ÖúËµÃ÷ ©¦     [m",
                 (viewed * 100) / tsize);
                ch = morekey ();
                move (t_lines - 1, 0);
                clrtoeol ();
    ...
    while (numbytes)
    {
        if (linesread <= numlines || numlines == 0)
        {
            ...
            if (i == t_lines - 1)
            {
                ...
                clrtoeol ();
                prints
                ("[0m[1;44;32m(%d%%) ÊÇ·ñ¼ÌÐø [[1;37mY/n[1;32m]   ±£Áô <[1;37mr[32m>    Çå³ý <[1;37mc[1;32m>   ¼Ä»ØÐÅÏä <[1;37mm[1;32m>                      [m",
                 (viewed * 100) / tsize);
                ch = morekey ();
                move (t_lines - 1, 0);
                clrtoeol ();
