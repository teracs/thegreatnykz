...
int bbsafind_main()
{
    ...
    init_all();
    printf("<center><font class=\"title\">%s - 精华区文章搜索</font><hr><br>\n", BBSNAME);
    type = atoi(getparm("type"));
    strsncpy(board, getparm("board"), 30);
    if (type == 0)
        return show_form2(board);
...
int show_form2(char *board)
{
    printf("<table><form action=bbsafind?type=1 method=post>\n");
    printf
