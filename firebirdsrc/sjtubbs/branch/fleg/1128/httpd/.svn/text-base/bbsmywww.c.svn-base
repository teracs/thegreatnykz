...
int bbsmywww_main()
{
    ...
    int fontn_mode = 1, ansisize_mode = 1, ansifont_mode = 0, theme_mode = 1;

    init_all();
    if (!loginok)
        http_fatal("匆匆过客不能定制界面");
    printf("<center><font class=\"title\">%s - WWW个人定制　　</font>[使用者: %s]<hr>",
    ...
    if (type > 0)
    {
        save_set(t_lines, link_mode, def_mode, font_mode, fontn_mode, ansisize_mode, ansifont_mode, theme_mode);
        http_quit();
    }
    ...
    printf("<p> 注意：将“浏览文章使用字体”设为 Verdana 可能会造成阅读的文章板面错乱。\n");
    printf("</form>\n");
}

int save_set(int t_lines, int link_mode, int def_mode, int font_mode,int fontn_mode, int ansisize_mode, int ansifont_mode, int theme_mode)
{
    FILE *fp;
    char path[80];
