    ...
    while (1)
    {
        ...
        if (!strcmp(f.filename, file))
        {
			...
			if(f.accessed[0] & FILE_DIGEST)
				http_fatal("不能删除收入文摘区(G)的文章");
			if(f.accessed[0] & FILE_MARKED)
				http_fatal("不能删除有Mark标记的文章");
			if(f.accessed[0] & FILE_TOP1)
				http_fatal("不能删除被置底/顶的文章");
            ...
            printf("onclick=\"location.href('bbsdoc?board=%s&start=%d&currentpoint=%d')\"", board, start, (num1-start));
            printf(" value=\"[返回本讨论区]\">\n");
