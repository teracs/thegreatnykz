...
int bbsmail_main()
{
    ...
    if (!file_exist(dir))
    {
        ...
        sprintf(buf, "mail/%c/%s", toupper(currentuser.userid[0]),
                currentuser.userid);
        if (!file_exist(buf))
            mkdir(buf, 0770);
        fp = fopen(dir, "a+");
    ...
    printf
    ("<tr bgcolor=e8e8e8><td>序号<td>删<td>状态<td>发信人<td>日期<td>信件标题\n");
    for (i = start; i < start + 19 && i < total; i++)
    {
        ...
        printf("<td><a href=bbsmailcon?file=%s&num=%d>", data[i].filename,
               i);
    ...
    free(data);
    printf("</table><hr color=green width=650>\n");
    ret = check_maxmail2(&max, &size);
    printf
    ("<input style='height:20px' onclick='return confirm(\"确实要删除所有标记的信件吗\")' type=submit value='删除标记的信件'>\n");
    ...
    printf
    ("<form><input type=submit value=跳转到> 第 <input style='height:20px' type=text name=start size=3> 封</form>");
    if (size > max)
