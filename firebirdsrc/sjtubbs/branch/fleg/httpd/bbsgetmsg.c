...
int bbsgetmsg_main()
{
    ...
    if (file_size(buf) > 0)
    {
        ...
        if (!getmsg_IsNumber(buf2+ IDLEN + 2 + sizeof(time_t) + sizeof(pid_t) + 24 + 13))
            // 从 telnet 给 web 发的信息
        {
            memcpy(p3, buf2+ IDLEN + 2 + sizeof(time_t) + sizeof(pid_t) + 24 + 95, 84);
            ...
            //从 web 给 web 发的信息
        {
            memcpy(p3, buf2+ IDLEN + 2 + sizeof(time_t) + sizeof(pid_t) + 24 + 30, 84);
        ...
        ("<td align=right><a target=f3 href=bbssendmsg?destid=%s>[回讯息]</a> <a href=bbsgetmsg>[忽略]</a>",
         toid);
        http_quit();
    }
    refreshto("bbsgetmsg", 120);
}
...
int getmsg_IsNumber(char* pchar)
{
    if(pchar == NULL)
        return 0;
