...
char *port, *path;
{
    ...
    struct timeval tout;
    ClientType *client =
        (ClientType *) mymalloc(sizeof(ClientType) * Maxclient);
    ...
    if (localbbsinnd < 0)
    {
        ...
        if (!inetdstart)
            fprintf(stderr,
                    "if no other innbbsd running, try to remove %s\n",
                    path);
        close(bbsinnd);
        return (-1);
    }
        ...
        */
        ...
        if (localdaemonready && FD_ISSET(localbbsinnd, &orfd))
        {
            ...
            FD_SET(ns, &rfd);   /* FD_SET(ns,&wfd); */
            {
                ...
                fprintf(client[i].Argv.out,
                        "200 %s InterNetNews DBZSERVER server %s (%s@%s).\r\n",
                        MYBBSID, VERSION, client[i].username,
                        client[i].hostname);
#else

                fprintf(client[i].Argv.out,
                        "200 %s InterNetNews INNBBSD server %s (%s@%s).\r\n",
                        MYBBSID, VERSION, client[i].username,
                        client[i].hostname);
#endif
        ...
        if (FD_ISSET(bbsinnd, &orfd))
        {
            ...
            if (getpeername(ns, (struct sockaddr *) &there, &length) >= 0)
            {
                ...
                hp = (struct hostent *) gethostbyaddr((char *) &there.
                                                      sin_addr,
                                                      sizeof(struct
                                                             in_addr),
                                                      there.sin_family);
                if (hp)
                    ...
                    strncpy(client[i].hostname,
                            (char *) inet_ntoa(there.sin_addr), 128);

                client[i].Argv.in = fdopen(ns, "r");
                client[i].Argv.out = fdopen(ns, "w");
                if ((char *)
                        search_nodelist(client[i].hostname,
                                        client[i].username) == NULL)
                {
                    bbslog(":Err: invalid connection (%s@%s).\n",
                           client[i].username, client[i].hostname);
                    fprintf(client[i].Argv.out,
                            "502 You are not in my access file. (%s@%s)\r\n",
                            client[i].username, client[i].hostname);
                    fflush(client[i].Argv.out);
        ...
        for (i = 0; i < Maxclient; ++i)
        {
            ...
            if (FD_ISSET(fd, &orfd))
            {
                ...
                printf("before read i %d in.used %d in.left %d\n", i,
                       client[i].in.used, client[i].in.left);
#endif
...
char **argv;
{
        ...
        case 'i':
            {
                ...
                if ((rel =
                            getsockname(0, (struct sockaddr *) &there,
                                        &len)) < 0)
                {
                    fprintf(stdout,
    ...
    {
        fprintf(stderr,
                "You should run INNBBSD as your bbs account (uid: %d) or group (gid: %d).\n",
