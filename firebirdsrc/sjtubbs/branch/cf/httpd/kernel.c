...
int cgi_head()
{
    ...
    switch(cache_age)
    {
		...
		case HEADER_PRIVATE:
			...
			break;
		default:
			if(cache_age <= 0) break; /* something wrong, skip it*/
    ...
    while (s[0])
    {
                ...
                || !strncasecmp(s, "ftp://", 6))
        {
            ...
            if (strcasestr(tmp, ".swf") && pic_mod)
            {
                ...
                if (pic_mod == 2)
                    printf
                    ("<EMBED src=\"%s\" TYPE=\"application/x-shockwave-flash\" HEIGHT=300 width=400 ></EMBED>",
                     nohtml(tmp));
                tmp = strtok(0, "");
                if (tmp == 0)
                    return;
    ...
    while (s[0])
    {
                ...
                || !strncasecmp(s, "ftp://", 6))
        {
            ...
            if (strcasestr(tmp, ".swf") && pic_mod)
            {
                ...
                if (pic_mod == 2)
                    fprintf
                    (stream,"<EMBED src=\"%s\" TYPE=\"application/x-shockwave-flash\" HEIGHT=300 width=400></EMBED>",
                     nohtml(tmp));
                tmp = strtok(0, "");
                if (tmp == 0)
                    return;
...
int http_init()
{
	...
	/* if the request from a proxy, HTTP_X_FORWARDED_FOR should be given */
...
int tcp_bind(int is_local,int port, void func(), int pid)
{
    ...
    for (i = NSIG; i >= 1; i--)
        signal(i, SIG_IGN);
	if(is_local)
		inet_pton(PF_INET,"127.0.0.1",&xs.sin_addr);
	else
		xs.sin_addr.s_addr = htonl(INADDR_ANY);
...
void *map_board_dir(char *board,char *file)
{
	...
	if(fd == -1)
		return 0;
	_map_dir.add = (struct fileheader *) mmap(NULL,_map_dir.len,PROT_READ,MAP_SHARED,fd,0);
...
struct fileheader* pre_in_map_dir()
{
...
struct fileheader* next_in_map_dir()
{
	if(0==_map_dir.add || -1 == _map_dir.offset || _map_dir.size==_map_dir.offset+1)
		return NULL;
	else
		return &_map_dir.add[_map_dir.offset+1];
}
int get_offset_of_mapdir()
{
