...
int bbsdoc_main()
{
    ...
    if (to != num)
        printf("<a href=bbsdoc?board=%s&page=%d>下一页</a>,", board,
               to/PAGE_SIZE);
	printf("<a onclick='return confirm(\"您的确要把这个板加入预定讨论区吗\")' href=bbsbrdadd?board=%s>订阅本板</a>\n",
         board);
	printf(",<a href=\"bbsrss?board=%s\"> <img border=0 src=\" http://bbs.sjtu.edu.cn:8000/bbs/rss.gif\"></a>",board);
    printf("<td align=right width=25%>");
    printf
    ("<input type=submit value=跳转到> 第 <input type=text onkeydown=\"return true;\" name=page size=4> 页");
    printf("</table>\n");
    ...
    if (to != num)
    {
        printf("<a href=bbsdoc?board=%s&page=%d>下一页</a>,", board,to/PAGE_SIZE);
