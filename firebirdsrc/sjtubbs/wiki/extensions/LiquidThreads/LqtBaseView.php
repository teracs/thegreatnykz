...
class LqtView {
	...
	protected $maxIndentationLevel = 20;
...
//               $this->user->getSkin()->userToolLinks( $author->getID(), $author->getName() );
		...
		$timestamp = $wgLang->timeanddate($thread->created(), true, true);
...
HTML
	...
	function listItemsForCommands($commands) {
		...
		foreach( $commands as $command ) {
			...
			if( $enabled ) {
				$result[] = "<li><a href=\"$href\" class=\"lqt_footer_commands\">$label</a></li>";
			} else {
	...
	function postDivClass( $thread ) {
        $level = $this->headerLevel % 6 + 1;
		return "lqt_post lqt_bg_color_$level";
	}
	...
	function indent($thread) {
		if( $this->headerLevel <= $this->maxIndentationLevel ) {
			$this->output->addHTML('<dl class="lqt_replies" style="margin:0"><dd>');
		} else {
