...
function get_recommend_boards($path,$dpath,$num,$sp) {
	...
	if($data[0][0] == '#') {
	   ...
	   $line = sprintf("%s<a
		   href=\"http://bbs.sjtu.edu.cn/bbsdoc?board=%s\"
...
/*	$query_junk = "select * from junk where time >
	...
	$line = sprintf("%s[1;37mµÚ[1;31m%2d[37mÃû [37mÐÅÇø:[33m%-12s[37m¡¾[32m%s[37m¡¿[36mP:%4d U:%3d I:%3d [35m%+12s\n",$line,$i+1,$post_sum_array_row[board],$post_sum_array_row[time],$post_sum_array_row[sum],$post_sum_array_row[user_sum],$post_sum_array_row[ip_sum],$post_sum_array_row[post_master]);
	$line_web = sprintf("%s<tr><td width=110>[<a
		href=\"http://bbs.sjtu.edu.cn/bbsdoc?board=%s\"
		target=\"_self\">%12s</a>]</td><td><a
		href=\"http://bbs.sjtu.edu.cn/bbstcon?board=%s&reid=%d\"
		target=\"_self\">%s
		</a></td><td
...
while ($row = mysql_fetch_assoc($result) ) {
    ...
    $line = sprintf("%s\n<tr><td><a href=\"http://bbs.sjtu.edu.cn/bbsdoc?board=%s\"
...
while ($row = mysql_fetch_assoc($result) ) {
    $line = sprintf("%s
	    <tr><td width=110>
	    [<a href=\"http://bbs.sjtu.edu.cn/bbsdoc?board=%s\"
	    target=\"self\">%s</a>]
	    </td><td>
	    <a href=\"http://bbs.sjtu.edu.cn/bbstcon?board=%s&reid=%d\"
	    target=\"_self\">%s</a>
	    </td><td>
	    %s
	    </td>
