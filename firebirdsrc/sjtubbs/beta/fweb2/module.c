...
static gboolean fweb_dlsym(FLIB *lib,const char *func_name, void **pptr)
{
	...
	if(result)
		return TRUE;
	g_warning("can't load func %s at module %s, %s",func_name,lib->name,g_module_error());
	return FALSE;
}
...
static FLIB* fweb_dlopen(const char *dir, const char *name, FLIB_FLAG flag)
{
	...
	lib->unload_event = create_event(buf,
		EVENT_FLAG_REVERSE_ORDER|EVENT_FLAG_IGNORE_BREAK|EVENT_FLAG_IGNORE_FAILED);
	...
	lib->flag = flag;
	if(!(flag&FLIB_FLAG_GLOBAL)) gflag |= G_MODULE_BIND_LOCAL;
	if(flag&FLIB_FLAG_LAZY) gflag |= G_MODULE_BIND_LAZY;
	...
	if(NULL==lib->mptr)
	{
		g_warning("can't load module %s at path %s, %s",name,lib->path,g_module_error());
...
LOAD_FUNC_ERROR:
	...
	{
		g_warning("unload module %s failed, %s",lib->name,g_module_error());
	}
LOAD_MODULE_ERROR:
...
static CALLBACK_RETURN free_module(CALLBACK_ID callback_id, void *data, void *userdata)
{
	FLIB *lib = (FLIB *) userdata;
	fweb_dlclose(lib);
	return CALLBACK_CONTINUE;
}
static CALLBACK_RETURN free_hash_table(CALLBACK_ID callback_id, void *data, void *userdata)
{
...
static gboolean load_module(CONFIG *config, CONF_DATA *data)
{
	...
	register_callback(event_program_exiting,free_hash_table,module_table);
	for(i=0;i<num;i++)
	{
		FLIB *lib;
...
static gboolean load_page(const char *url, const char *name)
{
	...
	{
		g_warning("can't load func '%s' at module '%s' for url '%s'",funcname,modname,url);
...
{
	unsigned int i, num;
	page_table = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, free_page);
...
{
	CONFIG config[] = {
		{group_fweb, "global_module_dir", CONF_TYPE_STRING, CONF_FLAG_REQURIED, set_dir, &global_module_dir},
		{group_fweb, "page_module_dir", CONF_TYPE_STRING, CONF_FLAG_REQURIED, set_dir, &page_module_dir},
		{group_fweb, "global_module", CONF_TYPE_STR_LIST, CONF_FLAG_NONE, load_module, (void *)_GLOBAL_MODULE},
		{group_fweb, "page_module", CONF_TYPE_STR_LIST, CONF_FLAG_REQURIED, load_module, (void *)_PAGE_MODULE},
		{group_pages, NULL, CONF_TYPE_ALL_LIST, CONF_FLAG_REQURIED, load_pages, NULL},
	};
...
gboolean invoke_page_processor(const char *url)
{
	...
	{
		g_warning("send enter_event for module %s failed",page->lib->name);
