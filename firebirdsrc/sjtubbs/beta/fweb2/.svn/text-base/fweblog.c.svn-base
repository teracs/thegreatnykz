...
static const char *key_levels[LOG_NUM] = { "debug", "message", "warning", "error"};
static const char *level_desc[LOG_NUM] = { "DEBUG", "MESSAGE", "WARNING", "ERROR"};
	...
	if(logFiles[level])
	{
		...
		if(NULL!=log_domain)
			fprintf(logFiles[level], "%s - %d - %.24s - (%s) %s\n",
				level_desc[level],pid,ctime(&now),log_domain,message);
		else
...
{
	CONFIG config[] = {
		{group_log,key_levels[L_DEBUG],CONF_TYPE_STRING,CONF_FLAG_NONE,set_log_file,(void *) L_DEBUG},
		{group_log,key_levels[L_MESSAGE],CONF_TYPE_STRING,CONF_FLAG_NONE,set_log_file,(void *) L_MESSAGE},
		{group_log,key_levels[L_WARNING],CONF_TYPE_STRING,CONF_FLAG_NONE,set_log_file,(void *) L_WARNING},
		{group_log,key_levels[L_ERROR],CONF_TYPE_STRING,CONF_FLAG_NONE,set_log_file,(void *) L_ERROR},
	};
