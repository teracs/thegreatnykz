...
static CALLBACK_RETURN fwebio_final(CALLBACK_ID callback_id, void *data, void *userdata)
{
...
gboolean fwebio_init(void)
{
	...
	if(FCGX_InitRequest(&fwebio_request,FCGI_DEFAULT_LISTEN_FD,FCGI_FAIL_ACCEPT_ON_INTR))
	{
		return FALSE;
	}
...
fweb_FILE *fweb_freopen(const char *path, const char *mode,
                        fweb_FILE *fp)
{
    if(fp->stdio_stream) {
...
size_t fweb_fwrite(const void *ptr, size_t size, size_t nmemb, fweb_FILE *fp)
{
    int n;
    if(fp->stdio_stream)
