...
static CALLBACK_RETURN config_final(CALLBACK_ID callback_id, void *data, void *userdata)
{
	if(fweb_config)
	{
	...
	{
		g_warning(\
			"fweb config hasn't been initialized, but call final");
	}
...
gboolean config_init(const char *config_path)
{
	...
	if( TRUE == g_key_file_load_from_file(\
		fweb_config,\
		config_path,\
		G_KEY_FILE_NONE,\
		&err))
...
static CALLBACK_RETURN free_dir(CALLBACK_ID callback_id, void *data, void *userdata)
...
static gboolean parse_string(CONFIG config)
{
	...
	val = g_key_file_get_string(\
		fweb_config, config.group, config.key, &err);
	if(err==NULL)
	{
		data.c_str = val;
...
static gboolean parse_str_list(CONFIG config)
{
	...
	gboolean ret;
	CONF_DATA data;
	val = g_key_file_get_string_list(\
		fweb_config, config.group, config.key, &num, &err);
	if(err==NULL)
	{
...
static gboolean parse_bool_list(CONFIG config)
{
	...
	gboolean ret;
	CONF_DATA data;
	val = g_key_file_get_boolean_list(\
		fweb_config, config.group, config.key, &num, &err);
	if(err==NULL)
	{
...
static gboolean parse_double_list(CONFIG config)
{
	...
	gboolean ret;
	CONF_DATA data;
	val = g_key_file_get_double_list(\
		fweb_config, config.group, config.key, &num, &err);
	if(err==NULL)
	{
...
static gboolean parse_all_list(CONFIG config)
{
	...
	gboolean ret;
	CONF_DATA data;
	key = g_key_file_get_keys(\
		fweb_config, config.group,  &num, &err);
	if(err==NULL)
	{
		char **val;
		unsigned int i;
		...
		for(i=0; i<num; i++)
		{
			val[i] = g_key_file_get_string(\
				fweb_config, config.group, key[i], &err);
			if(err!=NULL)
			{
