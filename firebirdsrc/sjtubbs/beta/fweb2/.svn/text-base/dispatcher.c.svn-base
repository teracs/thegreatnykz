	...
	for(index=start; index!=end; index += off)
	{
		...
		switch(cbret)
		{
			...
			case CALLBACK_CONTINUE:
				break;
			default:
				g_warning("callback return unknown CALLBACK_RETURN in event %s"\
					,event->name);
				ret = FALSE;
				brk = TRUE;
				break;
...
EVENT_CALLBACK *register_callback(EVENT *event, CALLBACK_F callback_f, void *user_data)
{
	EVENT_CALLBACK *callback;
#ifdef DEBUG_DISPATCHER
	...
	if(!event->active)
	{
		g_warning("event %s is disabled, but wanna register an callback",event->name);
		return NULL;
	}
...
gboolean remove_callback(EVENT_CALLBACK *callback)
{
	...
	{
		g_warning("callback is not registered in event %s, but call remove_callback",event->name);
		return FALSE;
	}
}
