...
DLLAPI fweb_FILE *fweb_freopen(const char *path, const char *mode, fweb_FILE *fp);

DLLAPI int        fweb_setvbuf(fweb_FILE *fp, char *buf, int bufmode, size_t size);
...
DLLAPI int        fweb_fprintf(fweb_FILE *fp, const char *format, ...) G_GNUC_PRINTF(2,3);
...
DLLAPI size_t     fweb_fread(void *ptr, size_t size, size_t nmemb, fweb_FILE *fp);
DLLAPI size_t     fweb_fwrite(const void *ptr, size_t size, size_t nmemb, fweb_FILE *fp);
