...
static inline void www_log_action(void)
{
	if(ntw_haslogin)
		...
		g_message("%s guest %s?%s", cgiRemoteAddr,	cgiScriptName, cgiQueryString);
	return;

}
static CALLBACK_RETURN  www_init_request(CALLBACK_ID callback_id, void *data, void *userdata)
{
...
static gboolean check_session(struct user_info *puinfo, void *userdata)
{
	...
	if( (puinfo->idle_time < ntw_now - SESSION_IDLE_TIME) && nt_is_web_user( puinfo))
	{
...
static CALLBACK_RETURN www_clean_session(CALLBACK_ID callback_id, void *data, void *userdata)
{
	static int count = 50;
...
static CALLBACK_RETURN ntwww_final(CALLBACK_ID callback_id, void *data, void *userdata)
{
...
gboolean ntwww_init(EVENT_ID unload_event, EVENT_ID enter_event, EVENT_ID exit_event)
{
	if(!nt_init())
	{
		return FALSE;
	...
	www_init_request_cid = register_callback(enter_event, www_init_request, NULL);
	www_clean_session_cid = register_callback(event_new_request,www_clean_session,NULL);
...
{
	char buf[128];
	snprintf(buf,sizeof(buf),"user %s@%s logout from www",puinfo->userid,puinfo->from);
...
static inline int html_insert_http_url(char *strptr, int len, NTW_IMG_MODE mode)
{
	int urllen;
	urllen = 7;
	while(urllen<len)
...
static inline int html_insert_ftp_url(char *strptr, int len, NTW_IMG_MODE mode)
{
	int urllen;
	urllen = 6;
	while(urllen<len)
...
static inline int html_insert_ansi(char *strptr, int len, gboolean output_color)
{
	/**
	...
	if(strptr[2]=='m')
	{
		...
		if(output_color) mp_grow(HTML_SPAN_RESET,strlen(HTML_SPAN_RESET));
		return 3;
	}
...
void ntw_html_str(char *ptr, int len, NTW_IMG_MODE mode, NTW_COLOR_MODE cmode)
{
	register int tmplen;
	register char tmpc;
	if(len < 1)
	...
	while(len)
	{
		...
		case '\x1B' /*ESCAPE char*/:
			if(len>2 && ptr[1]=='[')
			{
				tmplen = html_insert_ansi(ptr,len,(cmode==NTW_COLOR_ANSI)||(cmode==NTW_COLOR_QMD));
			}
			break;
...
static CALLBACK_RETURN ntw_tdict_final(CALLBACK_ID callback_id, void *data, void *userdata)
{
...
/* the default action is add "header" include to header.tpl, and more in future */
{
	...
	register_callback(ntw_exit_event,ntw_tdict_final,mp_copy(&dict,sizeof(dict)));
	return dict;
}
