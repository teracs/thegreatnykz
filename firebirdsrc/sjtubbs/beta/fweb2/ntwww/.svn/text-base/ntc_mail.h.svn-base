...
inline static int nt_setmailfile(char *buf, size_t buflen, const char *userid, const char *filename)
{
	return snprintf(buf,buflen, "mail/%c/%c/%s/%s", toupper(userid[0]), toupper(userid[1]), userid, filename);
}
