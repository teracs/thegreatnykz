...
int nt_get_active_uinfo(const char *userid, struct user_info *puinfos[], int max)
{
...
void nt_foreach_active_uinfo(gboolean (*walk_func)(struct user_info *,void *),
		void *userdata)
{
...
gboolean nt_check_login_limit(struct userec *puser)
{
	...
	if (HAS_PERM_SYSOP(puser))	return TRUE;
	
	nt_sethomefile(filename,sizeof(filename), puser->userid, "logincheck");
