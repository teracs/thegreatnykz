...
extern gboolean nt_mail_post(char *userid, char *nickname, char *fromip, int signature,
		char *toid, char *title, char *content);

extern gboolean nt_board_post(char *userid, char *nickname, char *fromip, int signature, 
		char *board, unsigned int reid, char *title, char *content);

extern gboolean nt_board_replace(char *fromip, char *board, unsigned int id, char *title, char *content);

extern gboolean nt_board_del(char *board, unsigned int id);

extern gboolean nt_board_del_some(char *board, unsigned int *id_arr, unsigned int num, unsigned int *delnum);

extern gboolean nt_board_del_range(char *board, unsigned int start_id,  unsigned int end_id, unsigned int *delnum);

extern gboolean nt_board_del_subject(char *board, unsigned int reid, unsigned int start_id, unsigned int end_id, unsigned int *delnum);
