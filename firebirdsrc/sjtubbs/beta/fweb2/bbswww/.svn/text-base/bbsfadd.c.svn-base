...
bbsfadd_main()
{
	...
	sprintf(path, "home/%c/%c/%s/friends", toupper(currentuser.userid[0]), toupper(currentuser.userid[1]),
		currentuser.userid);
	printf("<center><font class=\"title\">%s - 好友名单　　</font>[使用者: %s]<hr>\n", BBSNAME, currentuser.userid);
	...
	if (friendnum >= 199)
		http_quit_print("您的好友名单已达到上限, 不能添加新的好友");
	if (isfriend(userid))
		http_quit_print("此人已经在你的好友名单里了");
