...
bbsmodule_main()
{
	...
	printf("<center><font class=\"title\">%s - 使用模板 %s讨论区　　</font>[使用者: %s] <hr></center>\n", BBSNAME, board, currentuser.userid);
	printf("<table>\n");
	...
	while(i<num)
	{
			...
			printf("<input type=radio name=level value=%s checked>选定 <br>",module[i].filename);
			i++;
	}
	printf("</table>\n");
	printf("<hr><br>\n");
	printf("<input type=submit value='确定'> \n");
	printf("<input type=button onclick='javascript:history.go(-1)' value='取消并返回'> \n");
	printf("</form>");
	printf("</html>");
	return 0;
}
