...
bbstopb10_main()
{
	...
	printf("<br><center><font color=red style='font-size:18px'><b>%s - 热门讨论区(TOP20)</b></font><hr><br>\n", BBSNAME);
	printf("<table>\n");
	printf("<tr bgcolor=e8e8e8><td>名次<td>板名<td>中文板名<td>板主<td>人气\n");
	for (i = 0; i <= 20; i++)
	{
		char *ptr, BM[80] = "";
		...
		if (r == 6)
		{
			...
			if (ptr == 0)
				ptr = "诚征板主中";
			printf("<tr><td>%d<td><a href=bbsdoc?board=%s>%s</a><td width=200><a href=bbsdoc?board=%s>%s</a><td>%s<td>%s\n", i, name, name, name, cname, userid_str(ptr), cc);
		}
	}
	printf("</table><hr>\n");
