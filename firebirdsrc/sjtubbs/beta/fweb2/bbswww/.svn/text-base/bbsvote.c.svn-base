...
void
printitems(struct votebal *abuf, struct ballot *uservote)
{
	int i;
	for(i=0;i<=abuf->totalitems-1;i++)
	{
		...
		{
			if(voted_flag==YEA)
				printf("<tr><td><INPUT TYPE=\"RADIO\" NAME=\"SF\" VALUE=\"%d\" %s>%c.%s",
					i,((uservote->voted>> i) & 1?"CHECKED":" "),'A'+i,abuf->items[i]);
			else
				printf("<tr><td><INPUT TYPE=\"RADIO\" NAME=\"SF\" VALUE=\"%d\">%c.%s",
					i,'A'+i,abuf->items[i]);	
		...
		{
			if(voted_flag==YEA)
				printf("<tr><td><INPUT TYPE=\"RADIO\" NAME=\"DX\" VALUE=\"%d\" %s>%c.%s",
					i,((uservote->voted>> i) & 1?"CHECKED":" "),'A'+i,abuf->items[i]);
			else
				printf("<tr><td><INPUT TYPE=\"RADIO\" NAME=\"DX\" VALUE=\"%d\">%c.%s",
					i,'A'+i,abuf->items[i]);
		}
		else if(abuf->type==3)
		{
			if(voted_flag==YEA)
				printf("<tr><td><INPUT TYPE=\"CHECKBOX\" NAME=\"FX%d\" VALUE=\"%d\" %s>%c.%s",
					i,i,((uservote->voted>> i) & 1?"CHECKED":" "),'A'+i,abuf->items[i]);
			else
				printf("<tr><td><INPUT TYPE=\"CHECKBOX\" NAME=\"FX%d\" VALUE=\"%d\">%c.%s",
					i,i,'A'+i,abuf->items[i]);
		}
		else if(abuf->type==4)
		{
			if(voted_flag==YEA)
				printf("<tr>此次作答的值不能超过%d<tr>请输入一个值:<INPUT TYPE=\"TEXT\" NAME=\"SZ\" SIZE=\"10\" VALUE=\"%d\">",
					abuf->maxtkt,uservote->voted);
			else
				printf("<tr>此次作答的值不能超过%d<tr>请输入一个值:<INPUT TYPE=\"TEXT\" NAME=\"SZ\" SIZE=\"10\">",
					abuf->maxtkt);
			break;
		}
	...
	if(abuf->type!=5)
	{
		...
		if(voted_flag==YEA)
		{
			printf("<tr><td><TEXTAREA NAME=\"YJ\" WARP=\"PHYSICAL\" COLS=\"80\" ROWS=\"4\">");
			if(strcmp(uservote->msg[0],""))printf("%s\n",uservote->msg[0]);
			if(strcmp(uservote->msg[1],""))printf("%s\n",uservote->msg[1]);
			if(strcmp(uservote->msg[2],""))printf("%s\n",uservote->msg[2]);
			printf("</TEXTAREA>");
		}
		else
		{
	...
	else
		{
			if(voted_flag==YEA)
			{
				...
				printf("<tr><TEXTAREA NAME=\"WD\" WARP=\"PHYSICAL\" COLS=\"80\" ROWS=\"4\">");
				if(strcmp(uservote->msg[0],""))printf("%s\n",uservote->msg[0]);
				if(strcmp(uservote->msg[1],""))printf("%s\n",uservote->msg[1]);
				if(strcmp(uservote->msg[2],""))printf("%s\n",uservote->msg[2]);
				printf("</TEXTAREA>");
			...
			{
				printf("<tr>请输入您的作答(注意:超过三行的部分都会被舍弃):");
				printf("<tr><TEXTAREA NAME=\"WD\" WARP=\"PHYSICAL\" COLS=\"80\" ROWS=\"4\"></TEXTAREA>");
			}
...
*/
/*added by chaser to limit vote under web */
...
struct votebal abuf;
{
	...
	if (currentuser.firstlogin > abuf.opendate)
    	{
        	http_quit_print("对不起, 本投票在您帐号申请之前开启，您不能投票\n");
    	}
		...
		sprintf(listfilename,"vote/%s/%s",board,"vote.list");
        	if (access(listfilename,0) != 0)
            		http_quit_print ("对不起，本票箱需要设定好投票名册方可进行投票\n");
	 ...
	 else if (abuf.level & VOTEMASK)
    	{
        	if (currentuser.numlogins < abuf.x_logins
                	...
                	|| (unsigned int)currentuser.stay < (abuf.x_stay * 3600)
               		|| (unsigned int)currentuser.firstlogin > ( abuf.opendate - abuf.x_live * 86400))
...
bbsvote_main()
{
	...
	printf("<body>");
	printf("<form name=form1 action='bbsdovote' method=post onSubmit=\"return verify(this);\" enctype='multipart/form-data'>\n<input type=hidden name=board value='%s'>", board);
	...
	while(read(fd,&abuf,sizeof(struct votebal))==sizeof(struct votebal))
	{
		if(opendate==abuf.opendate)break;
	}
	...
	printf("<input type=hidden name=totalitems value='%d'>",abuf.totalitems); 
	printf("<table width=670><tr style='height:14px'><td><b>投票主题:%s</b></td><tr><td>投票类型:%s</td></tr>%s</table><hr>",
		abuf.title,vote_type[abuf.type - 1], abuf.showip ? 
		"<tr><td>本次投票将显示投票人ip信息</td></tr>" : "");
	...
	if (search_record (flagfile, (char *)&uservote, sizeof (uservote), cmpvuid,
...
out:
	...
	if(abuf.type==3)
	{
		...
		printf("var e=f.elements[4+i];\n");
		printf("if(e.checked){v++;\n}");
		printf("}if(v>%d){alert(\"您不能有超过%d个选项!\");return false;}\n",abuf.maxtkt,abuf.maxtkt);
