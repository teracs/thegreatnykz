...
bbs0an_main()
{
	...
	char *ptr, path[512], names[512], name[1024][80], file[1024][80],
		buf[512], title[256] = " ";
	char *board;
	cache_age = 60 * 60 * 24;	/* cache a day */
	init_all();
	...
	{
		fclose(fp);
		g_warning("denied access 0Announce b:%s u:%s ip:%s",board,currentuser.userid,fromhost);
		http_quit_print("目录不存在");
	}
	while (1)
	{
	...
	if (!strncasecmp(board, "p_", 2))
		sprintf(buf, "<a href=bbsqry?userid=%s>%s</a>个人文集",
			board + 2, board + 2);
	printf("<center>\n");
	printf("<div align=center><font class=\"title\">%s - %s</font>　　[浏览次数: %d次]</div>", BBSNAME, buf, get_count(path));
	printf("<hr>\n");
	if (total <= 0)
	{
		printf("<br><< 目前没有文章 >>\n");
	...
	for (i = 0; i < total; i++)
	{
		...
		{
			printf("<td>[目录] <td><a href=bbs0an,path,%s", url_quote(path));
			printf("%s.html>%s</a>", url_quote(file[i]), nohtml2(name[i]));
		}
		else
		{
			printf("<td>[文件] <td><a href=bbsanc,path,%s", url_quote(path));
			printf("%s.html>%s</a>", url_quote(file[i]), nohtml2(name[i]));
		}
#else
		else if (file_isdir(buf))
	...
	printf("</table><hr>[<a href='javascript:history.go(-1)'>返回上一页</a>] ");
	if (board[0])
	{
#ifdef STATIC_URL
		if (!strncasecmp(board, "p_", 2))
