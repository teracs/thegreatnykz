...
bbsalluser_main()
{
	...
	printf("<center><font class=\"title\">%s - 所有使用者列表</font>　　[用户总数: %d]<hr>\n", BBSNAME, total);
	start = atoi(getparm("start"));
	if (start > total || start < 0)
		start = 0;
	printf("<table width=610>\n");
	printf("<tr bgcolor=e8e8e8><td>编号<td>好友<td>使用者代号<td>昵称<td>上站次数<td>文章数<td>最近光临时间\n");
	fp = fopen(".PASSWDS", "r");
	for (i = 0; i < 20; i++)
	{
		if (start + i > total - 1)
		...
		if (fread(&x, sizeof(x), 1, fp) <= 0)
			break;
		printf("<tr><td>%d<td>%s<td><a href=bbsqry?userid=%s>%s</a><td>%s<td>%d<td>%d<td>%s\n", start + i + 1, isfriend(x.userid) ? "<font color=green>√</font>" : " ", x.userid, x.userid, nohtml(x.username), x.numlogins, x.numposts, Ctime(x.lastlogin) + 4);
	}
	fclose(fp);
	printf("</table><hr>\n");
	if (start > 0)
