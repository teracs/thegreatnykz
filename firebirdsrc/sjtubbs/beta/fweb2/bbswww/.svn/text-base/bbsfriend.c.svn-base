...
bbsfriend_main()
{
	...
	puser = (struct user_info *)mp_alloc(MAXACTIVE*sizeof(struct user_info));
	...
	printf("<tr bgcolor=e8e8e8><td>序号<td>友<td>使用者代号<td>使用者昵称<td>来自<td>动态<td>发呆\n");
	...
	for (i = 0; i < total; i++)
	{
		...
		printf("%s",
		       puser[i].
		       invisible ? "<font color=green>C</font>" : " ");
		...
		printf("<td>%s",
		       puser[i].invisible ? "隐身中..." : ModeType(puser[i].
								  mode));
		if (dt == 0)
		{
			printf("<td> \n");
