...
update_form(char *board, char *file)
{
	...
	int id;	//.DIRÎÄ¼şÖĞfileheaderµÄÎ»ÖÃ£¬Ìæ»»Ê±ÓÃµ½
	int changetitle = NA;
...
/*    fseek(fp, -30, 2);
    ...
    fseek(fp, -33-strlen(BBSHOST)-fromhostlen, 2);*/
    fprintf (fp,
               "\n[1;36m¡ù ĞŞ¸Ä%sÄÚÈİ:¡¤%s ÓÚ %16.16s ĞŞ¸Ä±¾ÎÄ¡¤[FROM: %-.46s][m\n",
               changetitle ? "±êÌâºÍ" : "",
...
bbsedit_main()
{
	...
	printf("<center><font class=\"title\">%s - ĞŞ¸ÄÎÄÕÂ¡¡¡¡</font>[Ê¹ÓÃÕß: %s]<hr>\n", BBSNAME, currentuser.userid);
	if (type != 0)
		return update_form(board, file);
	printf("<table border=1>\n");
	printf("<tr><td>");
	printf("<tr><td><form name=\"form1\" method=post action=bbsedit>\n");
	printf("Ê¹ÓÃ±êÌâ: <input type=text name=title size=40 maxlength=100 tabindex=\"1\" value='%s'>ÌÖÂÛÇø: %s<br>\n", nohtml(x->title), board);
	printf("±¾ÎÄ×÷Õß£º%s<br>\n", currentuser.userid);
	sprintf(path, "boards/%s/%s/%s", board, getfolderstr(file), file);
	fp = fopen(path, "r");
	if (fp == 0)
