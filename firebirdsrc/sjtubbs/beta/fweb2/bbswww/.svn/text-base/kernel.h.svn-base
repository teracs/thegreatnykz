...
extern struct fileheader *get_file_ent(char *board, char *file, int isPost);
...
extern int post_article(char * board, char * title, char * file, \
	char * id, char * nickname, char * ip, int sig, int autocr, \
	unsigned int currid, unsigned int reid);
extern void post_article2(char * board, char * title, char * file, \
	char * id, unsigned int currid, unsigned int reid);
...
extern gboolean bbswww_init(EVENT_ID unload_event, EVENT_ID enter_event, EVENT_ID exit_event);
...
extern int search_record(char *filename, char *rptr, int size, int (*fptr)(), char *farg);
