...
bbsmailcon_main()
{
	...
	printf("<script>if(!top.get_html) top.f4.location=top.f4.location</script>\n");
	printf("<center>\n");
	id = currentuser.userid;
	printf("<font class=\"title\">%s - 阅读信件　　</font>[使用者: %s]<hr>\n", BBSNAME, id);
	if (strncmp(file, "M.", 2))
	...
	if (num >= 0 && num < total)
	{
		...
		if (fread(&x, sizeof(x), 1, fp) > 0)
		{
			...
			printf("[<a href='bbspstmail?userid=%s&file=%s&title=%s'>回信</a>]", ptr, refilename, title2);
		}
	}
	if(num >=0 && num < total)
        {
	...
	printf("<tr><td>\n<pre>");
	sprintf(path, "mail/%c/%c/%s/%s", toupper(id[0]), toupper(id[1]), id, file);
	fp = fopen(path, "r");
	if (fp == 0)
