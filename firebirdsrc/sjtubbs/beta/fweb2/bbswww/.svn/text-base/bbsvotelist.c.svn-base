...
bbsvotelist_main()
{
	...
	printf("<body>");
	printf("<form name=form1 action='bbsvote' method=post onsubmit='this.page.value -= 1;return true'>\n<input type=hidden name=board value='%s'>", board);
	printf("<center>\n");
	printf("<table><tr><td>");
	...
	printf("<table width=670><tr style='height:14px'><td><b><a href=bbsmnote3?board=wine>投票箱列表</a></b></table><hr>");
	sprintf(controlfile,"vote/%s/control",board);
	num_of_vote=get_num_records (controlfile, sizeof (struct votebal));
	if(num_of_vote==0)
	{
		printf("<td><font style='font-size:12px; color:#f00000'>目前没有任何投票举行</font></td>");
		goto out;
	}
	...
	printf("<table width=670><tr bgcolor='e8e8e8'><td>编号<td>开启者<td width=72>开启日<td>投票主题<td>类别<td>天数<td>人数<td>本人投票否");	
	for(i=1;i<=num_of_vote;i++)
	{
		read(fd,&abuf,sizeof(struct votebal));
		sprintf (buf, "vote/%s/flag.%ld", board,abuf.opendate);
		if (search_record (buf, (char *)&uservote, sizeof (uservote), cmpvuid,
		 ...
		 printf("<tr><td>%d<td><a href=bbsqry?userid=%s>%s</a><td>%s<td><a href=bbsvote?board=%s&opendate=%lu>%s</a><td>%s<td>%d<td>%d<td>%s",
			i,abuf.userid,abuf.userid,datestring,board,abuf.opendate,abuf.title,vote_type[abuf.type - 1],abuf.maxdays,num_voted,voted_flag==YEA?"是":"否");
