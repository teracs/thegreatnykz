...
bbsbfind_main()
{
	...
	init_all();
	printf("<center><font class=\"title\">%s - 板内文章搜索</font><hr><br>\n", BBSNAME);
	...
	printf("<tr bgcolor=e8e8e8><td>编号<td>标记<td>作者<td>日期<td>标题\n");
	while (1)
	{
		...
		if (ctxt[0])
		{
			...
			sprintf(file1, "boards/%s/%s/%s", board, getfolderstr(x.filename), x.filename);
			fpc = fopen(file1, "r");
			if (!fpc)
				continue;
			while (1)
		...
		//modified by PerfectWorks for HTML标签过滤不严
...
show_form(char *board)
{
	...
	printf("<tr><td align=center><input type=submit value=递交查询结果>\n");
	printf("</form></table>");
