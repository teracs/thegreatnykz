...
bbsmybrd_main()
{
	...
	{
		if (shm_bcache->bcache[i].flag2[0] & BOARD_DIR_FLAG) //ignore all dirs
			continue;
		if (has_read_perm
		    (&currentuser, shm_bcache->bcache[i].filename))
		{
	...
	for (i = 0; i < total; i++)
	{
		...
		if (i % 3 == 0)
			printf("\n<tr>");
		printf("<td><input type=checkbox name=%s %s><a href=bbsdoc?board=%s>%s(%s)</a>", data[i].filename, buf3, data[i].filename, data[i].filename, data[i].title + 11);
	}
	printf("</table><hr>\n");
