...
bbsleftnew_main()
{
	...
	printf("<iframe id=\"framehide\" name=\"framehide\" width=\"0\" height=\"0\" src=\"\" frameborder=\"0\" scrolling=\"no\"></iframe>");
	if (!loginok)
	{
	...
	else
	{
		...
		printf("<br>\n用户: <a class=left href=bbsqry?userid=%s target=f3>%s</a><br>", currentuser.userid, currentuser.userid);
	...
	printf("选择讨论区: <br><input type=text name=board maxlength=15 size=8>");
	...
	printf("<img src=" UPLOAD_SERVER_HEADER
		"/link0.gif><a class=left target=f3 href=/mgb/summary.php>BLOG汇总</a><br>\n");
	if(loginok) {
/*	printf("<img src=" UPLOAD_SERVER_HEADER
	       "/link0.gif><a class=left target=f3 href=php/rss.php>rss板面收藏[Beta]</a><br>\n"); */
	if (loginok /*currentuser.userlevel & (PERM_BOARDS | PERM_EXT_IDLE)*/) {
		printf("<img src=" UPLOAD_SERVER_HEADER
	       "/link0.gif><a class=left target=f3 href=/mgb/admin.php>BLOG管理</a><br>\n");
		}
	}
	printf("<img src=" UPLOAD_SERVER_HEADER
	...
	if (loginok)
	{
		printf("<a href=javascript:submenu(\"main.hck\",1) target=\"_self\"><img id=\"img_main.hck\" src=\"/gif/folderclose.gif\" class=pm></a><a class=left href=\"bbsshowhome?boardfile=main.hck\" target=\"f3\">我的收藏夹</a><br>\n");
	...
	printf("<a class=left href=\"javascript: t(document.getElementById('div4'), document.img4)\">特别服务区</a><br>\n");
	...
	if ((fp = fopen("vote/regist_status", "r")) != NULL){
...
//		seek_in_qzdenyfile(&currentuser);
		if (HAS_PERM(PERM_LOGINOK) && !HAS_PERM(PERM_POST))
			printf("<script>alert('您被封禁了全站发表文章的权限, 请参看penalty板公告, 期满后在sysop板申请解封. 如有异议, 可在sysop板提出申诉.')</script>\n");
		if (count_new_mails() > 0)
			printf("<script>alert('您有新信件!')</script>\n");
	}
	if (loginok && !(currentuser.userlevel & PERM_LOGINOK)
	    && !has_fill_form())
		printf("<br><a class=left style='color:red; font-size:15px' target=f3 href=bbsform><b>填写注册单</b></a><br>\n");
   ...
   printf("<br>[广告]&nbsp;<a href=# class=left><span id=adb onclick='javascript:addiv();'>关闭</span></a><br /><div id=ad>");
	printf("<br /><a class=left href=\"https://www.morganstanley.com/about/careers/recruiting/\" target=_blank><img src=\"/file/Contact/1208690249300941.gif\" border=0></a><br/>");
	
   printf("</div>");

/*
   printf("<br>[广告]&nbsp;<a href=# class=left><span id=adb onclick='javascript:addiv();'>关闭</span></a><br /><div id=ad>");
	printf("<br /><a class=left href=\"/\" target=_blank>饮水思源</a><br/>");
	
   printf("</div>");
*/
   printf("<script src=\"http://www.google-analytics.com/urchin.js\" type=\"text/javascript\"></script><script type=\"text/javascript\"> _uacct = \"UA-144567-2\"; _usample=13.6;urchinTracker(); </script>");
   printf("</body></html>");
