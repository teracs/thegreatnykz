...
bbsreg_main()
{
	...
	r = random();
	printf("<nobr><center><font class=\"title\">%s - 新用户注册</font><hr>\n", BBSNAME);
	printf("<font color=green>欢迎加入本站. 以下资料除帐号, 昵称, 星座以外, 系统会为您保密, 请如实填写.</font>");
	printf("<form name=\"form1\" method=post action=bbsdoreg>\n");
	printf("<table width=700>\n");
	printf("<tr><td align=right>*请输入代号:<td align=left><input name=userid size=12 maxlength=12> (2-12字符, 必须全为英文字母) <input type=\"button\" value=\"检测用户名\" onclick=\"return chkid()\">\n");
	...
	printf("<tr><td align=right>*请输入验证码:<td align=left><input type=text name=vcode maxlength=4><img src=\"bbsvcode?file=%x\" />\n	<input type=hidden name=vcodefile value=%x />", r, r);
	...
	printf("<tr><td align=right>*学校系级或工作单位:<td align=left><input name=dept size=40 maxlength=64> (至少6个字符)\n");
	printf("<tr><td align=right>*详细通讯地址:<td align=left><input name=address size=40 maxlength=64> (至少6个字符)\n");
	...
	printf("<tr><td align=right>您的Email地址(可选):<td align=left><input name=email size=40 maxlength=64>\n");
	...
	printf("WriterWindow = window.open(url,\"writerWindow\",\"toolbar=no,width=300,height=150;,directories=no,status=no,scrollbars=no,resize=no,menubar=no\")\n");
	printf("}\n");
	printf("</SCRIPT>\n");
	printf("</html>");
	return 0;
