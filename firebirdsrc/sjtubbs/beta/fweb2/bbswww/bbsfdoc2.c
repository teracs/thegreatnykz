...
bbsfdoc2_main()
{
	...
	{
		char buf[256];
		snprintf(buf,256,"上载区为空，<a href=bbsupload?board=%s>上载文件</a>",board);
		http_quit_print(buf);
	}
	...
	printf("<center><font class=\"title\">%s - 板内软件下载　　</font>[讨论区: %s]<hr>\n", BBSNAME, board);
	printf("<table width=800 border=1><tr><td>序号<td>文件名<td>上载者<td>时间<td>大小<td>说明<td>备注<td>管理\n");
	for (i = 0; i < n_read; i++)
	{
		...
		printf("<td><a onclick='return confirm(\"确定?\")' href=bbsudel2?fid=%d&file=%s&board=%s", start + i, x[i].showname, x[i].board);
		printf(">删除</a>\n");
	}
	printf("</table><br>\n");
	printf("[<a href=bbsfdoc2?board=%s&start=%d>刷新</a>] [<a href=bbsdoc?board=%s>本讨论区</a>] [<a href=bbsupload?board=%s>上载文件</a>]", board, start, board, board);
	...
	printf("<form name=form1 action=bbsfdoc2 method=post onsubmit='this.start.value-=1' >\n");
	printf("<input type=hidden name=board value=%s>\n",board);
	printf("<input type=submit value=跳转到> 第 <input type=text name=start size=4 onkeydown=\"return true;\" >条 ");
	printf("</form>\n");
	return 0;
}
