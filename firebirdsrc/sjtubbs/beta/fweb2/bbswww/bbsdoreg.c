...
bbsdoreg_main()
{
	...
	struct regform record ;	//add by rwzmm @ sjtubbs
	...
	for (i = 0; x.userid[i]; i++)
		if (!strchr
		    ("ABCDEFGHIJKLMNOPQRSTUVWXYZ", toupper(x.userid[i])))
	...
	if (strlen(x.address) < 6)
		http_quit_print("通讯地址长度至少要6个字符(或3个汉字)");
...
////end
	...
	sprintf(filename, "home/%c/%c/%s", toupper(x.userid[0]), toupper(x.userid[1]), x.userid);
	mkdir(filename, 0755);
	...
	printf("留意您的站内信箱。\n");
	printf("如果您有任何疑问，可以去BBSHelp(新手求助)板发文求助。\n\n</pre></table>");
...
badstr(char *s)
{
	...
	for (i = 0; s[i]; i++)
		if (s[i] != '\x09' && ( (s[i] < '\x20' && s[i]>0) || s[i] == '\xFF'))
			return 1;
	return 0;
}
