	...
	for (i = 0; i < MAXACTIVE; i++)
		if (shm_utmp->uinfo[i].active)
			if (!strcasecmp(shm_utmp->uinfo[i].userid, destid))
			{
				...
				printf("<script>if(top.refresh_msg) top.refresh_msg(); else top.fmsg.location=top.fmsg.location;</script>\n");
				printf("<br><form name=form1><input name=b1 type=button onkeypress='if(event.keyCode==10) event.keyCode=13' onclick='history.go(-2)' value='[返回]'>");
				printf("</form>");
				http_quit();
			}
