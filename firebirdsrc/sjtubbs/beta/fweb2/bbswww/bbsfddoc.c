...
bbsfddoc_main()
{
	...
	{
		char buf[256];
		snprintf(buf,256,"ÉÏÔØÇøÎª¿Õ£¬<a href=bbsupload?board=%s>ÉÏÔØÎÄ¼ş</a>",board);
		http_quit_print(buf);
	}
	...
	printf("<center><font class=\"title\">%s - °åÄÚÈí¼şÏÂÔØ¡¡¡¡</font>[ÌÖÂÛÇø: %s]<hr>\n", BBSNAME, board);
	printf("<form method=post action=bbsurdel>\n");
	printf("<input type=hidden name=board value=%s>\n",board);
	printf("<table width=800 border=1><tr><td>ĞòºÅ<td>É¾<td>ÎÄ¼ş<td>ÉÏÔØÕß<td>Ê±¼ä<td>´óĞ¡<td>ËµÃ÷<td>±¸×¢\n");
	for (i = 0; i < n_read; i++)
	{
		printf("<tr><td>%d", start + 1 + i);
		printf("<td><input style='height:14px' type=checkbox name=del%06d%s>", start + i, x[i].showname);
		printf("<td><a href=" UPLOAD_FILE_URL_HEADER"/%s/%s  target=_blank>", x[i].board, x[i].showname);
		if (isimage(x[i].showname))
			printf("<img src=" UPLOAD_FILE_URL_HEADER"/%s/%s alt=%s onload=\"if(this.width>150) this.width = 150; if(this.height>150) this.height = 150\" ></a>",
	...
	printf("</table><hr>\n");
	printf("<input type=submit value=\"É¾³ı\" onclick='return confirm(\"É¾³ıÑ¡Ôñ¸½¼şÂğå?\")'><br>");
	printf("</form>");
	printf("[<a href=bbsfddoc?board=%s&start=%d>Ë¢ĞÂ</a>] [<a href=bbsdoc?board=%s>±¾ÌÖÂÛÇø</a>] [<a href=bbsupload?board=%s>ÉÏÔØÎÄ¼ş</a>]", board, start, board, board);
	...
	printf("<form name=form1 action=bbsfddoc method=post onsubmit='this.start.value-=1' >\n");
	printf("<input type=hidden name=board value=%s>\n",board);
	printf("<input type=submit value=Ìø×ªµ½> µÚ <input type=text name=start size=4 onkeydown=\"return true;\" >Ìõ ");
	printf("</form>\n");
	printf("</html>");
	return 0;
}
