...
bbspwd_main()
{
	...
	if (type == 0)
	{
		printf("<center><font class=\"title\">%s - 修改密码　　</font>[使用者: %s]<hr>\n", BBSNAME, currentuser.userid);
		printf("<form action=bbspwd method=post>\n");
		printf("<input type=hidden name=type value=1>\n");
		printf("<table>\n");
		printf("<tr><td>使用者帐号: <td>%s\n", currentuser.userid);
		printf("<tr><td>你的旧密码: <td><input maxlength=12 size=12 type=password name=pw1>\n");
		printf("<tr><td>你的新密码: <td><input maxlength=12 size=12 type=password name=pw2>\n");
		printf("<tr><td>再输入一次: <td><input maxlength=12 size=12 type=password name=pw3>\n");
		printf("</table><hr>\n");
