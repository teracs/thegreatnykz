...
inline static int is_strcase_with_prefix(const char *prefix,int len,const char *str)
{
	return !strncasecmp(prefix,str,len);
}
inline static int is_strcase_with_suffix(const char *suffix,int len,const char *str)
{
	int slen;
	slen = strlen(str);
	if(slen<len)
...
bbsdorlmail_main()
{
   ...
   //temporarily removed
/*     printf("该邮箱被禁止绑定！请查阅相关封禁记录或在sysop板提出。");*/
       http_quit();
    }
