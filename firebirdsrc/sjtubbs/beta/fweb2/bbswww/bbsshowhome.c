//made by heack 2005-06-23
#include "www.h"
int totalij=0;
char showboard[80];//current board
...
/*
static int f_append_file(char *file,struct boardcollection sstruct)
{
	int sd;
	//int num; 保险用
...
static void init_dir(struct boardcollection *dirr,char * dirname, char* updir)
{
	int i=0;
	dirr->subnum	=	0;
	dirr->boardnum	=	0;
...
static void print_dir(struct boardcollection currentdir)
{
	...
	{
		totalij++;
		printf("<tr><td align=\"center\"><font color=blue style='font-size:14px'>%d</font></td>",totalij);
    		printf("<td align=center><img src=/gif/folder.gif name=img1>&nbsp;</td>");
    		printf("<td><font style='font-size:14px'><a href=\"bbsshowhome?boardfile=%s\">%s</a></font></td>",
				currentdir.subdirsfile[i],currentdir.subdirname[i]);
    		printf("<td align=\"center\"><font style='font-size:14px'>目录</font></td>");
    		printf("<td align=center><font style='font-size:14px'><a href=\"bbsshowhome?boardfile=%s&targetfile=%s&deldir=%s\">删除</a></font></td></tr>",
				showboard,currentdir.dirname,currentdir.subdirsfile[i]);
	}
	if(currentdir.extends[0]!=0)//impossible if >20
	{
...
static void print_board(struct boardcollection currentdir)
{
	...
	for(j=0;j<currentdir.boardnum;j++)
	{
		...
		printf("<tr><td align=\"center\"><font color=blue style='font-size:14px'>%d</font></td>",totalij);
    		...
    		printf("<td><font style='font-size:14px'><a href=\"bbsdoc?board=%s\">%s</a></font></td>",
				currentdir.boards[j],currentdir.boards[j]);
    		printf("<td align=\"center\"><font style='font-size:14px'>讨论区</font></td>");
    		printf("<td align=center><font style='font-size:14px'><a href=\"bbsshowhome?boardfile=%s&targetfile=%s&delboard=%s\">删除</a></font></td></tr>",
...
{
	char goodboard[80],currentfilename[80],bboard[80],boardfile[80],newdir[80],newboard[80];
	...
	if(newdir[0]!=0)
	{
		...
		if(ndir>=MAX_DIRNUM)
		{
			printf("<html><meta HTTP-EQUIV=\"Content-Type\" CONTENT=\"text/html; charset=gb2312\">\n");
			printf("<link rel=stylesheet type=text/css href='/css/bbs.css'>\n");
			printf("<link rel=stylesheet type=text/css href='/css/blue.css'>\n");
			printf("<font class=\"title\">饮水思源 -添加目录</font><hr>\n");
			printf("您已超过最大目录数%d的限制,请按<a href=bbsshowhome?boardfile=%s>这里</a>返回\n"
	...
	if(newboard[0]!=0)
	{
		...
		if(find==0)
		{
			...
			printf("<link rel=stylesheet type=text/css href='/css/bbs.css'>");
			printf("<link rel=stylesheet type=text/css href='/css/blue.css'>");
			printf("<font class=\"title\">饮水思源 -添加讨论区</font><hr>");
			printf("您所要添加的讨论区不存在,请按<a href=bbsshowhome?boardfile=%s>这里</a>返回"
				,boardfile);
			http_quit();
		}
	...
	printf("<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=gb2312\">");
	printf("<link rel=stylesheet type=text/css href='/css/bbs.css'>");
	printf("<link rel=stylesheet type=text/css href='/css/blue.css'>");
	printf("<body topmargin=0 leftmargin=0><br><center><font color=blue style='font-size:18px'><b>饮水思源BBS-个人收藏夹</b></font><hr><br>");
	printf("<table border=\"1\" align=\"center\">");
	printf("<tr><td>序号</td><td>&nbsp;</td><td align=center>讨论区名称</td><td align=center>类别</td><td align=center>删除<td>");
    	

	snprintf(currentfilename,80,"home/%c/%c/%s/%s",toupper(currentuser.userid[0]),toupper(currentuser.userid[1]),currentuser.userid,boardfile);
	
	if(f_get_file(boardfile,&currentdir,1)==-1)//file not exists
	{
		if(!strcmp(boardfile,"main.hck"))
		{
			...
			while (fgets(bboard, 80, fp) != NULL)
			{
				...
				ptr = strstr(bboard,"\n");
				if(ptr) *ptr='\0';
				snprintf(currentdir.boards[currentdir.boardnum++],80,"%s",bboard);
			}
			fclose(fp);
			f_replace_file(currentdir);
		}
	...
	}//ignore other error status like -2 -3
	if(strcmp(boardfile,"main.hck"))
	{
		printf("<tr><td align=\"center\"> &nbsp;</td>");
		printf("<td align=center><img src=/gif/dir.gif> </td>");
		printf("<td align=\"center\" ><font style='font-size:14px'><a href=\"bbsshowhome?boardfile=%s\">返回上一级</a></td>",
			currentdir.updir);
	...
	printf("<center>");
	printf("<form action=\"bbsshowhome?boardfile=%s\">增加目录名<input name=newdir size=24 maxlength=20 type=text value=""><input type=submit value=确定><input type=hidden name=select value=0>",
		currentdir.dirname);
	printf("<input type=hidden name=boardfile value=\"%s\"></form>",boardfile);
	printf("<form action=\"bbsshowhome?boardfile=%s\">增加讨论区<input name=newboard size=24 maxlength=20 type=text value=""><input type=submit value=确定><input type=hidden name=select value=0>",
		currentdir.dirname);
	printf("<input type=hidden name=boardfile value=\"%s\"></form>",boardfile);
	printf("</center>");
	printf("</form>");
