...
static int
do_fwd(struct fileheader *x,char *target)
{
        char /**p,*/ title[512], path[200];

        sprintf(path, "mail/%c/%c/%s/%s", toupper(currentuser.userid[0]), toupper(currentuser.userid[1]), currentuser.userid, x->filename);
        if (!file_exist(path))
...
/*        if (!has_read_perm(&currentuser, board))
        ...
        printf("<center><font class=\"title\">%s - 转寄/推荐给好友　　</font>[使用者: %s]<hr>\n", BBSNAME, currentuser.userid);
        if (target[0])
        {
                if (!strstr(target, "@"))
                {
