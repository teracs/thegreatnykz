...
bbsrlmail_main()
{
	...
	printf("<nobr><center><font class=\"title\">%s - 邮箱认证 (%s)</font><hr>\n", BBSNAME, currentuser.userid);
	printf("<pre>");
    ...
    if((ftemp=fopen(filename,"r"))==NULL)
	{
		...
		printf("<tr><td align=right>*请输入邮箱:<td align=left><input name=email size=39 maxlength=39> (少于40字符)\n");
    ...
    else
    {
        ...
        {
			printf("<font color=red>您的帐号尚未激活邮箱，未激活邮箱的账号不能发文。<br>");
 			printf("<form method=post action=bbsdorlmail>\n");
			printf("<table width=600>\n");
			printf("<tr><td align=right>*您想重新绑定的邮箱:<td align=left><input name=email size=39 maxlength=39> (少于40字符)\n");
