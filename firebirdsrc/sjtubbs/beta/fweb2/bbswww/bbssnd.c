...
get_unique_file_name(char *name)
{
	...
	if (NULL != postfix){
		...
		snprintf(buf, 40, "%ld%d%d%s", time(0), getpid(), counter,postfix);
	}
	else
	{
...
#ifdef LOGHANDLER
static int
log2db(char *user_id, char *ip_address,char *filename,char *board_name,char *title, char *content,unsigned int reid)
{
	...
	/* content_ok size: (content_len+1)*2 query_string size:(content_len+1)*2+1024 */
	content_ok = mp_alloc((content_len+1)*4+1024);
	query_string = content_ok + (content_len+1)*2;

	title_len = strlen(title);
	title_len = (int)sizeof(title_ok)/2-1>title_len?title_len:(int)sizeof(title_ok)/2-1;
	
	mysql_escape_string(content_ok, content,content_len);
	mysql_escape_string(title_ok, title,title_len);
	query_len = snprintf(query_string,(content_len+1)*2+1024,LOGHANDLER_SQL"insert into %s (user_id,ip_addr,time,filename,board,title,content,ext,reid) values ('%s','%s',FROM_UNIXTIME(%ld),'%s','%s','%s','%s','www','%d')\n",table_name,user_id,ip_address,timestamp,filename,board_name,title_ok,content_ok,reid);
...
do_uploadfile()
{
	...
	printf("<tr><td><form name=\"form1\" method=post action=bbspst enctype='multipart/form-data'>\n");
	printf("<input type=hidden name=board value=\"%s\">",x.board); 
	
	printf("<input type=hidden name=title value=\"%s\">",getparm("title"));
	//printf("<input type=hidden name=url value=\"%s\">",URL); 
	...
	//printf("<textarea>%s</textarea>",getparm("text"));
	//printf("\n<textarea id=text name=text rows=2 cols=80 wrap=physicle>%s\n%s</textarea><p>",getparm("text"),URL);
	printf("附件上载成功, ");
	...
	/*sprintf(upbuf,
                "bbspst?board=%s&text=%s&title=%s&url=%s&file=%s",
              x.board, getparm("text"),getparm("title"),URL,getparm("file"));
		redirect(upbuf);*/	
	return 0;
}
static int
if_exist_title(char *title)
{
	struct
	{
		int hash_ip;
...
static void 
write_posts(char *id, char *board, char *title)
{
	FILE *fp;
	struct post_log x;
...
//added by proto@sjtubbs 2006.09.21	
	if (title[0] == ' ')
	{
		i = 1;
		while (title[i] == ' ') i++;
		if (i == strlen(title)) sprintf(title, "无标题");
	}
//added end by proto@sjtubbs 2006.09.21
...
//	strsncpy(reid_str, getparm("reidstr"), 16);
	...
	/*changed by chaser for newers to post on the board WelcomeNewer*/
...
//              keywords[ii][len-1]='\0';
	...
	{
		if (noip)
			r = post_article(board, title, filename, board,
					 "我是匿名天使", "匿名天使的家", -1,
					 1, currid, reid);
		else
