...
static void print_dir(struct boardcollection currentdir,int level)
{
	...
	for(i=0;i<currentdir.subnum;i++)
	{
		...
		printf("<a href=javascript:submenu('%s',%d) target=_self><img id='img_%s' src='/gif/folderclose.gif' class='pm'></a>",
			currentdir.subdirsfile[i],level+1,currentdir.subdirsfile[i]);
		printf("<a class=left href='bbsshowhome?boardfile=%s' target='f3'>%s</a></div>",
			currentdir.subdirsfile[i],currentdir.subdirname[i]);
		printf("<div id='%s' style='display:none'></div>",currentdir.subdirsfile[i]);
	}
	if(currentdir.extends[0]!=0&&currentdir.subnum>=20)//impossible if >20
	{
...
static void print_board(struct boardcollection currentdir,int level)
{
	...
	for(i=0;i<currentdir.boardnum;i++)
	{
		...
		printf("<a class=left href='bbsdoc?board=%s' target='f3'>%s</a></div>",currentdir.boards[i],currentdir.boards[i]);
	}
	if(currentdir.extends[0]!=0&&currentdir.boardnum>=20)//impossible if >20
	{
		if(f_get_file(currentdir.extends, &currentdir, 1)!=0)
			return ;
...
int bbsshowmenu_main()
{
	...
	printf("parent.document.getElementById(\"%s\").innerHTML=\"",boardfile);
