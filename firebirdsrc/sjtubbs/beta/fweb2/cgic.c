...
#define CGIC_USE_MEMPOOL
#ifdef CGIC_USE_MEMPOOL
#include "mempool.h"

inline static void *real_malloc(size_t size)
{
	return malloc(size);
}
inline static void real_free(void *ptr)
{
	free(ptr);
}

#define malloc(x) mp_alloc(x)
#define free(x) ((void)(x))


#else /* CGIC_USE_MEMPOOL */

#define real_malloc malloc
#define real_free free

#endif /* CGIC_USE_MEMPOOL */
...
serv_request()
{
	...
	{
#ifdef CGICDEBUG
		CGICDEBUGSTART fprintf(dout, "POST recognized\n");
		CGICDEBUGEND
#endif /* CGICDEBUG */
			if (cgiStrEqNc
			    (cgiContentType,
			     "application/x-www-form-urlencoded"))
		{
			...
			CGICDEBUGEND
#endif /* CGICDEBUG */
				if (cgiParsePostFormInput() !=
				    cgiParseSuccess)
			{
#ifdef CGICDEBUG
				CGICDEBUGSTART
					fprintf(dout,
						"PostFormInput failed\n");
				CGICDEBUGEND
#endif /* CGICDEBUG */
					cgiFreeResources();
				return -1;
	...
	else if (cgiStrEqNc(cgiRequestMethod, "get"))
	{
		...
		{
#ifdef CGICDEBUG
			CGICDEBUGSTART fprintf(dout, "GetFormInput failed\n");
			CGICDEBUGEND
#endif /* CGICDEBUG */
				cgiFreeResources();
			return -1;
...
cgiParsePostFormInput()
{
	...
	input = (char *) real_malloc(cgiContentLength);
	if (!input)
	{
		return cgiParseMemory;
	}
	if (((int) fwebio_read(input, cgiContentLength))
	    != cgiContentLength)
	{
		return cgiParseIO;
	}
...
int
mpRead(mpStreamPtr mpp, char *buffer, int len)
{
	int ilen = len;
	int got = 0;
	while (len)
	...
	if (len)
	{
		int fgot = fwebio_read(buffer,  len);
		if (fgot >= 0)
		{
			mpp->offset += (got + fgot);
			return got + fgot;
...
void
mpPutBack(mpStreamPtr mpp, char *data, int len)
{
	mpp->offset -= len;
	while (len)
	{
...
static int readHeaderLine(mpStreamPtr mpp,
			  char *attr,
			  int attrSpace, char *value, int valueSpace);
...
static cgiParseResultType
getTempFileName(char *tfileName)
{
#ifndef WIN32
...
#define RAPPEND(string, ch) \
	{ \
		if (string##Len  == string##Space)  { \
			mp_growc(ch);\
			string##Space ++;\
			string##Space += mp_grow_room();\
		} else { \
			mp_growc_fast(ch);\
		}\
		string##Len ++;\
...
cgiParseResultType
afterNextBoundary(mpStreamPtr mpp, FILE * outf, char **outP,
		  int *bodyLengthP, int first)
{
	int outLen = 0;
	int outSpace = 256;
	...
	{
#ifdef CGIC_USE_MEMPOOL
		out = (char *)1; /* just make out to be true value */
		outSpace = mp_grow_room();
#else
...
static void
decomposeValue(char *value,
	       char *mvalue, int mvalueSpace,
	       char **argNames, char **argValues, int argValueSpace)
{
	char argName[1024];
...
static int
readHeaderLine(mpStreamPtr mpp,
	       char *attr, int attrSpace, char *value, int valueSpace)
{
	int attrLen = 0;
	int valueLen = 0;
	int valueFound = 0;
...
static cgiParseResultType
cgiParseFormInput(char *data, int length)
{
...
cgiUnescapeResultType
cgiUnescapeChars(char **sp, char *cp, int len)
{
	char *s;
...
static cgiFormResultType cgiFormEntryString(cgiFormEntry * e, char *result,
					    int max, int newlines);
...
cgiFormResultType
cgiFormFileName(char *name, char *result, int resultSpace)
{
	cgiFormEntry *e;
	int resultLen = 0;
	char *s;
...
cgiFormResultType
cgiFormFileContentType(char *name, char *result, int resultSpace)
{
	cgiFormEntry *e;
	int resultLen = 0;
	char *s;
...
cgiFormResultType 
cgiFormFilePath(char *name, char *buf, int buflen)
{	
	cgiFormEntry *e;
	int path_len ;
	e = cgiFormEntryFindFirst(name);
	...
	memcpy(buf,e->tfileName,path_len+1);
	return cgiFormSuccess;
}
...
cgiFormResultType
cgiFormFileRead(cgiFilePtr cfp, char *buffer, int bufferSize, int *gotP)
{
	int got = 0;
	if (!cfp)
	{
...
cgiFormResultType
cgiFormStringMultiple(char *name, char ***result)
{
	char **stringArray;
	cgiFormEntry *e;
	int i;
	...
	e = cgiFormEntryFindFirst(name);
#ifdef CGICDEBUG
	CGICDEBUGSTART fprintf(dout, "StringMultiple Beginning\n");
	CGICDEBUGEND
#endif /* CGICDEBUG */
		if (e)
	{
		...
		*result = stringArray;
#ifdef CGICDEBUG
		CGICDEBUGSTART fprintf(dout, "StringMultiple Succeeding\n");
		CGICDEBUGEND
#endif /* CGICDEBUG */
			return cgiFormSuccess;
	}
...
cgiFormResultType
cgiFormStringSpaceNeeded(char *name, int *result)
{
	cgiFormEntry *e;
	e = cgiFormEntryFindFirst(name);
	if (!e)
...
static cgiFormResultType
cgiFormEntryString(cgiFormEntry * e, char *result, int max, int newlines)
{
	char *dp, *sp;
	int truncated = 0;
	int len = 0;
...
cgiFormResultType
cgiFormIntegerBounded(char *name, int *result, int min, int max, int defaultV)
{
...
cgiFormResultType
cgiFormDoubleBounded(char *name, double *result, double min, double max,
		     double defaultV)
{
		    ...
		    int *result, int defaultV)
{
	...
	for (i = 0; (i < choicesTotal); i++)
	{
		...
		{
#ifdef CGICDEBUG
			CGICDEBUGSTART fprintf(dout, "MATCH\n");
			CGICDEBUGEND
#endif /* CGICDEBUG */
				* result = i;
			return cgiFormSuccess;
...
cgiFormResultType
cgiFormSelectMultiple(char *name, char **choicesText, int choicesTotal,
...
extern cgiFormResultType
cgiFormCheckboxMultiple(char *name, char **valuesText, int valuesTotal,
			int *result, int *invalid)
{
...
cgiFormResultType
cgiCookieString(char *name, char *value, int space)
{
	char *p = cgiCookie;
	while (*p)
	{
			 ...
			 char *path, char *domain)
{
	...
	if(secondsToLive!=0)
	{
		...
		{
			fwebio_printf(
				"Set-Cookie: %s=%s; expires=%s, %02d-%s-%04d %02d:%02d:%02d GMT; path=%s; domain=%s\r\n",
				name, value, 
				days[gt->tm_wday],
				gt->tm_mday,
				months[gt->tm_mon],
				gt->tm_year + 1900,
				gt->tm_hour, gt->tm_min, gt->tm_sec, path, domain);
		}
		else
		{
			fwebio_printf(
				...
				months[gt->tm_mon],
				gt->tm_year + 1900,
				gt->tm_hour, gt->tm_min, gt->tm_sec, path);
		}
	}
	else
	{
	...
	while (e)
	{
		...
		if (cgiFormFileOpen(e->attr, &fp) == cgiFormSuccess)
		{
			...
			while (cgiFormFileRead(fp, buffer,
					       ...
					       &got) == cgiFormSuccess)
			{
				if (((int) fwrite(buffer, 1, got, out)) !=
				    got)
				{
					cgiFormFileClose(fp);
					goto error;
...
static int
cgiWriteString(FILE * out, char *s)
{
...
cgiReadEnvironment(char *filename)
{
	...
	while (1)
	{
		...
		if (fileFlag)
		{
			...
			fclose(out);
			e->tfileName =
				(char *) malloc((int) strlen(tfileName) + 1);
			if (!e->tfileName)
			{
...
static int
cgiStrBeginsNc(char *s1, char *s2)
{
	while (1)
	{
		if (!(*s2))
...
static cgiFormEntry *
cgiFormEntryFindFirst(char *name)
{
	cgiFindTarget = name;
