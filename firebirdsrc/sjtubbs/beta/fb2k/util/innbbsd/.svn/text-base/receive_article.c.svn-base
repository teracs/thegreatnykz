...
char *post_article
ARG((char *, char *, char *, int (*)(), char *, char *));
...
if(badidfp){
    ...
    for (pathptr = PATH;
            pathptr != NULL
            && (pathptr = strstr(pathptr, ".edu.tw")) != NULL;)
    {
        if (pathptr != NULL)
        {
    ...
    *hispaths = '\0';
    ...
    for (ngptr = *splitptr; ngptr != NULL; ngptr = *(++splitptr))
    {
                ...
                nboardptr = (char *) strchr(boardptr, ','))
        {
            ...
            boardhome =
                (char *) fileglue("%s/boards/%s", BBSHOME, boardptr);
            if (!isdir(boardhome))
            {
            ...
            else
            {
                 ...
                 */
                fname =
                    (char *) post_article(boardhome, userid, boardptr,
                                          bbspost_write_post, NULL,
                                          firstpath);
                if (fname != NULL)
                {
                    fname = (char *) fileglue("%s/%s/%s", boardptr, getfolderstr(fname), fname);
                    if (firstpath[0] == '\0')
                    {
                        sprintf(firstpath, "%s/boards/%s", BBSHOME, fname);
                        firstpathbase =
                            firstpath + strlen(BBSHOME) +
                            strlen("/boards/");
                    }
                    if (strlen(fname) + strlen(hispaths) + 1 <
                            sizeof(hispaths))
                    {
            ...
            */
            ...
            if (isdir(boardhome))
            {
                ...
                if (*subject)
                    SUBJECT = subject;
                fname =
                    (char *) post_article(boardhome, FROM, "deleted",
                                          bbspost_write_cancel, filename,
                                          firstpath);
                if (fname != NULL)
                {
                    if (firstpath[0] == '\0')
                    {
                        sprintf(firstpath, "%s/boards/deleted/%s", BBSHOME,
                                fname);
                        firstpathbase =
                            firstpath + strlen(BBSHOME) +
                            strlen("/boards/");
                    ...
                    if (storeDB
                            (HEADER[MID_H],
                             (char *) fileglue("deleted/%s", fname)) < 0)
                    {
                        /* should do something */
                        bbslog("store DB fail\n");
                        /* return -1; */
...
char *pathname, *firstpath;
{
    ...
    while (1)
    {
        sprintf(article, "%s/%s/%s", homepath, getfolderstr(name), name);
...
char *board, *file;
{
    ...
    while (1)
    {
        ...
        read(fd, &header, size);
        if (strncmp(header.filename, file, STRLEN))
            if (ent == 0)
            {
                close(fd);
                return 0;
    ...
    if (strcmp(file, header.filename) == 0)
    {
        ...
        if (header.accessed[0] & FILE_MARKED)
        {
            close(fd);
            return 0;
        }
        for (i = ent + 1; i <= numents; i++)
        {
            ...
            if (lseek(fd, (i - 1) * size, SEEK_SET) == -1)
                break;
            if (safewrite(fd, &header, size) != size)
                break;
        }
        ftruncate(fd, (off_t) size * (numents - 1));
        flock(fd, LOCK_UN);
        close(fd);
        sprintf(buf, "%s/boards/%s/%s/%s", homepath, board, getfolderstr(file), file);
        unlink(buf);
        return 0;
    }
}
...
char *board, *file;
{
    ...
    sprintf(fname, "%s/boards/%s/%s/%s", homepath, board, getfolderstr(file), file);
    unlink(fname);
...
#else
error
("You should choose one of the systems: PhoenixBBS, PowerBBS, or PalmBBS")
#endif
#else
