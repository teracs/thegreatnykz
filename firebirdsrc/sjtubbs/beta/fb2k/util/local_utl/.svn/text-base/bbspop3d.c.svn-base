...
/*
   使用方法： ./bbspop3d 1110
   表示 bbs pop3 的启动在 1110 端口上。( 无参数则为缺省的 110 端口)
   要使用 1024 以下的端口，必须用 root 的身份来执行，并且，该端口没有被占用
*/
...
void logattempt(uid, frm)       /* Leeward 98.07.25 */
char *uid, *frm;
{
    char fname[STRLEN];
    int fd, len;
...
int Retr()
{
	...
	outs(genbuf);
	sprintf(genbuf, "Content-Type: text/plain; charset=gb2312");
	outs(genbuf);
	sprintf(genbuf, "Content-Disposition: inline");
	outs(genbuf);
