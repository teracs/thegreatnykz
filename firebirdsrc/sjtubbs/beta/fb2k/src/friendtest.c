...
{
    sprintf(s, "home/%c/%c/%s/.FRIENDTEST", toupper(id[0]), toupper(id[1]), id);
}

void sethometop(char *s, char *id)
{
    sprintf(s, "home/%c/%c/%s/"FRIENDTOP, toupper(id[0]), toupper(id[1]), id);
}

void sethomestat(char *s, char *id)
{
    sprintf(s, "home/%c/%c/%s/.FRIENDSTAT", toupper(id[0]), toupper(id[1]), id);
}
...
void new_friendtest(void)
{
    ...
    {
        empty = 0;
        getdata(1, 0, "已经存在你的账号，是否继续? [y/N] ", ans, 2, true, true);
        if (toupper(ans[0]) != 'Y')
            return;
    }
...
again:
        new_friendtest_reset();
        prints("第%d题(共%d题)\n", i + 1, questst);
        getdata(2, 0, "请输入题目(40个汉字内): ", quests[i].topic, 80, true, true);
        if (!quests[i].topic[0])
        {
            new_friendtest_reset();
            getdata(1, 0, "退出建立账号? [Y-退出/N-继续/E-不再输入新题]: ", ans, 2, true, true);
            if (toupper(ans[0]) == 'Y')
                return;
    ...
    getdata(2, 0, "以上输入是否正确，选择否取消，选择是建立[Y/n]", ans, 2, true, true);
    if (toupper(ans[0]) == 'N')
        return;
    save_quests(direct);
    load_users(FRIENDTOP);
...
void do_test_reset(void)
{
    ...
    setfcolor(YELLOW, 1);
    setbcolor(BLUE);
    prints("   FRIENDTEST      友谊竞赛 %s <--> %s                                           ", currentuser.userid, lookupuser.userid);
    resetcolor();
    prints("\n");
}
...
void admin_stat(void)
{
    ...
    while (1)
    {
        ...
        getdata(7, 0, "选择[q-退出,d-删除,e-修改,a-添加,r-排行重置,s-察看,l-列表] ", ans, 2, true, true);
...
againa:
            ...
            {
                new_friendtest_reset();
                getdata(1, 0, "取消? [Y-退出/N-继续]: ", ans, 2, true, true);
                if (toupper(ans[0]) == 'Y')
                    break;
                goto againa;
            }
...
againb:
            new_friendtest_reset();
            prints("第%d题(共%d题)\n", i + 1, questst);
            getdata(2, 0, "请输入题目(40个汉字内): ", quests[i].topic, 80, true, false);
            if (!quests[i].topic[0])
            {
                new_friendtest_reset();
                getdata(1, 0, "取消? [Y-退出/N-继续]: ", ans, 2, true, true);
            ...
            do
            {
                ...
                clrtoeol();
                sprintf(buf, "%d", k);
                getdata(3, 0, "请输入选项个数[1--5]: ", buf, 2, true, false);
                j = atoi(buf);
            }
            while (j < 1 || j > 5);
            if (j < 5)
            ...
            break;
        case 'R':
            getdata(8, 0, "一旦重置之后，排行榜将被抹去，确认吗(y/N)", ans, 2, true, true);
            if (toupper(ans[0]) != 'Y')
                break;
            load_users(FRIENDTOP);
            j = 1;
...
int admin_menu(void)
{
    ...
    getdata(3, 0, "选择(0-删题,1-禁止开题,2-允许开题,3-禁止做题,4-允许做题,5-列表,其他-离开) ", ans, 2, true, true);
    if (ans[0] >= '0' && ans[0] <= '4')
    {
