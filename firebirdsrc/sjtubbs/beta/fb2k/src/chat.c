...
*/
/*
$Id: chat.c,v 1.3 2004/07/02 23:10:21 rwzmm Exp $
*/
...
chat_load_alias (void)
{
    ...
    chat_alias_count = 0;
    chat_aliases =
        (struct chatalias *) malloc (sizeof (struct chatalias) * MAXDEFINEALIAS);
    ...
    while (c > 0)
    {
        ...
        if (*bptr == '/')
        {
            ...
            case 't':
                ...
                sprintf (genbuf,
                         "[1;44;33m ·¿¼ä£º [32m%-12.12s  [5;31;44m%6.6s[0;1;44;33m»°Ìâ£º[36m%-49.49s[m",
...
ent_chat (char *chatbuf)
{
    ...
    while (1)
    {
        getdata (2, 0, "ÇëÊäÈëÁÄÌì´úºÅ(ÍË³ö: *)£º", inbuf, CHAT_IDLEN,
                 DOECHO, YEA);
        if (inbuf[0] == '*')
            return 0;
        sprintf (chatid, "%.8s",
...
printuserent (struct user_info *uentp)
{
    ...
    if (!uentp->active || !uentp->pid)
        return 0;
    //yukuan comment to deny any chatcloak
    /*
    	if ((uentp->invisible && !(HAS_PERM(PERM_SYSOP) || HAS_PERM(PERM_SEECLOAK) || usernum==uentp->uid)) || isreject(uentp))
    		return 0;
    */
#if 0
...
zap_user (char *arg)
{
    ...
    if (arg[i] == 0)
    {
        ...
        {
            printchatline ("¡ï[1;32mÄãÃ»ÓÐÉèÖÃºöÂÔÁÐ±í[m¡ï");
            return;
        }
        printchatline ("¡ï[1;32m¼º¾­ºöÂÔµÄÓÃ»§ÁÐ±í[m¡ï");
    ...
    if (seek_in_file (chatbuf, arg))
    {
        ...
        printchatline ("¡ï[1;32m¼º¾­½«¸ÃÓÃ»§´ÓºöÂÔÁÐ±íÖÐÉ¾³ý[m¡ï");
        return;
    }
    else
    {
        add_to_file (chatbuf, arg);
        printchatline ("¡ï[1;32m¼º¾­½«¸ÃÓÃ»§¼ÓÈëµ½ºöÂÔÁÐ±íÖÐ[m¡ï");
        return;
    }
...
query_user (char *arg)
{
    ...
    sprintf (qry_mail_dir, "mail/%c/%c/%s/%s",
             toupper (lookupuser.userid[0]), toupper (lookupuser.userid[1]), lookupuser.userid, DOT_DIR);
    sprintf (msg, "[1;37m%s[m ([1;33m%s[m) ¹²ÉÏÕ¾ [1;32m%d[m ´Î, ·¢±í"
             "¹ý [1;32m%d[m ÆªÎÄÕÂ", lookupuser.userid,
    ...
    {
        sprintf (msg, "%sÉÏ´ÎÉÏÕ¾Ê±¼ä£º [[1;32m%s[m] ",
    ...
    {
        sprintf (msg,
                 "%sÉÏ´ÎÔÚ [[1;32m%s[m] ÓÉ [[1;32m%s[m] µ½±¾Õ¾Ò»ÓÎ",
                 (HAS_DEFINE (lookupuser.userdefine, DEF_S_HOROSCOPE)) ? buf
...
call_user (char *arg)
{
    ...
    if (good_id == YEA && canmsg (uin))
    {
        ...
        sprintf (msg, "[1;32m%s[37m %s[m", userid,
                 uin && !uin->invisible ? "ÎÞ·¨ºô½Ð" : "²¢Ã»ÓÐÉÏÕ¾");
    printchatline (msg);
}
...
chat_date (void)
{
    ...
    time (&thetime);
    sprintf (genbuf, "[1m %s±ê×¼Ê±¼ä: [32m%s[m", BoardName,
             Cdate (&thetime));
                        ...
                        
{
    ...
    if (!uentp->active || !uentp->pid)
        return 0;
    //yukuan comment to deny cloak

    if (!(HAS_PERM(PERM_SEECLOAK)||HAS_PERM(PERM_SYSOP)||usernum==uentp->uid) && uentp->invisible)
        return 0;
...
set_rec (void)
{
    ...
    if (recflag == 0)
    {
        ...
        sprintf (genbuf,
                 "[1;44;33m ·¿¼ä£º [32m%-12.12s  [5;31;44m%6.6s[0;1;44;33m»°Ìâ£º[36m%-49.49s[m",
                 chatname, (recflag == 1) ? "[Â¼Òô]" : "      ", tmptopic);
        prints ("%s", genbuf);
