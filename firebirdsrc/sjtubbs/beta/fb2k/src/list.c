...
static int
friend_search (int uid, struct user_info *uentp, int tblsize)
{
    int hi, low, mid;
    int cmp;
...
// Add by Flier - 2000.5.12 - Begin
int
compare_user_record (struct user_info *left, struct user_info *right)
{
    int retCode;
...
int
deal_key (int ch, int allnum, int pagenum)	//ª∑πÀÀƒ∑Ω¥¶¿Ì∞¥º¸
{
...
*/
    ...
    case 'R':
        ...
        {
            sprintf (buf, "%s “—‘⁄%s√˚µ•", user_record[allnum]->userid, desc);
        }
        else
        {
            sprintf (buf, "%s ¡–»Î%s√˚µ•", user_record[allnum]->userid, desc);
        }
        msgflag = YEA;
        break;
    case 'd':
...
int
deal_key2 (int ch, int allnum, int pagenum)	//ÃΩ ”Õ¯”—¥¶¿Ì∞¥º¸
{
    char buf[STRLEN];
    static int msgflag;
...
printuent (void *vuentp)
{
    ...
    prints (" %5d%2s%s%-12s%s %-17s %6d %4d %10s %-16s\n", i + 1,
            ...
            (toggle2 == 0) ? (unsigned int)(uentp->stay / 3600) : uentp->nummails,
...
int do_query (int star, int curr)
{
    if (user_record[curr]->userid != NULL)
    {
        ...
        move (t_lines - 1, 0);
        prints
        ("[0;1;37;44m¡ƒÃÏ[[1;32mt[37m] ºƒ–≈[[1;32mm[37m] ÀÕ—∂œ¢[[1;32ms[37m] º”,ºı≈Û”—[[1;32mo[37m,[1;32md[37m] —°‘Ò π”√’ﬂ[[1;32m°¸[37m,[1;32m°˝[37m] «–ªªƒ£ Ω [[1;32mf[37m] «Ûæ»[[1;32mh[37m][m");
    }
	return 0;
}
...
int do_query2 (int star, int curr)
{
    if (user_data[curr - star].userid != NULL)
    {
        ...
        move (t_lines - 1, 0);
        prints
        ("[0;1;37;44m          ºƒ–≈[[1;32mm[37m] º”,ºı≈Û”—[[1;32mo[37m,[1;32md[37m] ø¥Àµ√˜µµ[[1;32m°˙[37m,[1;32mRtn[37m] —°‘Ò[[1;32m°¸[37m,[1;32m°˝[37m] «Ûæ»[[1;32mh[37m]          [m");
    }
	return 0;
}
...
int
choose (int update, int defaultn, int (*title_show) (void), int (*key_deal) (int,int,int), int (*list_show) (void), int (*read) (int,int))
{
    int ch, number, deal;
