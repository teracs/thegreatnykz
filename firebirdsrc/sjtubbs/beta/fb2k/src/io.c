...
#include "g2b_table.h" /* for GtoB and BtoG table */
...
int multi_getdata(int line, int col, int maxcol, char *prompt, char *buf, int len, int maxline,
    int clearlabel)
{
    int ch, x, y, startx, starty, now, i, j, k, i0, chk, cursorx=0, cursory=0;
...
#ifdef SPEED_LIMIT
/* check if this process uses too many cpu resource */

#define SL_CHECK_POINT (96*1024)    /* check once while writing 96k bytes */
#define SL_SPEED_LIMIT (2048)   /* avg speed limit is 1.5k bytes/seconds */
/* if exceeding avg speed for more than SL_COUNTER_MAX seconds, client will be disconnected*/
#define SL_COUNTER_MAX (120)
/* the bigger the longer sleep time as punishment. default is 20, it should be fine*/
...
static void speed_limit(int written_bytes)
{
    ...
    static char warn[] = "detect robot. if any problem, report to board SYSOP";
    ...
    if (r_bcounter > SL_CHECK_POINT)
    {
        time_t now;
        now = time(NULL);
        r_bcounter -= SL_SPEED_LIMIT * (now - prev_chktime) + sl_read_counter * 4;
        sl_read_counter = 0;
        prev_chktime = now;
        if (r_bcounter < 1)
        {
        ...
        {
            int stime = r_bcounter * SL_SLEEP_FACTOR / SL_SPEED_LIMIT * written_bytes;
            stime += counter * 50000;
            if (stime > 100000)
            {
                ++counter;
...
#else
/* NULL function, will be deleted after compiler optimization */
