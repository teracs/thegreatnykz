...
void
draw_entry (char *(*doentry) (int,void*), struct keeploc *locmem, int num, int ssize)
{
    char *str;
    int base, i;
...
void
i_read (int cmdmode, char *direct, void (*dotitle) (void), char *(*doentry) (int,void*), struct one_key *rcmdlist, int ssize)
{
    ...
    if (last_line == 0)
    {
        ...
        else if (cmdmode == GMENU)
        {
            ...
            if (genbuf[0] == 'a' || genbuf[0] == 'A')
                (friendflag) ? friend_add (0,NULL,NULL) : reject_add (0,NULL,NULL);
        }
        else
        {
	...
	/*added by kongyang 05-05-31*/
	    if(ch=='!')
		{
		 ...
		 else mode = i_read_key (rcmdlist, locmem, ch, ssize);
		 /*added end*/
...
int
i_read_key (struct one_key *rcmdlist, struct keeploc *locmem, int ch, int ssize)
{
	...
	move(t_lines - 1, 0);
	tch[0] = '\0';
	getdata(t_lines - 1, 0, "查看分区十大:(1~b)区 0)取消 [0]:", tch, 3, DOECHO, YEA);
    ...
    case KEY_END:
		...
		if(digestmode == 9)/*here is recycle bin*/
			last_line=i+get_return_dir_num(currdirect, sizeof(struct fileheader));//++
		else 
...
SR_BMfunc (int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    while (1)
    {
        ...
        case 3:
            ...
            break;
        case 4:
			SR_fptr.accessed[0] |= FILE_VISIT;  /* 将标志置位 */
			substitute_record(direct, &SR_fptr, sizeof(SR_fptr), locmem->crs_line);
...
int
search_author (struct keeploc *locmem, int offset, char *powner)
{
...
int
auth_post_down (int ent, struct fileheader *fileinfo, char *direct)
{
    struct keeploc *locmem;
...
int
auth_post_up (int ent, struct fileheader *fileinfo, char *direct)
{
    struct keeploc *locmem;
...
int
t_search_down (int ent, struct fileheader *fileinfo, char *direct)
{
    struct keeploc *locmem;
...
int
t_search_up (int ent, struct fileheader *fileinfo, char *direct)
{
    struct keeploc *locmem;
...
int
thread_up (int ent, struct fileheader *fileinfo, char *direct)
{
    struct keeploc *locmem;
...
int
thread_down (int ent, struct fileheader *fileinfo, char *direct)
{
    struct keeploc *locmem;
...
int
search_post (struct keeploc *locmem, int offset)
{
...
int
search_title (struct keeploc *locmem, int offset)
{
...
int
search_thread (struct keeploc *locmem, int offset, char *title, unsigned int reid)
{
...
sread (int readfirst, int auser, struct fileheader *ptitle)
{
    ...
    while (!isend)
    {
        ...
        case 'Y':
#if 0
/* FIXME!!!! what the hell do_reply_push to do??? by jawahh */
        case 'R':
		{
			struct shortfile *bp;
...
int
search_articles (struct keeploc *locmem, char *query, unsigned int reid, int offset, int aflag, int newflag)
{
    char *ptr;
	char *temp_ptr = NULL;
...
/* modified by rwzmm @ sjtubbs */
			if((uinfo.mode == RMAIL) && (search_auther_in_mail == 1))/* reading mail */
			{
				temp_ptr = strstr(ptr, " (");
				if(temp_ptr != NULL)
				{
					length_ptr = temp_ptr - ptr;
					memset(upper_ptr+length_ptr, 0x00, STRLEN - length_ptr);
				}
			}
/* modified end */
...
int
locate_the_post (struct fileheader *fileinfo, char *query, int offset, int aflag, int newflag)
{
    struct keeploc *locmem;
