...
domailclean (void *vfhdrp)
{
    ...
    if (fhdrp == NULL)
    {
        ...
        if (delcnt)
        {
            sprintf (buf, "mail/%c/%c/%s/%s", toupper (curruser[0]), toupper (curruser[1]), curruser,
                     DOT_DIR);
            while (delcnt--)
...
cleanmail (void *vurec)
{
    ...
    if (urec->userid[0] == '\0' || !strcmp (urec->userid, "new"))
        return 0;
    sprintf (genbuf, "mail/%c/%c/%s/%s", toupper (urec->userid[0]), toupper (urec->userid[1]),
             urec->userid, DOT_DIR);
...
void
trace_state (int flag, char *name, int size)
{
    char buf[STRLEN];
