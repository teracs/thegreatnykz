...
*/
/*
$Id: vote.c,v 1.3 2004/07/08 14:06:24 bbs Exp $
*/
...
int HiddenBoard(char* name)
{
	...
	if(i==MAXBOARD)
		return 0;/* not hidden, perhaps this statment never happens?*/
	bptr = &bcache[i];
...
int getVoterIP(char* file, struct voter_info v_info[MAXUSERS])
{
	...
	if ((fd = open(file, O_RDONLY, 0)) == -1)
		return 0;
	while (read( fd, &v_info[nums], sizeof(v_info[nums])) == sizeof(v_info[nums]))
	{
		nums++;
	}
	return nums;
...
/* add by Ellias, 07.17.2007*/
int writeVoterIP(char* file, char* board, struct voter_info v_info[MAXUSERS], int nums)
{
	FILE *fd;
	int i;
...
mk_result (int num)
{
    ...
    char sugname[STRLEN];
    char title[STRLEN];
	char title2[STRLEN];	// add by Ellias
	char ipname[STRLEN];	// add by Ellias
	struct voter_info vinfo[MAXUSERS];	// add by Ellias
	int votenums;	// add by Ellias
	char ipname2[STRLEN];	// add by Ellias
    int i;
	...
	/* modified by rwzmm@sjtubbs & Ellias*/
	...
	if(strncmp(currboard, "vote", 4))//not vote board
	{
...
/* modified by Ellias, 07.16.2007*/
int
user_vote (int page, int num)
{
	...
	sprintf (uipname, "vote/%s/uip.%ld", currboard, currvote.opendate);	// Éè¶¨ipĞÅÏ¢ÎÄ¼ş
    ...
    else if (currvote.level & VOTEMASK)
    {
        if (currentuser.numlogins < currvote.x_logins
                ...
                || (unsigned int)currentuser.firstlogin > (currvote.opendate - currvote.x_live * 86400))
	 ...
	 else
	 {
	 	if (search_ip(ipfname,currentuser.lasthost)>=MAXVOTEPERIP)
	 	{
		...
		else 
       	{
			...
			{
				if (append_record (uipname, &v_info, sizeof(v_info)) == -1)
				{
					move (2, 0);
					clrtoeol ();
					prints ("Í¶Æ±Ê§°Ü! ÇëÍ¨ÖªÕ¾³¤²Î¼ÓÄÇÒ»¸öÑ¡ÏîÍ¶Æ±\n");
...
dele_vote (int num)
{
    ...
    unlink (buf);
//    sprintf (buf, "vote/%s/uip3.%d", currboard, currvote.opendate);
//    unlink (buf);
...
#define maxrecp 300
int
ListFileEdit (char *fname, char *msg, int response)//if response=1, mail info to user
{
    char listfile[STRLEN], uident[13], ans[3];
    int cnt, i, n, fmode = NA;

	//add by zsong, 2006-03-05
    ...
    while (1)
    {
        ...
        case 'a':
            ...
            cnt++;
			//add by zsong
			if(response) //for add board.allow only
			{
        ...
        case 'd':
            if (seek_in_file (listfile, uident))
            {
				...
				// ÕâÀïÅĞ¶ÏÒ»ÏÂÕâ¸öÓÃ»§ÊÇ·ñ´æÔÚ£¬·ÀÖ¹ÒòÎªÕÊºÅËÀÍöÔì³ÉÉ¾³ıµÄÊ±ºòµôÏß
				// by smartfish @ liumang bbs
				{
					memset(mail_buffer, 0x00, 1024);
					memset(mail_title, 0x00, STRLEN);
					sprintf (mail_title, 
...
vote_key (int ch, int allnum, int pagenum)
{
    ...
    case Ctrl ('E'):
        ...
        ListFileEdit (genbuf, "±à¼­¡ºÍ¶Æ±Ãûµ¥¡»", 0);
        break;
    case 'E':
        if (!chk_currBM (currBM))
            return YEA;
...
b_vote (void)
{
    ...
    if (!HAS_PERM (PERM_VOTE) || ((currentuser.stay < 1800)&&(currentuser.numlogins <10)))
        return DONOTHING; 
    setcontrolfile ();
...
int
check_global_vote_status(char* bname)
//vote: return 1
//no vote: return 0
{
//	struct boardheader fh;
...
//	for(i = 0; i < num_of_vote; i++)
	...
	while((fread(&vote_rec, sizeof(struct votebal), 1, fp_control)) > 0)
	{
		...
		if(fp_flag == NULL)
			return 1;
		while((fread(&vote_ballot, sizeof(struct ballot), 1, fp_flag)) >0)
		{
			callforvoted = 1;
		...
		if(callforvoted == 1)
			return 1;
//		memset(&vote_rec, 0x00, sizeof(struct votebal));
	}
	fclose(fp_control);
	fp_control = NULL;
	return 0;
...
// Add by clist: Juhui Regist Admin
struct registdetail
{
	char id[IDLEN + 1];
...
m_EditJuhuiRegLog(void)
{
    ...
    aborted = vedit ("vote/regist_log", NA, YEA);
	clear();

	if (aborted != -1)
    {
        prints ("¾Û»á±¨ÃûÈÕÖ¾¸üĞÂ");
        sprintf (secu, "ĞŞ¸Ä¾Û»á±¨ÃûÈÕÖ¾");
        securityreport (secu);
	}
...
m_EditJuhuiRegNote(void)
{
	...
	clear();
    modify_user_mode (EDITSFILE);
    aborted = vedit ("vote/regist_note", NA, YEA);
	clear();

	if (aborted != -1)
    {
        prints ("¾Û»á±¨ÃûÌáÊ¾¸üĞÂ");
        sprintf (secu, "ĞŞ¸Ä¾Û»á±¨ÃûÌáÊ¾");
        securityreport (secu);
	}
...
m_RegJuhuiMannually(void)
{
	...
	while (num < 0)
	{
		getdata(1, 0, "ÊäÈëÓûÌí¼Ó±¨Ãû»òÉ¾³ı±¨ÃûµÄºÅÂë: ", buf, 5, DOECHO, YEA);
		if (!isdigit(buf[0]))
			continue;
		num = atoi(buf);
	}

	if (askyn("Ìí¼Ó±¨Ãû£¿£¨Ñ¡ÔñN¿ÉÉ¾³ı¸ÃºÅÂë£©", YEA, YEA) == YEA)
	{
		move(2, 0);
		usercomplete("ÊäÈë±¨ÃûµÄÊ¹ÓÃÕßÕÊºÅ: ", genbuf);
		...
		strncpy(id, genbuf, IDLEN + 1);

		sprintf(rm_file_path, "home/%c/%c/%s/real_email", toupper(id[0]), toupper(id[1]), id);

		if ((stream=fopen(rm_file_path,"r"))==NULL)
		...
		while (bus<0 || bus>3)
		{
			getdata (3, 0, "ÊÇ·ñ³Ë×øĞ£³µ(0:·ñ;1:À´;2:»Ø;3:½Ô): ", ans, 3, DOECHO, YEA);
			bus = atoi(ans);
		}

		getuser(id);
		gender = (lookupuser.gender == 'M')? 1 : 2;
	...
	{
		if (askyn("É¾³ı¸ÃºÅÂë£¿", YEA, YEA) == YEA)
		{
			detail.num = 0;
			sprintf(detail.id, "%s", "\0");
			sprintf(detail.email, "%s", "\0");
			detail.bus = 0;
			detail.gender = 0;
