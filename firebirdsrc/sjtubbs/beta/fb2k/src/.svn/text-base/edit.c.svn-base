...
*/
/*
$Id: edit.c,v 1.3 2004/07/15 10:18:49 bbs Exp $
*/
...
searchline (char *text)
{
    ...
    for (; p && p != can_edit_end; p = p->next)
    {
        ...
        {
            addr =
                (int) (strstr (p->data + tt, text) - p->data) + strlen (text);
            currpnt = addr;
            break;
        }
    }
...
insert_char (register int ch)
{
    ...
    p = p->next;
    if (wordwrap && p->len >= 1)
    {				//Èç¹ûÊÇ´Ê£¬ÇÒÆäºóÃ»ÓÐ¿Õ¸ñ£¬Ôò×Ô¶¯²¹ÉÏ¿Õ¸ñ
        i = p->len;
        if (p->data[i - 1] != ' ')
        {
            p->data[i] = ' ';
...
write_header (FILE * fp, int mode)
{
    ...
    //   if(!(bp->flag & OUT_FLAG)) local_article = YEA;
    if (in_mail)
    ...
    else
    {
        ...
        fprintf (fp, "·¢ÐÅÈË: %s (%s), ÐÅÇø: %s\n",
                 (header.chk_anony) ? currboard : uid,
                 (header.chk_anony) ? "ÎÒÊÇÄäÃûÌìÊ¹" : uname, currboard);
    }
    fprintf (fp, "±ê  Ìâ: %s\n", save_title);
    getdatestring (now, NA);
...
char *format_pushbuf(char *pushbuf, char *bufformatted)
{
	...
	*(tempbuf+j)='\0';
	...
	snprintf(buf,256,"[1;32mÍÆ [m[1;33m%-12.12s:[m[33m%-40s[m [37m%15s[m [37m%2d/%2d[m",currentuser.userid,strtok(tempbuf,"\n"),
		currentuser.lasthost,now->tm_mon+1,now->tm_mday);
	strncpy(bufformatted+j,buf,strlen(buf));
	j+=strlen(buf);
	while((p=strtok(NULL,"\n")))
	{
		snprintf(buf,256,"\n[32m¡ú [m[1;33m%-12.12s:[m[33m%-40s[m [37m%15s[m [37m%2d/%2d[m",currentuser.userid,p,
		currentuser.lasthost,now->tm_mon+1,now->tm_mday);
		strncpy(bufformatted+j,buf,strlen(buf));
		j+=strlen(buf);
	}
...
int
valid_article (char *pmt, char *abort, int sure)
{
    ...
    if (uinfo.mode == POSTING || uinfo.mode == EDIT)
    {
        ...
        {
            strcpy (pmt,
                    "E.ÔÙ±à¼­, S.×ªÐÅ, L.±¾Õ¾·¢±í, A.È¡Ïû ,P.Í¬Ê±·¢²¼µ½Blog or T.¸ü¸Ä±êÌâ?[L]: ");
        }
    }
	if (sure)
    {
...
#ifdef LOGHANDLER
int
log2db(char *user_id, char *ip_address,char *filename,char *board_name,unsigned int reid,char *content,int blog_tag)
{
	char *query_string, *content_ok, *content_ok2, title_ok[256], table_name[16];
...
/**
 * content_ok size: (content_len+1)*2 
 * content_ok2 size: (content_len+1)*5
 * query_string size: (content_len+1)*10+1024
 **/
	...
	query_len=snprintf(query_string, (content_len+1)*10+1024 ,LOGHANDLER_SQL"insert into %s (user_id,ip_addr,time,filename,board,title,content,reid) values ('%s','%s',FROM_UNIXTIME(%ld),'%s','%s','%s','%s','%u')\n",table_name,user_id,ip_address,timestamp,filename,board_name,title_ok,content_ok,reid);
	loghander_write(query_string,query_len);
	if(blog_tag == 1) 
	{
		filter_html(content_ok, content_ok2);
		query_len = snprintf(query_string, (content_len+1)*10+1024, LOGHANDLER_SQL"INSERT INTO blog.yssy_articles( user_id,blog_id,status,date,properties, slug ) VALUES ( %s,%s,'1',FROM_UNIXTIME(%ld),'a:1:{s:16:\"comments_enabled\";s:1:\"1\";}','%s')\n",blog_user_id,blog_id,timestamp,title_ok);
		query_len += snprintf(&query_string[query_len], (content_len+1)*10+1024-query_len, LOGHANDLER_SQL"INSERT INTO blog.yssy_article_categories_link (article_id, category_id) VALUES (LAST_INSERT_ID(), %s )\n",blog_catId);
		query_len += snprintf(&query_string[query_len], (content_len+1)*10+1024-query_len, LOGHANDLER_SQL"INSERT INTO blog.yssy_articles_text (article_id, topic, text, normalized_text, normalized_topic) VALUES( LAST_INSERT_ID(), '%s','<pre>%s</pre>','%s', '%s')\n", title_ok,content_ok2,content_ok2,title_ok);
		loghander_write(query_string,query_len);
	}
	free(content_ok);
	return 0;
...
#endif
int
write_file (char *filename, int write_header_to_file, int addfrom, int sure,unsigned int reid)
{
    struct textline *p;
    FILE *fp = NULL;
...
/*
    ...
    if (uinfo.mode == POSTING)
    {
        if (local_article == YEA && write_module == 0) {
            ...
            strcpy (p_buf,
                    "L.±¾Õ¾·¢±í, S.×ªÐÅ, A.È¡Ïû, T.¸ü¸Ä±êÌâ ,P.Í¬Ê±·¢²¼µ½blog or E.ÔÙ±à¼­? [L]: ");
			else 
				strcpy (p_buf,
                    "L.±¾Õ¾·¢±í, S.×ªÐÅ, A.È¡Ïû, T.¸ü¸Ä±êÌâ or E.ÔÙ±à¼­? [L]: ");
        	}
	else if(local_article == YEA && write_module == 1)
	        strcpy (p_buf, "(L)´¢´æµµ°¸, (A)·ÅÆú±à¼­, (E)¼ÌÐø±à¼­? [L]: ");
	else
            strcpy (p_buf,
                    "S.×ªÐÅ, L.±¾Õ¾·¢±í, A.È¡Ïû, T.¸ü¸Ä±êÌâ or E.ÔÙ±à¼­? [S]: ");
    }
...
/*        }*/
    ...
    else if ((abort[0] == 't' || abort[0] == 'T') && uinfo.mode == POSTING && write_module == 0)
    {
        char buf[STRLEN];
    ...
    else if ((abort[0] == 's' || abort[0] == 'S') && (uinfo.mode == POSTING)&& write_module == 0)
    {
        local_article = NA;
    }
    ...
    while (p != NULL)
    {
        ...
        if (aborted != -1)
        {
            if (uinfo.mode != EDIT || p != can_edit_end->prev ||
                    (ADD_EDITMARK && strncmp (p->data, "[1;36m¡ù ÐÞ¸ÄÄÚÈÝ:¡¤", 17)))
            {
...
/*            if (p == can_edit_end->prev && uinfo.mode == EDIT && ADD_EDITMARK)
            {
                fprintf (fp,
                         "[1;36m¡ù ÐÞ¸ÄÄÚÈÝ:¡¤%s ÓÚ %16.16s ÐÞ¸Ä±¾ÎÄ¡¤[FROM: %-.20s][m\n",
...
/*        fseek(fp, -30, 2);
        ...
        fseek(fp, -33-strlen(BBSHOST)-fromhostlen, 2);*/
        fprintf (fp,
                   "\n[1;36m¡ù ÐÞ¸ÄÄÚÈÝ:¡¤%s ÓÚ %16.16s ÐÞ¸Ä±¾ÎÄ¡¤[FROM: %-.46s][m\n",
                   currentuser.userid, datestring + 6,
                   currentuser.lasthost);
/*        color = (rand() % 7) + 31;
        fprintf (fp, "\n[m[1;%2dm%s[m\n",
...
///////////////////////////////////////////////////add end by proto@sjtubbs
	...
	if (aborted != -1) 
	{
		...
		if(buf)
		{
			...
			if(uinfo.mode == POSTING)
			{
				if((abort[0] == 'p' || abort[0] == 'P') &&  HAS_PERM(PERM_BOARDS) ) 
				{
					log2db( currentuser.userid,  currentuser.lasthost,(strrchr(filename,'/')+1),currboard,reid,buf,1);
				} 
				else 
				{
					log2db( currentuser.userid,  currentuser.lasthost,(strrchr(filename,'/')+1),currboard,reid,buf,0);
				}
			}
			free(buf);
		}
...
#define FROM_WHICH_PAGE "¶ÁÈ¡¼ôÌù²¾µÚ¼¸Ò³? (0-9,a-f) [Ô¤ÉèÎª 0]"
#define SAVE_ALL_TO     "°ÑÕûÆªÎÄÕÂÐ´Èë¼ôÌù²¾µÚ¼¸Ò³? (0-9,a-f) [Ô¤ÉèÎª 0]"
#define SAVE_PART_TO    "°ÑÇø¿éÄÚÈÝÐ´Èë¼ôÌù²¾µÚ¼¸Ò³? (0-9,a-f) [Ô¤ÉèÎª 0]"
...
/* valid arg are    '0' - '7' */
{
    ...
    case 'I':
        sprintf (filename, "home/%c/%c/%s/clip_%c",
                 toupper (currentuser.userid[0]), toupper (currentuser.userid[1]), currentuser.userid, arg);
        if ((fp = fopen (filename, "r")) != NULL)
        {
            insert_from_fp (fp);
            fclose (fp);
    ...
    if (msg[0] != '\0')
    {
        ...
        {			/* need redraw */
            move (t_lines - 2, 0);
            clrtoeol ();
            prints ("[1m%s%s%s[m", msg, ", Çë°´ÈÎÒâ¼ü·µ»Ø±à¼­»­Ãæ...",
                    ANSI_RESET);
            igetkey ();
            newch = '\0';
            editansi = showansi = 0;
            ...
            if (currpnt > 0)
    {
        ...
        case Ctrl ('P'):
                            ...
                            if (currline != can_edit_begin)
                    {
                        ...
                        currline = currline->prev;
                        currpnt =
                            (currline->len > lastindent) ? lastindent : currline->len;
                    }
            break;
        case Ctrl ('N'):
...
int
raw_vedit (char *filename, int write_header_to_file, int modifyheader,unsigned int reid)
{
    ...
    while (ch != EOF)
    {
        ...
        case Ctrl ('W'):
                        ...
                        foo = write_file (filename, write_header_to_file, YEA, sure,reid);
            if (foo != KEEP_EDITING)
                return foo;
            redraw_everything = YEA;
            break;
...
int
vedit (char *filename, int write_header_to_file, int modifyheader)
{
    int ans, t = showansi;
...
int vedit_reid (char *filename, int write_header_to_file, int modifyheader,unsigned int reid)
{
    int ans, t = showansi;
