...
m_init (void)
{
    sprintf (currmaildir, "mail/%c/%c/%s/%s", toupper (currentuser.userid[0]), toupper (currentuser.userid[1]), 
...
do_send (char *userid, char *title)
{
    ...
    if (!(lookupuser.userlevel & PERM_MAILINFINITY)
        ...
        return -4;
    sprintf (filepath, "mail/%c/%c/%s", toupper (userid[0]), toupper (userid[1]), userid);
    ...
    sprintf (filepath, "mail/%c/%c/%s/%s", toupper (userid[0]), toupper (userid[1]), userid, fname);
    ip = strrchr (fname, 'A');
    count = 0;
...
edit_mail_file:
    ...
    else
    {
        ...
        sprintf (genbuf, "mail/%c/%c/%s/%s", toupper (userid[0]), toupper(userid[1]), userid,
                 newmessage.filename);
        if (stat (genbuf, &st) == -1)
            file_size = 0;
        else
        ...
        *sizeptr = file_size;
        ...
        sprintf (genbuf, "mail/%c/%c/%s/%s", toupper (userid[0]), toupper(userid[1]), userid, DOT_DIR);
...
int
autoreplymail_file (char *userid, char *title)
{
    struct fileheader newmessage;
    struct stat st;
    ...
    memset (&newmessage, 0, sizeof (newmessage));
    getuser (userid);
    sprintf (tmpfile, "home/%c/%c/%s", toupper (lookupuser.userid[0]), toupper (lookupuser.userid[1]),
             lookupuser.userid);
    ...
    strncpy (save_title, newmessage.title, STRLEN);
    sprintf (filepath, "mail/%c/%c/%s", toupper (currentuser.userid[0]), toupper (currentuser.userid[1]),
             currentuser.userid);
    if (stat (filepath, &st) == -1)
    {
        if (mkdir (filepath, 0755) == -1)
    ...
    sprintf (fname, "M.%ld.A", time (NULL));
    sprintf (filepath, "mail/%c/%c/%s/%s", toupper (currentuser.userid[0]), toupper (currentuser.userid[1]),
...
m_send (char *userid)
{
	...
	//add by smartfish, fix bug 0000006
...
mailtitle()
{
	...
	prints("Àë¿ª[[1;32m¡û[m,[1;32me[m]  Ñ¡Ôñ[[1;32m¡ü[m,[1;32m¡ý[m]  ÔÄ¶ÁÐÅ¼þ[[1;32m¡ú[m,[1;32mRtn[m]  »ØÐÅ[[1;32mR[m]  ¿³ÐÅ£¯Çå³ý¾ÉÐÅ[[1;32md[m,[1;32mD[m]  ÇóÖú[[1;32mh[m][m\n");
    ...
    if (fd != -1)
    {
        while (read (fd, &fcache, ssize) == ssize)
        {
            sprintf (mailfile, "mail/%c/%c/%s/%s",
                     toupper (userid[0]), toupper (userid[1]), userid, fcache.filename);
...
mail_read (int ent, struct fileheader *fileinfo, char *direct)
{
	...
	//add by zsoong
	if (strlen(fileinfo->filename) <= 1)//ÎÄ¼þÃû²»¿ÉÄÜÐ¡ÓÚ1µÄ£¬¶¼ÊÇM.1234567890.A¸ñÊ½
		return DONOTHING;
	//add end
    ...
    if (delete_it)
        return mail_del (ent, fileinfo, direct);	/* ÐÞ¸ÄÐÅ¼þÖ®bug
    						 * ¼ÓÁËreturn */
    else
    {
...
mail_reply (int ent, struct fileheader *fileinfo, char *direct)
{
    ...
    sprintf (quote_file, "mail/%c/%c/%s/%s", toupper (currentuser.userid[0]), toupper (currentuser.userid[1]),
...
//added by proto@sjtubbs 2006.08.29
...
struct one_key mail_comms[] =
    {
        ...
        {'R', mail_reply,},
        {'m', mail_mark,},
//      {'i', full_Save_post,},//masked off by chaser 2006.05.22
//      {'J', part_Save_post,},
//      {'I', Import_post,},
        {'T', edit_title,},
        //      {'x', into_myAnnounce,},
...
//      {Ctrl ('N'), SR_first_new,},
        ...
        {'l', show_allmsgs,},
        {Ctrl ('C'), do_cross,},
//        {Ctrl ('S'), SR_read,},   //masked off by chaser 2006.05.22
//        {'n', SR_first_new,},
//        {'p', SR_read,},
//        {Ctrl ('X'), SR_read,},
//        {Ctrl ('U'), SR_author,},
...
int
do_gsend (char **userid, char *title, int num)
{
    struct stat st;
...
/* add by rwzmm @ sjtubbs */
/* remove by zsong @ sjtubbs, 2006.02.24*/
//		if(!AllowGroupMail(uid, currentuser.userid))
//			continue;
/* add end */

        sprintf (filepath, "mail/%c/%c/%s", toupper (uid[0]), toupper (uid[1]), uid);
        if (stat (filepath, &st) == -1)
        {
            if (mkdir (filepath, 0755) == -1)
            {
...
#if 0
    //-----add by yl to calculate the length of a mail -----
    sprintf (genbuf, "mail/%c/%c/%s/%s", toupper (userid[0]), toupper (userid[1]), userid, fname);
    if (stat (genbuf, &st) == -1)
        file_size = 0;
    else
        file_size = st.st_blksize * st.st_blocks;
...
int
in_group (char (*uident)[80], int cnt)
{
    int i;
...
int
doforward (char *direct, struct shortfile *fh, int mode)
{
    ...
    if (!internet_mail)
    {
		...
		if(search_record(filepath, &baditem, sizeof (baditem), cmpfnames, currentuser.userid))
		{
            prints ("[%s] ²»ÏëÊÕµ½ÄúµÄÐÅ¼þ¡£\n", lookupuser.userid);
            return -4;
		}
	...
	if (in_mail || (strstr(direct, "0Announce") != NULL))
		...
		sprintf (genbuf, "%s/%s/%s", direct, getfolderstr(fh->filename), fh->filename);
