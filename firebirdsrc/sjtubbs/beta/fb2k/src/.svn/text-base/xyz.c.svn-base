...
unsigned int
setperms (unsigned int pbits, char *prompt, int numbers, int (*showfunc) (unsigned int,int,int))
{
...
x_edits (void)
{
    ...
    static char *explain_file[] =
        { "¸öÈËËµÃ÷µµ", "Ç©Ãûµµ", "×Ô¼ºµÄ±¸ÍüÂ¼", "ÀëÕ¾µÄ»­Ãæ",
          "µ×²¿Á÷¶¯ĞÅÏ¢",
          "ÔİÍ£½ÓÊÜÕ¾ÍâÓÊ¼ş", "×Ô¶¯»ØĞÅÄÚÈİ", NULL
        };
	...
	if(!strcmp (e_file[ch], "plans"))//check plan size
	{
		struct stat st;
...
x_lockscreen (void)
{
    ...
    prints
    ("\n[1;36mÓ©Ä»ÒÑÔÚ[33m %s[36m Ê±±»[32m %-12s [36mÔİÊ±Ëø×¡ÁË...[m",
...
ent_sortdir (void)
{
    ...
    clear ();
    stand_title ("°åÃæÎÄÕÂÅÅĞò");
    get_a_boardname (buf, "ÊäÈëÌÖÂÛÇøÃû (°´¿Õ°×¼ü×Ô¶¯ËÑÑ°): ");
...
fill_date (void)
{
    ...
    next = (time_t) time (0) - ((atoi (h) * 3600) + (atoi (m) * 60) + atoi (s)) + 86400;	/* Ëã³ö½ñÌì 0:0:00 µÄÊ±¼ä, È»ááÔÙÍùáá¼ÓÒ»Ìì */
...
sendGoodWish (char *userid)
{
    ...
    {
        move (7, 0);
        prints ("¶Ô²»Æğ£¬ %s ²»Ô¸Òâ½ÓÊÜÄúµÄ×£¸£/ÁôÑÔ\n", uid);
        pressanykey ();
        clear ();
        return -1;
    }
    move (5, 0);
    prints ("[m¡¾¸ø [1m%s[m ÁôÑÔ¡¿\n", uid);
    move (6, 0);
...
void anywhere_lockscreen(void)
{
    ...
    while (*buf=='\0'||!checkpasswd(currentuser.passwd,buf))
    {
        ...
        clrtobot();
        update_endline();
        move(t_lines-1,87);//ÕâÀïµÄ87ºÍÏÂÃæµÄ66¶¼ÊÇÊı³öÀ´µÄ
        prints("[33m[[36mËøÆÁÖĞ[33m][32m ÇëÊäÈëÃÜÂë[33m:[m               ");
