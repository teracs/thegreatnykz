...
{	
	char filename[80];
	snprintf(filename,80,"home/%c/%c/%s/%s",toupper(currentuser.userid[0]),toupper(currentuser.userid[1]), currentuser.userid,file);
...
f_replace_file(struct boardcollection sstruct)
{
	int sd;
	char file[80];
	snprintf(file,80,"home/%c/%c/%s/%s",toupper(currentuser.userid[0]),toupper(currentuser.userid[1]),currentuser.userid,sstruct.dirname);
...
int getfilename(void)
{
	...
	int filesd,count=0;;
	t = time(0);
    	snprintf(buf3,80,"home/%c/%c/%s/%ld",toupper(currentuser.userid[0]),toupper(currentuser.userid[1]),currentuser.userid,t);
...
f_get_file(char *file,struct boardcollection *sstruct, int n)
{
	...
	snprintf(currentfilename,80,"home/%c/%c/%s/%s",toupper(currentuser.userid[0]),toupper(currentuser.userid[1]),currentuser.userid,file);
...
int getdirnumber(void)
{
	...
	char cnum[10];
	FILE *fp;
	snprintf(bufnum,80,"home/%c/%c/%s/%s",toupper(currentuser.userid[0]),toupper(currentuser.userid[1]),currentuser.userid,".dirnum");
...
int do_adddir(char *boardfile,char* newdir)
{
	...
	time_t now;
	snprintf(boardfilename,80,"home/%c/%c/%s/%s",toupper(currentuser.userid[0]), toupper(currentuser.userid[1]), currentuser.userid,boardfile);
	...
	if(currentdir.subnum<20)
	{
		now=getfilename();
		currentdir.subnum++;
		snprintf(currentdir.subdirsfile[currentdir.subnum-1],80,"%ld",now);
...
int do_addboard(char *boardfile,char* newboard)
{
	...
	if(f_get_file(boardfile, &currentdir, 1)!=0)
		return -1;
	if(boardisexist(currentdir,newboard))//modified by chaser 06.05.24
	{
		move(2,0);
	...
	for (i=0; i<MAXBOARD; i++)
       {
             	...
             	{	
			if (!(brdshm->bcache[i].level & PERM_POSTMASK) && !HAS_PERM (brdshm->bcache[i].level) && !(brdshm->bcache[i].level & PERM_NOZAP))
			                return -1;
        		return i;       		
...
int
cmpboard (const void *vbrd, const void *vtmp)
{
	...
	if((brd->flag2[0] & BOARD_DIR_FLAG) && (!(tmp->flag2[0] & BOARD_DIR_FLAG)))
		return -1;
	if(!(brd->flag2[0] & BOARD_DIR_FLAG) && ((tmp->flag2[0] & BOARD_DIR_FLAG)))
		return 1;
	...
	while(1)
	{
		if(brdnum==-1)
		{
			if(f_get_file(filename,&currentstruct,1)==-1)//file not exists
			{
				if(!strcmp(filename,"main.hck"))
				{
					...
					snprintf(goodboard,80,"home/%c/%c/%s/.goodbrd",toupper(currentuser.userid[0]), toupper(currentuser.userid[1]), currentuser.userid);
			...
			//free(ntbrd);
			ntbrd=(struct newpostdata *)malloc(sizeof(struct newpostdata)*brdnum);
		...
		if(dirnum+brdnum<=0)move(3,0);
        	...
        	switch (ch)
        	{
				...
				move(t_lines - 1, 0);
			        tch[0] = '\0';
			        getdata(t_lines - 1, 0, "²é¿´·ÖÇøÊ®´ó:(1~b)Çø 0)È¡Ïû [0]:", tch, 3, DOECHO, YEA);
			...
			case 'T':
				{
					...
					if(getdata (0, 0, "ÇëÊäÈëÐÂÄ¿Â¼Ãû³Æ: ",dname, IDLEN + 1, DOECHO, YEA))
					{
						...
						if(f_get_file(currentstruct.dirname, &currentstruct, 1)!=0)
							break;
					}
					page=-1;
					break;
			...
			case 'A':		//add a new dir to my favourate. Added by heack@sjtubbs
				...
				if(ndir>=MAX_DIRNUM)
				{
					snprintf(askynbuf,80,"ÄúÒÑ³¬¹ý%d¸öÄ¿Â¼µÄ×î´óÏÞÖÆ.",MAX_DIRNUM);
				...
				{			
                	if (gettheboardname(1, "ÊäÈëÌÖÂÛÇøÃû (°´¿Õ°×¼ü×Ô¶¯ËÑÑ°): ", &pos, &fh, bname))
                	{
						if(fh.flag2[0] & BOARD_DIR_FLAG)
						{
							move(2, 0);
							clrtoeol();
							prints("¶Ô²»Æð£¬Äã²»ÄÜÖ±½ÓÌí¼Ó·ÖÇø×ÓÄ¿Â¼µ½ÊÕ²Ø¼Ð£¡");
							pressanykey();
							page = -1;
							brdnum=-1;
							break;
...
inGoodBrds (char *bname)
{
    ...
    for (i = 0; i < 100/*&&GoodBrd[i].id != 0*/ ; i++) {
	    if(GoodBrd[i].type == 1)
		    continue;
        if ((!strcasecmp (bname, GoodBrd[i].name)) && GoodBrd[i].parent == currdir)
            return i + 1;
    }
    return 0;
}
int 
inGoodBrds_d (char *bname)
{
	int i;
	for (i = 0; i<100/* && GoodBrd[i].id != 0*/; i++) {
		if((!strcasecmp (bname, GoodBrd[i].name)) && GoodBrd[i].parent ==currdir )
...
load_boards (void)
{
    ...
    for (n = 0; n < numboards; n++)
    {
        ...
        if (goodbrd == 0)
        {
			...
			{
				if(bptr->flag2[0] & BOARD_DIR_FLAG)//filter all dirs
					continue;
				if (boardprefix != NULL
    ...
    if (brdnum == 0 && !yank_flag && boardprefix == NULL && (currdir == 0 || boardparent == -1))
    {
        if (goodbrd)
        {
            GoodBrd_count= 0;
...
int
unread_position (char *dirfile, struct newpostdata *ptr)
{
    struct fileheader fh;
    char filename[STRLEN];
		...
		if((ptr->flag2[0] & BOARD_DIR_FLAG))
		{
			...
			if(!newflag)
			{
				prints("  %4d %s %-14s %s %-30s %s  %c%c%c %-12s\n",
					n+1, (ptr->zap&& !(ptr->flag & NOZAP_FLAG)) ? "£­" : "£«",
					ptr->name, " ", ptr->title + 1,	"  \033[1;32m-\033[m",
					' ', ' ', ptr->status, "SYSOP");
			}
			else
			{
				prints("   [Ä¿Â¼]%c%-14s %s %-30s %s  %c%c%c %-12s\n",
					...
					ptr->name, " ", ptr->title + 1,	"  \033[1;32m-\033[m",
					' ', ' ', ptr->status, ptr->BM[0] <= ' ' ? "SYSOP" : strtok (tmpBM, " "));
			}
		} 
...
int
showgood_brdlist (int page, int clsflag, int newflag, struct boardcollection currentstruct, int dirnum, int brdnum, struct newpostdata *nbrd)
{
	struct newpostdata *ptr;
	struct newgood tmpgood;
	int n,i,inboard = -1;
...
choose_board (int newflag)
{
            ...
            else
    {
        ...
        case 'z':
			...
			if (HAS_PERM (PERM_BASIC) && !(nbrd[num].flag & NOZAP_FLAG)/*&& !(nbrd[num].flag2[0] & BOARD_DIR_FLAG)*/)
            {
                ptr = &nbrd[num];
                ptr->zap = !ptr->zap;
                ptr->total = -1;
        ...
        case KEY_RIGHT:
			{
				tmp = num;
				num = 0;
				//modified by zsong, subboard debug @ 20060326
...
//cited by zsong
//because this function is canceled since heack modified the Good board modula
/*
int
choose_board1 (newflag)
int newflag;
{
    static int num;
    struct newpostdata newpost_buffer[MAXBOARD];
    struct newpostdata *ptr;
    int page, ch, tmp, number, tmpnum, exit_flag=0;
    int loop_mode = 0;
	char tch[4];
...
/ *            if (brdnum <= 0)
	...
	case 'A':
            ...
            if (GoodBrd_count >= GOOD_BRC_NUM)
            {
                presskeyfor ("¸öÈËÈÈÃÅ°åÊýÒÑ¾­´ïÉÏÏÞ(20)", t_lines - 1);
            }
            else if (GoodBrd_count && uinfo.mode == FAVOURITE)
            {
                int newid,i;
                char dname[STRLEN];
                if(getdata (0, 0, "ÇëÊäÈëÐÂÄ¿Â¼Ãû³Æ: ",dname, IDLEN + 1, DOECHO, YEA))
                {
                    if (!inGoodBrds_d (dname))
                    {
                        strcpy (GoodBrd[GoodBrd_count].name, dname);
			newid = GoodBrd[0].id;
			for(i=0;GoodBrd[i].id !=0;i++) {
				if(GoodBrd[i].id > newid) 
					newid = GoodBrd[i].id;
			}
			GoodBrd[GoodBrd_count].id = ++newid;
			GoodBrd[GoodBrd_count].type = 1;
			GoodBrd[GoodBrd_count].parent = currdir;
                        save_GoodBrd ();
                        reload_GoodBrd = 1;		//fixme
                        brdnum = -1;
                        break;
                    }
                }
...
break;		
        case 'a':
            ...
            else if (GoodBrd_count && uinfo.mode == FAVOURITE)
            {
                ...
                if (gettheboardname(1, "ÊäÈëÌÖÂÛÇøÃû (°´¿Õ°×¼ü×Ô¶¯ËÑÑ°): ", &pos, &fh, bname))
                {
                    if (!inGoodBrds (bname))
                    {
                        strcpy (GoodBrd[GoodBrd_count].name, bname);
			newid = GoodBrd[0].id;
			for(i=0;GoodBrd[i].id !=0;i++) {
				if(GoodBrd[i].id > newid) 
					newid = GoodBrd[i].id;
    ...
    {		memset(&GoodBrd,0,sizeof(GoodBrd));
        ...
        close (fp);
                ...
                {	int newid;
                    ...
                    if(askyn(askynbuf, YEA, YEA) == YEA)
                    {
                        strcpy (GoodBrd[GoodBrd_count].name, bname);
			newid = GoodBrd[0].id;
			   for(i=0;i<GoodBrd_count;i++) {
				if(newid < GoodBrd[i].id) 
					newid = GoodBrd[i].id;
			   	}
			   newid++;
			   GoodBrd[GoodBrd_count].id = newid;
			   GoodBrd[GoodBrd_count].type = 0;
			   if(GoodBrd[currdir].type == 1) {
			   	GoodBrd[GoodBrd_count].parent = currdir;
			   	}
                        save_GoodBrd ();
			   reload_GoodBrd = 1;
		...
		if(nbrd[num].status == 'd') {
			pos = getposbyid (nbrd[num].total);
	                for (i = pos - 1; i < GoodBrd_count ; i++){
	                    memcpy (&GoodBrd[i], &GoodBrd[i + 1],sizeof(GoodBrd[i]));
                    }    
		for(j = 0;j<GoodBrd_count;j++) {
			if(GoodBrd[j].parent == nbrd[num].total) {
				pos = j+1;
				for(i = pos -1;i<GoodBrd_count;i++)
					memcpy(&GoodBrd[i],&GoodBrd[i + 1],sizeof(GoodBrd[i]));
			}
	...
	case KEY_LEFT:
	    if(currdir != 0) {
		    int i;
		    for(i=0;i<GoodBrd_count;i++) {
			    if(GoodBrd[i].id == currdir)
				    break;
		    }
		    currdir = GoodBrd[i].parent;
		    brdnum = -1;
		    page = -1;
	    } else {
		    exit_flag = 1;
...
int brc_putpushrecord(void)
{
	...
	//if(strcmp(lastboard,currboard))
		//return -1;
	snprintf(filepath,80, "home/%c/%c/%s/.push.%s", toupper(currentuser.userid[0]), toupper(currentuser.userid[1]), currentuser.userid, currboard);
...
int brc_getpushrecord(void)
{
	...
	if((fd=open(filepath,O_RDONLY|O_CREAT,0644))==-1)
		return -1;
	while(read(fd,&brc_pushlist[brc_pushnum],sizeof(struct unread_push))==sizeof(struct unread_push))
	{
		brc_pushnum++;
	}
...
int
brc_updatepush (struct fileheader *fileinfo)
{
	int ftime, n;

	ftime = atoi (&(fileinfo->filename[2]));
    	if ((fileinfo->filename[0] != 'M' && fileinfo->filename[0] != 'G') || fileinfo->filename[1] != '.')
    	{
        	return 0;
    	}
...
int
brc_pushunread (struct fileheader *fileinfo)
{
	int ftime, n;
