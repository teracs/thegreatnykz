...
int do_sendmsg(struct user_info *uentp, char *msgstr, int mode, int userpid)
{
    char uident[STRLEN];
    time_t now;
    struct user_info *uin;
    ...
    *(timestr+24) = '\0';
    ...
    {
		char buffer[512]; //added by proto@sjtubbs 2006.11.30
        sprintf(msgbuf, "[0;1;5;44;33mÕ¾³¤ ÓÚ[36m %-25.25s [33m¹ã²¥£º[m\n[1;37;44m%s[m\n", timestr, msgstr);
		//added by proto@sjtubbs 2006.11.30
		sprintf(buffer, "broadcast '%s' to all users by %s", msgstr, currentuser.userid);
    	report(buffer);
		//added end
    }
    else if (mode == 1)
