        ...
        if (titlebuf[0] == '\0')
{
        ...
        sprintf (mybuf,
		 ...
		 ((bp->flag2[0] & MODULE_FLAG) && ismoduleexist()) ? ",\033[1;32mF\033[mÊ¹ÓÃÄ£°å" : "",
                 (header->postboard
	...
	else if ((ans[0]== 'F'||ans[0]=='f') && (bp->flag2[0] & MODULE_FLAG))//added by chaser 
	{
	    ...
	    getdata(0,0,"ÏÂÃæÊÇËùÓÐµÄÄ£°å,ÇëÊäÈëÄãÒªÑ¡ÓÃµÄÐòºÅ: ",ans1,2,DOECHO,YEA);
...
//   struct module_index module[3];
   ...
   while(fread(&module[number],sizeof(struct module_index),1,fp)!=0)
  {
	...
	module[number].filename[i-2]='\0';//È¥µô_Mºó×º
      	number++;
  }
   fclose(fp);
   return number;
