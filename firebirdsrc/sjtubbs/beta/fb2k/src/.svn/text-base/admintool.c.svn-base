...
int
getbnames (char *userid, char *bname, int *find)
{
    int oldbm = 0;
    FILE *bmfp;
...
m_ordainBM (void)
{
	...
	if(strncmp(bname, "BMTraining", 10))	//not a bmt board
	{
		sprintf (genbuf, "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n"
			 ...
			 "\tÉÏÈÎºóÈıÊ®ÌìÎªÊµÏ°ÆÚ£¬\n"
			 "\tÆÚÂúºóÆßÈÕÄÚÇëÔÚ±¾ÇøDistrict°åÌá½»×ªÕı±¨¸æ£¬\n"
			 "\tÓâÆÚÎ´½»×ªÕı±¨¸æ£¬½«°´ÊµÏ°ÆÚ´ÇÖ°´¦Àí¡£\n",
	...
	{
		currboard[0] = '\0';
		autoreport (bmfilename, buffer, lookupuser.userid, districtname);
	}
...
m_retireBM (void)
{
    ...
    if (right)
    {
        sprintf (genbuf, "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n"
                 ...
                 "\tÔÚ´Ë£¬¶ÔÆäÔø¾­ÔÚ %s °åµÄĞÁ¿àÀÍ×÷±íÊ¾¸ĞĞ»¡£\n\n"
                 "\tÏ£Íû½ñºóÒ²ÄÜÖ§³Ö±¾°åµÄ¹¤×÷.",
...
int ChangeTheDir(struct boardheader* oldfh, int pos)
{
	...
	if (oldfh)
	{
		if (!(oldfh->flag2[0] & BOARD_DIR_FLAG)) // not a dir...
			return 0;
	}
    if (oldfh)
    ...
    {
        prints
        ("Äú½«ÖØĞÂÉè¶¨ [\033[32m%s\033[m] µÄÄ¿Â¼ÊôĞÔ [Enter-->Éè¶¨²»±ä]",
         oldfh->filename);
    }
    else
        prints ("Äú½«¿ªÊ¼´´½¨Ò»¸ö\033[32mĞÂ\033[mµÄÄ¿Â¼ [ENTER-->È¡Ïû´´½¨]");
    while (1)
    {
        struct boardheader dh;

        getdata (3, 0, "Ä¿Â¼Ãû³Æ(Ó¢ÎÄÃû): ", buf, BDNAME_LEN, DOECHO, YEA);
        if (buf[0] == '\0')
        {
            if (oldfh)
    ...
    prints ("\n[Ä¿Â¼ËµÃ÷]·ÖÇø´úÂë + ·ÖÇøÃèÊö + ×ªĞÅ±êÖ¾ + ÌÖÂÛÇø±êÌâ\n");
    if (oldfh)
    {
        ...
        prints ("¸ñÊ½·¶Àı£ºa[±¾Õ¾] ¡ğ \033[32mÕâÊÇÒ»¸öÄ¿Â¼\033[m\n");
    while (1)
    {
        if (oldfh)
            ...
            buf[0] = '\0';
        getdata (8, 0, "Ä¿Â¼±êÌâ(ÖĞÎÄÃû): ", buf, 41, DOECHO, NA);
        if (buf[0] == '\0')
        {
            if (oldfh == NULL)
                return -1;
    ...
    while (1)
    {
        ...
        num[0] = ch;
        num[1] = '\0';
        getdata (11, 0, "¸ÃÄ¿Â¼·ÅÖÃÔÚÄÄÒ»·ÖÇø(²Î¿¼ÓÒ±ßÌáÊ¾±àºÅ)£¿: ",
                 num, 2, DOECHO, NA);
        if (num[0] >= 'a' && num[0] <= 'z')
    ...
    while (1)
    {
        if (oldfh == NULL
            ...
            chs[0] = oldfh->title[0];
        chs[1] = '\0';
        getdata (14, 0, "ÇëÊäÈë¸ÃÄ¿Â¼µÄ·ÖÀà·ûºÅ: ", chs, 2, DOECHO, NA);
        if (chs[0] == '\0')
            continue;
    ...
    while (1)
    {
        if (oldfh)
        {
            ...
            buf[0] = '\0';
        getdata (15, 0, "ÇëÊäÈë¸ÃÄ¿Â¼µÄ·ÖÀàÃèÊö: ", buf, 7, DOECHO, NA);
        if (buf[0] == '\0')
            continue;
        if (killwordsp (buf) == 0)
            continue;
...
//add by zsong, allow to set bm for directory
...
//add end
	...
	if (askyn ("±¾Ä¿Â¼ĞèÒªÉèÖÃ READ »ò POST ÏŞÖÆÂğ", NA, NA) == YEA)
    {
        char ans[4];
    ...
    if (i > 0)
    {
        ...
        if (oldfh)
        {
            sprintf (genbuf, "¸ü¸ÄÄ¿Â¼ %s µÄ×ÊÁÏ --> %s",
    ...
    numboards = -1;
    {
        ...
        if (oldfh)
            sprintf (secu, "ĞŞ¸ÄÄ¿Â¼£º%s(%s)", oldfh->filename, newfh.filename);
        else
            sprintf (secu, "³ÉÁ¢ĞÂÄ¿Â¼£º%s", newfh.filename);
        securityreport (secu);
    }
    pressanykey ();
    clear ();
...
ChangeTheBoard (struct boardheader *oldfh, int pos)
{
    ...
    char num[2], chs[2], items[9], nntp[3], group[16];
    int i, j, a_mv, k;
	char folderpath[100]; //added by clist at sjtubbs
	...
	if (gettheboardname (17, "ÊäÈëËùÊôÄ¿Â¼Ãû(¸ùÄ¿Â¼Ö±½Ó»Ø³µ): ", &tmppos, &tmpfh, genbuf))
		newfh.group = tmppos;
	else 
		newfh.group = 0;
	...
	if (askyn ("ÉèÖÃ±¾°åÎª²»¿É Re Ä£Ê½°åÃæÂğ",
		...
		newfh.flag &= ~NOREPLY_FLAG;
	// added by cf,2005 05 27
	if(askyn("ÉèÖÃ±¾°åÎª²»¼ÆPOSTÊı°åÃæÂğ", (newfh.flag & NOPOST_FLAG)?YEA:NA,NA)==YEA)
		newfh.flag |= NOPOST_FLAG;
	else
		newfh.flag &= ~NOPOST_FLAG;
         ////////////////////////////////////////////////////////////////////////////////////
	if(askyn("ÉèÖÃ±¾°åÖ§³Ö·¢ÎÄÄ£°åÂğ?",
			  (newfh.flag2[0] & MODULE_FLAG) ? YEA : NA,NA) == YEA)
	{
		FILE *fp;
		char buf[STRLEN];
//		setbfile(buf,newfh.filename,"module");
//		mkdir(buf,0755);
	...
	if(askyn("ÉèÖÃ±¾°åÎªÍÆÎÄÄ£Ê½°åÃæÂğ", (newfh.flag & PUSH_FLAG)?YEA:NA,NA)==YEA)
		newfh.flag |= PUSH_FLAG;
	else
		newfh.flag &= ~PUSH_FLAG;
    ...
    if (oldfh == NULL)
    {
		...
		//build folders: add by clist at sjtubbs
...
m_newbrd (void)
{
	...
	sprintf (dirbuf, "ĞÂ½¨Ò»¸ö (B)°åÃæ »ò (D)Ä¿Â¼  [B]: ");
...
ShowBrdSet (struct boardheader fh)
{
	...
	if(fh.group >0)
	{
		prints ("ËùÊôÄ¿Â¼         : %s(%d)", brdshm->bcache[fh.group - 1].filename, fh.group);
	}
	else
	{
		prints ("ËùÊôÄ¿Â¼         : ¸ùÄ¿Â¼");
    ...
    move(i++,0);
    prints("ÊÇ·ñÖ§³ÖÄ£°å :%s",
	   (fh.flag2[0] & MODULE_FLAG) ? "Ö§³Ö" :"²»Ö§³Ö"); 
    move (i++, 0);
    prints ("±¾ÌÖÂÛÇøËµÃ÷     : %s\n\n", fh.title);
}
...
m_editbrd (void)
{
    ...
    if (askyn ("ÊÇ·ñĞèÒª¸ü¸ÄÒÔÉÏÊôĞÔ", NA, NA) == YEA)
    {
        clear ();
        stand_title ("ĞŞ¸ÄÌÖÂÛÇø»ù±¾ÊôĞÔ");
		if(fh.flag2[0] & BOARD_DIR_FLAG)//dir
			ChangeTheDir(&fh, pos);
		else
...
//    }
    ...
    {
		// modified by rwzmm @ tech group
		// approved by altheimer, for security purpose
        ...
        if (uident[0] != '\0')
		{
            ...
            else
            {
                sprintf (genbuf, "home/%c/%c/%s/register",
                         toupper (lookupuser.userid[0]), toupper (lookupuser.userid[1]), lookupuser.userid);
...
d_user (char *cid)
{
    ...
    securityreport (secu);
    sprintf (genbuf, "mail/%c/%c/%s", toupper (lookupuser.userid[0]), toupper(lookupuser.userid[1]),
             lookupuser.userid);
    f_rm (genbuf);
    sprintf (genbuf, "home/%c/%c/%s", toupper (lookupuser.userid[0]), toupper (lookupuser.userid[1]), 
             lookupuser.userid);
    f_rm (genbuf);
...
a_edits (void)
{
    ...
    static char *e_file[] =
        { "../Welcome", "../Welcome2", "issue", "logout", "../vote/notes",
          ...
          "DISTmaster.ini", "macroboards",  "goodbm", "../ALL_NO_POST_TIME",
		  "../NO_POST_TIME", "keywords","infobrds", "bad_remail",
#ifdef REAL_NAME_EMAIL
		  "../"REAL_NAME_EMAIL_RULE,"../"REAL_NAME_EMAIL_RULE_DSC,
#endif		  
		  "district_data.inc.php" ,NULL
        };
    static char *explain_file[] =
        { "ÌØÊâ½øÕ¾¹«²¼À¸", "½øÕ¾»­Ãæ", "½øÕ¾»¶Ó­µµ", "ÀëÕ¾»­Ãæ",
          ...
          "ÔÊĞíÊ¹ÓÃºê¶¨Òå°å","ÓÅĞã°åÖ÷Ãûµ¥", "È«Õ¾Ö»¶ÁÊ±¼ä¶Î","Ğ£ÍâÖ»¶ÁÊ±¼ä¶Î", "²»¿É°üº¬´Ê»ã", "ÆôÓÃinfoµÄ°å","²»¿É°ó¶¨µÄÓÊÏä",
#ifdef REAL_NAME_EMAIL          
          "°ó¶¨ÓÊÏä¹æÔò","°ó¶¨ÓÊÏä¹æÔòËµÃ÷",
#endif          
          "·ÖÇøÊ®´óÉèÖÃ",NULL
        };
    ...
    if (aborted != -1)
    {
		...
		else if(!strcmp (e_file[ch], "keywords"))
		{
			FILE* fp_filter = NULL;
			char mybuffer[256] = {0};
			int mycount = 0;
			...
			if(fp_filter != NULL)
			{
				...
				while(fgets(mybuffer, sizeof(mybuffer), fp_filter))
				{
					if(item_count >= MAX_ITEM)
						break;
					if (strlen(mybuffer) <= 4)//too short, overlook
					{
						memset(mybuffer, 0x00, sizeof(mybuffer));
						continue;
					}
					if(strlen(mybuffer) > MAX_FILTER_LENGTH + 1)// a "\n" exists
					{
					...
					strncpy(brdshm->wordfilter[item_count], mybuffer, strlen(mybuffer));
		...
		else if(!strcmp (e_file[ch], "../ALL_NO_POST_TIME"))
		{
			FILE* fp_nopost = NULL;
		...
		else if(!strcmp (e_file[ch], "../NO_POST_TIME"))
		{
			FILE* fp_nopost = NULL;
			char buffer[2][16] = {{0},{0}};
...
m_retireQZ (void)
{
    ...
    if(counts == 10)
    {
        ...
        sprintf (msg,
                 "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n\t¾­Õ¾Îñ×éÌÖÂÛ£º\n\tÍ¬Òâ %s ´ÇÈ¥ A ÇøÇø³¤µÄÖ°Îñ¡£\n\tÔÚ´Ë£¬¶ÔÆäÔø¾­ÔÚ A ÇøµÄĞÁ¿àÀÍ×÷±íÊ¾¸ĞĞ»¡£\n\n",
                 lookupuser.userid);
    }
    ...
    else if(counts == 11)
    {
        ...
        sprintf (msg,
                 "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n\t¾­Õ¾Îñ×éÌÖÂÛ£º\n\tÍ¬Òâ %s ´ÇÈ¥ B ÇøÇø³¤µÄÖ°Îñ¡£\n\tÔÚ´Ë£¬¶ÔÆäÔø¾­ÔÚ B ÇøµÄĞÁ¿àÀÍ×÷±íÊ¾¸ĞĞ»¡£\n\n",
                 lookupuser.userid);
    }
    ...
    else
    {
        ...
        sprintf (msg,
                 "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n\t¾­Õ¾Îñ×éÌÖÂÛ£º\n\tÍ¬Òâ %s ´ÇÈ¥ %d ÇøÇø³¤µÄÖ°Îñ¡£\n\tÔÚ´Ë£¬¶ÔÆäÔø¾­ÔÚ %d ÇøµÄĞÁ¿àÀÍ×÷±íÊ¾¸ĞĞ»¡£\n\n",
...
/* add by rwzmm, district master add bmtraining bms */

int
m_AddTrainingBM (void)
{
    int id, pos, oldbm = 0, i, find, bm = 1;
    struct boardheader fh;
    FILE *bmfp;
	...
	{
		move(2, 0);
		prints("ÄúÃ»ÓĞµ£ÈÎÊµ¼ÊµÄÇø³¤Ö°Îñ¡£\n\nÈç¹ûÄúÊÇÕ¾³¤£¬ÇëÊ¹ÓÃ°åÖ÷ÈÎÃüÑ¡µ¥½øĞĞ²Ù×÷£¡");
		pressanykey();
		return 0;
	}
	...
	pos = search_record (BOARDS, &fh, sizeof (struct boardheader), cmpbnames, bname);
...
m_MinusTrainingBM (void)
{
	...
	{
		move(2, 0);
		prints("ÄúÃ»ÓĞµ£ÈÎÊµ¼ÊµÄÇø³¤Ö°Îñ¡£\n\nÈç¹ûÄúÊÇÕ¾³¤£¬ÇëÊ¹ÓÃ°åÖ÷ÀëÈÎÑ¡µ¥½øĞĞ²Ù×÷£¡");
		pressanykey();
		return -1;
	}
...
//        prints (" %s %s°å°åÖ÷£¬ÈçÓĞ´íÎó£¬ÇëÍ¨Öª³ÌĞòÕ¾³¤¡£",
  //              lookupuser.userid, (oldbm) ? "²»ÊÇ¸Ã" : "Ã»ÓĞµ£ÈÎÈÎºÎ");
		prints(" %s ²»ÊÇ %s °å°åÖ÷£¬ÈçÓĞ´íÎó£¬ÇëÍ¨Öª³ÌĞòÕ¾³¤¡£", lookupuser.userid, bname);
        pressanykey ();
        clear ();
        return -1;
    }
