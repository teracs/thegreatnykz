...
*/
/*
$Id: namecomplete.c,v 1.3 2004/07/04 07:09:39 bbs Exp $
*/
...
int
NumInList (register struct word *list)
{
    register int i;
...
struct word *
            GetSubList (register char *tag, register struct word *list)
{
    ...
    while ((ch = igetkey ()) != EOF)
    {
        if (ch == '\n' || ch == '\r')
        {
            *temp = '\0';
            ...
            {			/*  °åÃæ ID Ñ¡ÔñµÄÒ»¸ö¾«È·Æ¥ÅäÎÊÌâ  period */
                struct word *list;
...
int
UserMaxLen (char (*cwlist)[13], int cwnum, int morenum, int count)
{
    int len, max = 0;
...
int
UserSubArray (char (*cwbuf)[13], char (*cwlist)[13], int cwnum, int key, int pos)
{
    int key2, num = 0;
    int n, ch;
