...
visitlog (void)
{
    ...
    close (vfp);
    sprintf (genbuf,
             "[1;32m´Ó [[36m%4dÄê%2dÔÂ%2dÈÕ[32m] Æð, ×î¸ßÈËÊý¼ÇÂ¼: [[36m%d[32m] ÀÛ¼Æ·ÃÎÊÈË´Î: [[36m%lu[32m][m\n",
...
inline int is_strcase_with_prefix(const char *prefix,int len,const char *str)
{
	return !strncasecmp(prefix,str,len);
}
inline int is_strcase_with_suffix(const char *suffix,int len,const char *str)
{
	int slen;
	slen = strlen(str);
	if(slen<len)
...
int check_email_domain(const char *domain)
{
	...
	int i;
	/**
	 * first char must be alnum, the last char must be alpha 
	 * all other chars maust be alnum or . or -
	 */
	 ...
	 * for each line, if its first char is 
	 * p/P: permit/prohibid domain with prefix same as rest of the line
	 * s/S: permit/prohibid domain with suffix same as rest of the line
	 * a/A: permit/prohibid domain same as rest of the line
	 * others: ignore this line
	 *
	 * the first meet rule takes effect
	 */
	if( !(fp = fopen(REAL_NAME_EMAIL_RULE,"r"))) return 1;
...
mailto_real_name_mail(char *userid)
{
	...
	const char title[] = BBSNAME"×¢²áÈ·ÈÏÐÅ";
	...
	if(content == NULL)
		return 2;
	fprintf(content,"\n\
    ÄãºÃ£¡ÕâÊÇ%s(%s)×¢²áÈ·ÈÏÐÅ¡£\n\
    Èç¹ûÄãÔøÔÚ%s×¢²áID %s,ÇëÓÃÕâ¸öµÄÁ´½ÓÍê³É×¢²á£º\n\
    http://%s/bbsdochkmail?id=%s&code=%08x%08x%08x%08x\n\
    Èç¹ûÄãÃ»ÓÐ×¢²á¹ý´ËID£¬ÇëÉ¾³ý±¾ÐÅ¡£ÎÒÃÇÎª´ËÐÅµÄ´òÈÅ¸Ðµ½Ç¸Òâ¡£\n\n\n\
                                                  %sÕ¾Îñ×é\n",
...
static int
check_real_name_email_blocked(const char *email)
/* check the remail if it's blocked.
 *  * RETURN: 0 for blocked,1 for a non-blocked one
 *   */
{
...
ask_real_mail(void)
{
	...
	clear();
	move(3,0);
	if(askyn("ÏÖÔÚÒª°ó¶¨ÓÊÏäÂð",YEA,YEA)==NA)
		return;
	get_real_name_email(email);
	buf[0] = '\0';
	sprintf(genbuf,"ÇëÊäÈëÓÊÏä[%s] :",email);
	while(check_real_name_email(buf))
	{
		clear();
		getdata(1,0,genbuf,buf,REAL_NAME_EMAIL_LEN,DOECHO,YEA);
		if( askyn("ÊäÈëÕýÈ·Âð ",YEA,NA))
		{
			if(strlen(buf)==0) 
				strcpy(buf,email);
		}
...
/*		prints("¸ÃÓÊÏäÒÑ±»½ûÖ¹°ó¶¨");*/
	...
	if(mailto_real_name_mail(currentuser.userid))
		prints("·¢ËÍÈ·ÈÏÓÊ¼þÊ§°Ü");
	else
		prints("È·ÈÏÓÊ¼þÒÑ·¢ËÍ£¬Çë²éÊÕ");
	pressanykey();
...
user_login (void)
{
    ...
    currentuser.lasthost[59] = '\0';	/* dumb mistake on my part */
...
/* added by cf,get blog default post catId,20050723*/
...
/* Add by Ellias@sjtubbs, Jan 6th, 2008 */
int
seek_in_qzdenyfile()
{
...
//				fclose(fp);
				...
				if (flag=='a' && (strncmp (datestring,date,16) >= 0))
				{
					if ( del_from_file(filename, currentuser.userid))
					{
						...
						sprintf(repbuf, "»Ö¸´%sÔÚÈ«Õ¾µÄ·¢ÎÄÈ¨ÏÞ",
								currentuser.userid);
						sprintf(msgbuf, "\n  %s ÍøÓÑ£º\n\n"
								"\tÒò·â½ûÊ±¼äÒÑ¹ý£¬ÏÖ»Ö¸´ÄúÔÚÈ«Õ¾µÄ¡º·¢±íÎÄÕÂ¡»È¨Á¦¡£\n\n"
								"\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tBy %s",
								currentuser.userid, BBSID);
#ifdef DENY_POST_BOARDNAME
						if (strncmp(currboard, "BMTraining", 10) == 0)
						{
							autoreport(repbuf,msgbuf,currentuser.userid,TRAIN_ANNOUNCE_BOARDNAME);
						}
						else
						{
							autoreport(repbuf,msgbuf,currentuser.userid,DENY_POST_BOARDNAME);
						}
	
#else
	
						autoreport(repbuf,msgbuf,currentuser.userid,NULL);
#endif
...
#endif
{
	...
	seek_in_qzdenyfile();	/* ¼ì²éÊÇ·ñ±»È«Õ¾ */
    num_alcounter ();
...
c_recover (void)
{
    ...
    sprintf (fname, "home/%c/%c/%s/%s.deadve", toupper (currentuser.userid[0]), toupper (currentuser.userid[1]),
    ...
    getdata (0, 0,
             "[1;32mÄúÓÐÒ»¸ö±à¼­×÷Òµ²»Õý³£ÖÐ¶Ï£¬ (M) ¼Ä»ØÐÅÏä (Q) ËãÁË£¿[M]£º[m",
             genbuf, 2, DOECHO, YEA);
    switch (genbuf[0])
    {
    case 'Q':
...
/*
    ...
    case 's':
        while (1)
        {
            ...
            if (a >= 0 && a <= 7)
            {
                sprintf (buf, "home/%c/%c/%s/clip_%d",
...
*/
    default:
		stat (fname, &st);
		if(st.st_size > 65536*10)//over 64k file, check mailbox size
		{
			if(getmailsize (currentuser.userid) + st.st_size > (getmailboxsize (currentuser.userid, currentuser.userlevel) * 2))
			{
				move(5, 0);
				prints("ÐÅÏäÒÑÂú£¬ÎÞ·¨¼Ä»Ø£¡");
				pressanykey();
				unlink (fname);
				return;
			}
...
tlog_recover (void)
{
    ...
    sprintf (buf, "home/%c/%c/%s/talk_log", toupper (currentuser.userid[0]), toupper (currentuser.userid[1]),
             currentuser.userid);
