...
check_uinfo (struct userec *u, int MUST)
{
    ...
    {				// ¼ì²é³öÉúÈÕ
        changeIT = MUST || (u->birthday < 1) || (u->birthday > 31)
                   || (((u->birthmonth < 8 && !(u->birthmonth % 2)) || (u->birthmonth > 7 && (u->birthmonth % 2))) && u->birthday > 30 )
...
uinfo_query (struct userec *u, int real, int unum)
{
    ...
    case '3':
        ...
        else
        {
            ...
            else
            {
                ...
                if (*genbuf != '\0')
                {
		    if (id_with_num (genbuf))/*added by chaser to avoid fleg's careless...*/
       		    {
    ...
    if (askyn ("È·¶¨Òª¸Ä±äÂğ", NA, YEA) == YEA)
    {
        ...
        {
            sprintf (src, "mail/%c/%c/%s", toupper (u->userid[0]), toupper (u->userid[1]), u->userid);
            sprintf (dst, "mail/%c/%c/%s", toupper (newinfo.userid[0]), toupper (newinfo.userid[1]),
                     newinfo.userid);
            rename (src, dst);
