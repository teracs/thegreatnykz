...
*/
/*
$Id: stuff.c,v 1.2 2004/06/29 14:59:22 rwzmm Exp $
*/
...
pressreturn (void)
{
            ...
            msgmorebar (char *filename)
            {
                ...
                prints
                ("[0m[1;44;32mÑ¶Ï¢ä¯ÀÀÆ÷   ±£Áô <[1;37mr[32m>    Çå³ý <[1;37mc[1;32m>   ¼Ä»ØÐÅÏä<[1;37mm[1;32m>                                [m");
                move (t_lines - 1, 0);
...
int askyn (char *str, int defa, int gobottom)
{
    ...
    sprintf (realstr, "%s %s", str, (defa) ? "(YES/no)? [Y]" : "(yes/NO)? [N]");
    if (gobottom)
        move (t_lines - 1, 0);
    getyx (&x, &y);
    clrtoeol ();
