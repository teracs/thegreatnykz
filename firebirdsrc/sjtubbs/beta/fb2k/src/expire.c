 ...
 *
 * CVS: $Id: expire.c,v 1.2 2004/06/29 14:59:21 rwzmm Exp $
 */
/*-------------------------------------------------------*/
...
#define DEF_DAYS        60
//next 2 lines removed by alt@sjtu
...
struct life
{
    char bname[BDNAME_LEN];	/* board ID */
...
expire (life *brd)
{
    ...
    /*
       ...
       #endif
     */
    //  next 2 lines changed by alt@sjtu
    ...
    if ((fdr = open (index, O_RDONLY, 0)) > 0)
    {
        ...
        if ((fdw = open (tmpfile, O_WRONLY | O_CREAT | O_EXCL, 0644)) > 0)
        {
            while (read (fdr, &head, sizeof head) == sizeof head)
            {
                ...
                {
                    sprintf (fname, "%s/%s", getfolderstr(head.filename), head.filename);
                    unlink (fpath);
                    printf ("\t%s\n", fname);
                    total--;
                }
