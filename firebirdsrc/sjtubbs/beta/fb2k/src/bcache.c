...
// add by Ellias @ sjtubbs
// check whether current user is in the board allow list
// 1: in; 0: not in; 2: has no list
int
chk_list(board)
char *board;
{
...
int
apply_boards (int (*func) (struct shortfile*))
{
    register int i;
    ...
    for (i = 0; i < numboards; i++)
        if ((bcache[i].level & PERM_POSTMASK || HAS_PERM (bcache[i].level) ||
			...
			&& (HAS_PERM (PERM_SYSOP) || chk_list(bcache[i].filename)) )	//modified by Ellias, add a param
...
void
getuserid (char *userid, unsigned int uid)
...
char *
u_namearray (char (*buf)[13], int *pnum, char *tag)
{
...
int
apply_ulist (int (*fptr) (struct user_info *))
{
...
int
apply_ulist_address (int (*fptr) (struct user_info *))
{
    int i, max;
        ...
        *uentp = utmpshm->uinfo[i];
        if ((*fptr) (farg, uentp))
        {
            ...
            if ((uentp->invisible == 0) || (uentp->uid == usernum)
                    || ((uentp->invisible == 1) && HAS_PERM (PERM_SYSOP | PERM_SEECLOAK)))
            {
                num++;
            }
            else
