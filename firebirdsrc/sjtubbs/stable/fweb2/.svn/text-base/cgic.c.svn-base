...
int serv_request()
{
    ...
    if (cgiStrEqNc(cgiRequestMethod, "post"))
    {
        ...
        {
#ifdef CGICDEBUG
            CGICDEBUGSTART fprintf(dout, "Calling PostFormInput\n");
            CGICDEBUGEND
#endif                          /* CGICDEBUG */
                if (cgiParsePostFormInput() != cgiParseSuccess)
            {
#ifdef CGICDEBUG
                CGICDEBUGSTART fprintf(dout, "PostFormInput failed\n");
                CGICDEBUGEND
#endif                          /* CGICDEBUG */
                    cgiFreeResources();
                return -1;
...
#ifdef CGICDEBUG
            CGICDEBUGSTART fprintf(dout, "PostFormInput succeeded\n");
            CGICDEBUGEND
#endif                          /* CGICDEBUG */
        }
        ...
        {
#ifdef CGICDEBUG
            CGICDEBUGSTART fprintf(dout, "Calling PostMultipartInput\n");
            CGICDEBUGEND
#endif                          /* CGICDEBUG */
...
#ifdef CGICDEBUG
            CGICDEBUGSTART fprintf(dout, "PostMultipartInput succeeded\n");
            CGICDEBUGEND
#endif                          /* CGICDEBUG */
        }
    }
    else if (cgiStrEqNc(cgiRequestMethod, "get"))
    {
        ...
        {
#ifdef CGICDEBUG
            CGICDEBUGSTART fprintf(dout, "GetFormInput succeeded\n");
            CGICDEBUGEND
#endif                          /* CGICDEBUG */
        }
    }
...
static cgiParseResultType cgiParsePostFormInput()
{
    ...
    if (((int) fwebio_read(input, cgiContentLength)) != cgiContentLength)
    {
        return cgiParseIO;
    }
    ...
    while (1)
    {
               ...
               (mpp, attr, sizeof(attr), value, sizeof(value)))
        {
            ...
            else if (cgiStrEqNc(attr, "Content-Type"))
            {
                ...
                decomposeValue(value,
                               fcontentType,
                               sizeof(fcontentType), argNames, 0, 0);
            }
        }
...
static cgiUnescapeResultType cgiUnescapeChars(char **sp, char *cp,
                                              int len);
...
cgiFormResultType cgiFormFilePath(char *name, char *buf, int buflen)
{
    cgiFormEntry *e;
    int path_len;
...
cgiFormResultType cgiFormStringMultiple(char *name, char ***result)
{
    ...
    else
    {
        *result = stringArray;
#ifdef CGICDEBUG
        CGICDEBUGSTART fprintf(dout, "StringMultiple found nothing\n");
        CGICDEBUGEND
#endif                          /* CGICDEBUG */
            return cgiFormNotFound;
    }
...
cgiFormResultType
cgiFormIntegerBounded(char *name, int *result, int min, int max,
                      int defaultV)
{
                    ...
                    int *result, int defaultV)
{
    ...
    {
#ifdef CGICDEBUG
        CGICDEBUGSTART fprintf(dout, "%s %s\n", choicesText[i], e->value);
        CGICDEBUGEND
#endif                          /* CGICDEBUG */
                         ...
                         char *path, char *domain)
{
        ...
        if (domain)
        {
            fwebio_printf
                ("Set-Cookie: %s=%s; expires=%s, %02d-%s-%04d %02d:%02d:%02d GMT; path=%s; domain=%s\r\n",
                 name, value, days[gt->tm_wday], gt->tm_mday,
                 months[gt->tm_mon], gt->tm_year + 1900, gt->tm_hour,
                 gt->tm_min, gt->tm_sec, path, domain);
        }
        else
        {
            fwebio_printf
    ...
    else
    {
        if (domain)
        {
            fwebio_printf("Set-Cookie: %s=%s; path=%s; domain=%s\r\n",
                          name, value, path, domain);
        }
        else
        {
            fwebio_printf("Set-Cookie: %s=%s; path=%s\r\n",
                          name, value, path);
        }
    }
}
