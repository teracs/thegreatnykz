#include "www.h"
extern char *get_anno_path(char *); 
extern int is_voting(char *);

int bbsdocxml_main()
{
    char board[80], page_str[10], filename[80], dirname[80],check_str[2];
    struct shortfile *x1;
    struct fileheader *fh;
    struct stat dirfile;
    int width = 720, page = -1, num = 0, from, to, showtop = 0, i,check=0;

    silence = 1;
    init_all();
    strsncpy(board, getparm("board"), 32);
    strsncpy(page_str, getparm("page"), 10);
    strsncpy(check_str, getparm("check"), 2);
    ...
    printf("<wishwords><![CDATA[");
    show_bm_words(board, 610);
    printf("]]></wishwords>\n");
    printf("<vote>%d</vote>",(int )is_voting(board));
    printf("<annpath><![CDATA[ %s ]]></annpath>",get_anno_path(board));
    printf("</meta>\n");
    printf("<posts>\n");
    for (i = from; i != to; i++)
    {
