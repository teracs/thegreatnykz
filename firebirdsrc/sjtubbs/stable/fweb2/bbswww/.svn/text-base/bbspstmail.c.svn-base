...
int bbspstmail_main()
{
    ...
    if ((!HAS_PERM(PERM_POST) || !HAS_PERM(PERM_PERSONAL))&& !strncmp(file,"",20))
        http_quit_print("抱歉, 你被封禁了发文权限, 无法发信.");
    ...
    printf("<br>\n");
    printf("<textarea name=text rows=20 cols=76 wrap=physicle onkeydown=\"ctlent()\">\n\n");
    if (file[0])
    {
        int lines = 0;
