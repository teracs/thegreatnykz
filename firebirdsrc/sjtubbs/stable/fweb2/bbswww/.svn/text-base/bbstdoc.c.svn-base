...
int bbstdoc_main()
{
    ...
    printf("<nobr><center>\n");
    printf
        ("<font class=\"title\">%s - 主题阅读　　</font>[讨论区: %s] 板主[%s] 文章%d, 主题%d个<hr>\n",
...
#ifdef STATIC_URL
        printf("<td><a href=bbstcon,board,%s,reid,%d.html>○ %s </a>(%d回复)",
               board, fhs[i]->reid, void1(nohtml2(fhs[i]->title)), replynum);
#else
        printf("<td><a href=bbstcon?board=%s&reid=%d>○ %s </a>(%d回复)",
               board, fhs[i]->reid, void1(nohtml2(fhs[i]->title)), replynum);
#endif
    }
    unmap_board_dir();
    if (showtop)
    {
        ...
        if (NULL != fh)
        {
            ...
            for (i = 0; i < size; i++)
            {
                ...
                printf
                    ("<td><a href=bbstopcon?board=%s&file=%s>&nbsp;&nbsp;%s%s </a>%s",
