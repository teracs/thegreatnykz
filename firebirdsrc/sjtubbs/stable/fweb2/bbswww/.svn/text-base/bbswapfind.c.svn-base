...
static int
search2(char *id, char *pat, char *pat2, char *pat3, int dt, int dt2)
{
    FILE *fp = 0;
    ...
    printf
        ("<font class=\"title\">%s - 站内文章查询结果</font> <br>\n",
         BBSNAME);
    printf("作者: %s ", id);
    ...
    for (i = 0; i < MAXBOARD; i++)
    {
        ...
        while (1)
        {
            ...
            printf
                ("<td><a href=\"bbswapcon?board=%s&file=%s&num=%d\" target=\"_blank\">%s</a>\n",
        ...
        if (total == 0)
            continue;
        printf("<br>以上%d篇来自 <a href=bbswapdoc?board=%s>%s</a><br><br>\n",
               total, board, board);
    }
    if (fp != 0)
        fclose(fp);
...
int bbswapfind_main()
{
    ...
    if (!flag)
    {
        printf("<hr>\n");
        printf("<table>\n");
        printf("<form action=bbswapfind method=post>\n");
    ...
    else
    {
        ...
        if (day2 > 9999)
            day2 = 9999;
        search2(user, title, title2, title3, day * 86400, day2 * 86400);
    }
    printf("</html>");
    return 0;
}
