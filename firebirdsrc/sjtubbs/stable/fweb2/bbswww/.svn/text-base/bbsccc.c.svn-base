...
static int do_ccc(struct fileheader *x, char *board, char *board2)
{
    FILE *fp, *fp2;
    char title[512], buf[512], path[200], path2[200], i, origintime[32], *left, *right;
    time_t now = time(0);
    ...
    if (buf[0]){
       ...
       if (right > left){
           *right = 0;
           sprintf(origintime, "%s", left + 1);
           *right = ')';
       }    
    }    
    if (!origintime[0])
       sprintf(origintime, "Î´ÖªÊ±¼ä");
    fprintf(fp2, "[37;1m¡¾ ÒÔÏÂÎÄ×Ö×ªÔØ×Ô [32m%s [37mÌÖÂÛÇø ¡¿\n",
            board);
    fprintf(fp2, "[37;1m¡¾ Ô­ÎÄÓÉ [32m%s[37m ÓÚ [33m%s[37m ·¢±í ¡¿\033[m\n",
            x->owner, origintime);
    fprintf(fp2, "[[Post:%s,%s|¡¾ Ô­ÎÄÁ´½Ó ¡¿]]\n\n", board, x->filename);
    while (1)
    {
        if (fgets(buf, 256, fp) == 0)
            break;
    ...
    cross_article(board2, title, path2, currentuser.userid,
                 currentuser.username, fromhost);
    unlink(path2);
    printf("'%s' ÒÑ×ªÌùµ½ %s °å.<br>\n", nohtml(title), board2);
    printf
        ("<input type=\"button\" name=\"a\" onclick=\"history.go(-2)\" value=\"[·µ»Ø]\">\n");
...
int bbsccc_main()
{
    ...
    printf
        ("<center><font class=\"title\">%s - ×ªÔØÎÄÕÂ¡¡¡¡</font>[Ê¹ÓÃÕß: %s]<hr>\n",
    ...
    if (target[0])
    {
        ...
        if (!has_post_perm(&currentuser, target))
            http_quit_print("´íÎóµÄÌÖÂÛÇøÃû³Æ»òÄãÃ»ÓĞÔÚ¸Ã°å·¢ÎÄµÄÈ¨ÏŞ");
