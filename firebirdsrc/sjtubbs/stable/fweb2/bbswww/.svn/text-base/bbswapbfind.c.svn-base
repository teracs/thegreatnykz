...
int bbswapbfind_main()
{
    ...
    init_all();
    printf
        ("<font class=\"title\">%s - 板内文章搜索</font><hr><br>\n",
         BBSNAME);
    ...
    printf
        ("<tr bgcolor=e8e8e8><td>编号<td>标记<td>作者<td>日期<td>标题\n");
    while (1)
    {
        ...
        printf("<td><a href=bbswapcon?board=%s&file=%s&num=%d>%40.40s </a>\n",
    ...
    printf("<br>\n");
    printf
        ("[<a href=bbswapdoc?board=%s>返回本讨论区</a>] [<a href='javascript:history.go(-1)'>返回上一页</a>]",
         board);
    printf("\n</html>");
    return 0;
}
...
static void show_form(char *board)
{
    printf("<table><form action=bbswapbfind method=post>\n");
    ...
    printf
        ("<tr><input type=submit value=递交查询结果>\n");
    printf("</form></table>");
    printf
        ("[<a href='bbswapdoc?board=%s'>返回上一页</a>] [<a href=bbswapfind>全站文章查询</a>]",
         board);
    printf("</html>");
}
