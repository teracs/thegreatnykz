#include "www.h"

int bbsalluser_main()
{
    FILE *fp;
    struct userec x;
    int pos[MAXUSERS], total = 0;
    int p, i, start;

    cache_age = 60 * 60 * 24;
    init_all();
    if (!loginok)
        http_quit_print("匆匆过客不能查询所有使用者列表，请先登录");
    for (i = 0; i < MAXUSERS; i++)
    {
        if (shm_ucache->userid[i][0] > 32)
        {
            pos[total] = i;
            total++;
        }
    }
    printf
        ("<center><font class=\"title\">%s - 所有使用者列表</font>　　[用户总数: %d]<hr>\n",
         BBSNAME, total);
    start = atoi(getparm("start"));
    if (start > total || start < 0)
        start = 0;
    printf("<table width=610>\n");
    printf
        ("<tr bgcolor=e8e8e8><td>编号<td>好友<td>使用者代号<td>昵称<td>上站次数<td>文章数<td>最近光临时间\n");
    fp = fopen(".PASSWDS", "r");
    for (i = 0; i < 20; i++)
    {
        if (start + i > total - 1)
            break;
        p = pos[start + i];
        fseek(fp, sizeof(struct userec) * p, SEEK_SET);
        if (fread(&x, sizeof(x), 1, fp) <= 0)
            break;
        printf
            ("<tr><td>%d<td>%s<td><a href=bbsqry?userid=%s>%s</a><td>%s<td>%d<td>%d<td>%s\n",
             start + i + 1,
             isfriend(x.userid) ? "<font color=green>√</font>" : " ",
             x.userid, x.userid, nohtml(x.username), x.numlogins,
             x.numposts, Ctime(x.lastlogin) + 4);
    }
    fclose(fp);
    printf("</table><hr>\n");
    if (start > 0)
        printf("[<a href=bbsalluser?start=0>第一页</a>] ");
    if (start > 0)
        printf("[<a href=bbsalluser?start=%d>上一页</a>] ",
               start - 20 < 0 ? 0 : start - 20);
    if (start < total - 19)
        printf("[<a href=bbsalluser?start=%d>下一页</a>]", start + 20);
    if (start < total - 19)
        printf("[<a href=bbsalluser?start=%d>最后一页</a>]\n", total - 19);
    printf("<form action=bbsalluser>\n");
    printf("<input type=submit value=跳转到>");
    printf("第<input name=start type=text size=5>个使用者\n");
    printf("</form>\n");
    printf("\n</html>");
    return 0;
}
