...
inline static int nt_sethomefile(char *buf, size_t buflen,
                                 const char *userid, const char *filename)
{
    return snprintf(buf, buflen, "home/%c/%c/%s/%s", toupper(userid[0]),
 ...
 */
extern int nt_get_active_uinfo(const char *userid,
                               struct user_info *puinfos[], int max);
/**
 ...
 */
extern int nt_get_active_uinfo_condition(struct user_info *puinfos[],
                                         int max,
                                         gboolean(*condition) (struct
                                                               user_info *,
                                                               void *),
                                         void *userdata);
/**
...
/**
 * check if user has filled the register form
 *
 * @param userid the user
 *
 * @return TRUE if has already filled, FLASE if not
 */
extern gboolean nt_has_fill_form(const char *userid);
