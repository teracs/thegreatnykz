...
extern int ntp_bbswaplogin(void);


static gboolean www_login_user2(struct userec *user)
{
    char buf[256];
    struct user_info *u;
    int n;
...
static void www_check_multi2(struct userec *puser)
{
...
extern gboolean www_check_multi_guest(char *fromhost);

int ntp_bbswaplogin(void)
{
    int t;
    ...
    if (ntw_haslogin)
    {
        ...
        {
            /*相同用户已经在本机登录 */
            cgiHeaderLocation("/framewap.html");
            return 0;
        }
    }
    ...
    if (!www_login_user2(&user))
    {
        errmsg = "已经达到系统登录上限，或者系统繁忙，请稍候再试";
        goto ERROR;
    }

    if (HAS_PERM_REALNAME(&user) || 0==strcmp(user.userid, "guest") )
        cgiHeaderLocation("/framewap.html");
    else
