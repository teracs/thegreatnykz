...
int AnnounceIndex(int level)
{
    ...
    char buf[256], file[256],*bufptr, *ptr, currentpath[1024], title[80];
    ...
    while (bool)
    {
        ...
        *(ptr + 1) = '\0';
        ...
        if (isdir)
        {
            sprintf(printstr, "%s[[Digdir:%s/%s|\033[m\033[1;32m%s\033[m]]\033[1m %s\033[m\n",
                    prefix, &currentpath[8],file, title, visitnumstr);
        }
        else if (!dironly)
        {
            sprintf(printstr, "%s[[Digest:%s/%s|\033[m%s]] \n", prefix,&currPath[8],file, title);
        }
        else
            sprintf(printstr, "");
