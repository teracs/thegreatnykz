...
*
*    $Id: remail.c,v 1.2 2004/06/29 14:59:23 rwzmm Exp $
*/
    ...
    while (dirp = readdir(dp))
    {
        ...
        while (workdirp = readdir(workdp))
        {
            ...
            while ((userdirp = readdir(userdp)) != NULL)
            {
                ...
                mailcounts +=
                    do_remail(userpath, userdirp->d_name, dot_dir,
                              workdirp->d_name);
                allactions++;
            }
            closedir(userdp);
...
char *username;
{
    ...
    {
        if (strstr(buf, "[0;1;44;36m") || strstr(buf, "[1;32;40mTo"))
            msgbackup = 1;
        if (strstr(buf, "·¢ÐÅÈË: ") ||
                strstr(buf, "×÷  Õß: ") || strstr(buf, "¼ÄÐÅÈË: "))
        {
            ptr = &buf[8];
            ptr2 = strchr(ptr, ' ');
            if (ptr2 != NULL)
    ...
    if (step != 2)
    {
        ...
        {
            strncpy(fh.title, "²¹¾ÈÎÄ¼þ£¬Èç²»ÐèÒªÇëÊÖ¹¤É¾³ý£¡",
                    sizeof(fh.title));
