...
#define RESTRICTED(u)   (users[(u)].flags == 0)
...
#define OUTSIDER(u)     (users[(u)].flags & PERM_666LIFE)
...
int chatid_to_indx(int unum, char *chatid)
{
    ...
    for (i = 0; i < MAXACTIVE; i++)
    {
        ...
        if (!strcasecmp(chatid, users[i].chatid) ||
            !strcasecmp(chatid, users[i].userid))
        {
...
void send_to_room(int room, char *str)
{
    ...
    for (i = 0; i < MAXACTIVE; i++)
    {
        ...
        {
            sethomefile(chatbuf, users[i].userid, "/chatignores");
            if (!seek_in_file(chatbuf, currchatid))
...
void exit_room(int unum, int disp, char *msg)
{
    ...
    if (oldrnum != -1)
    {
        if (--rooms[oldrnum].occupants)
        {
            ...
            case EXIT_LOSTCONN:
                sprintf(chatbuf,
                        "[1;37m¡ï [32m[[36m%s[32m] Ïñ¶ÏÁËÏßµÄ·çóİ ... [37m¡ï[m",
                        users[unum].chatid);
                break;
...
void chat_rename(int unum, char *msg)
{
    int rnum;
    ...
    {
        send_to_unum(unum,
                     "[1;31m¡ò [37mÇëÖ¸¶¨ĞÂµÄÁÄÌìÊÒÃû³Æ [31m¡ò[m");
        return;
    }
    ...
    sprintf(chatbuf,
            "[1;37m¡ï [32m[[36m%s[32m] ½«ÁÄÌìÊÒÃû³Æ¸ÄÎª [1;33m%.11s [37m¡ï[m",
...
void chat_topic(int unum, char *msg)
{
    ...
    send_to_room(rnum, chatbuf);
    sprintf(chatbuf,
            "[1;37m¡ï [32m[[36m%s[32m] ½«»°Ìâ¸ÄÎª [1;33m%42.42s [37m¡ï[m",
            users[unum].chatid, msg);
    send_to_room(rnum, chatbuf);
}
...
int enter_room(int unum, char *room, char *msg)
{
    ...
    if (rnum == -1)
    {
        ...
        {
            send_to_unum(unum,
                         "[1;31m¡ò [37m±§Ç¸£¬Äú²»ÄÜ¿ªĞÂ°üÏá [31m¡ò[m");
            return 0;
        }
        for (i = 1; i < MAXROOM; i++)
        {
    ...
    {
        sprintf(chatbuf,
                "[1;31m¡õ [37m[[1;33m%s[36;1m(%s)[37m] ½øÈë [35m%s [37m°üÏá [31m¡õ[m",
                users[unum].userid, users[unum].chatid, rooms[rnum].name);
        send_to_room(rnum, chatbuf);
    }
...
int print_user_counts(int unum)
{
    ...
    sprintf(chatbuf,
            "[1;31m¡õ[37m »¶Ó­¹âÁÙ¡º[32m%s[37m¡»µÄ¡¾[36m%s[37m¡¿[31m¡õ[m",
...
int login_user(int unum, char *msg)
{
    ...
    if (!strncasecmp("localhost", users[unum].host, 30))
    {
        users[unum].flags = atoi(level) & ~(PERM_666LIFE);
...
int chat_do_user_list(int unum, char *msg, int whichroom)
{
    ...
    stop = atoi(msg);
    send_to_unum(unum,
                 "[1;33;44m ÁÄÌì´úºÅ©¦Ê¹ÓÃÕß´úºÅ  ©¦ÁÄ    Ìì    ÊÒ[Op][m");
    for (i = 0; i < MAXACTIVE; i++)
    {
        rnum = users[i].room;
            ...
            && !(users[i].cloak == 1 && !CLOAK(unum)))
        {
            ...
            sprintf(chatbuf,
                    " [0;1;37m%-8s©¦[31m%s%-12s[37m©¦[32m%-14s[37m[[33m%-2s[37m][m",
                    users[i].chatid, OUTSIDER(i) ? "[1;35m" : "[1;36m",
...
void chat_list_by_room(int unum, char *msg)
{
    ...
    else
    {
        ...
        {
            sprintf(chatbuf, "[1;31m¡ò [37mÃ» %s Õâ¸ö·¿¼äà¸ [31m¡ò[m",
                    roomstr);
            send_to_unum(unum, chatbuf);
            return;
        }
    ...
    for (i = 0, c = 0; i < MAXACTIVE; i++)
    {
            ...
            && !(users[i].cloak == 1 && !CLOAK(unum)))
        {
            ...
            sprintf(chatbuf + (c * 50),
                    ...
                    (users[i].cloak == 1) ? 'C' : ' ', users[i].chatid,
                    (ROOMOP(i)) ? '*' : ' ',
                    OUTSIDER(i) ? "[1;35m" : "[1;36m", users[i].userid,
                    (c < 2 ? "©¦" : "  "));
            if (++c == 3)
            {
                send_to_unum(unum, chatbuf);
...
void chat_private(int unum, char *msg)
{
    ...
    if (recunum < 0)
    {
        ...
        if (recunum == -1)
            ...
            sprintf(chatbuf,
                    "[1;31m ¡ò[37m ÄÇÎ»²ÎÓëÕß½ĞÊ²÷áÃû×Ö [31m¡ò[m");
        send_to_unum(unum, chatbuf);
        return;
    }
    if (*msg)
    {
        sprintf(chatbuf, "[1;32m ¡ù [36m%s [37m´«Ö½ÌõĞ¡ÃØÊéÀ´µ½[m: ",
                users[unum].chatid);
        strncat(chatbuf, msg, 80);
    ...
    {
        sprintf(chatbuf,
                "[1;31m ¡ò[37m ÄãÒª¸ú¶Ô·½ËµĞ©Ê²÷áÑ½£¿[31m¡ò[m");
        send_to_unum(unum, chatbuf);
    }
}

int put_chatid(int unum, char *str)
{
    int i;
...
void chat_act(int unum, char *msg)
{
    if (*msg)
    {
        strcpy(currchatid, users[unum].userid);
...
void chat_join(int unum, char *msg)
{
    ...
    {
        send_to_unum(unum,
                     "[1;31m¡ò[37m·¿¼äÃûÖĞ²»ÄÜÓĞ¡¾*:/%¡¿µÈ²»ºÏ·¨×Ö·û[31m¡ò[37m");
        return;
    }
...
struct action party_data[] = {
    ...
    {"byebye", "Íû×Å", "ÀëÈ¥µÄ±³Ó°½¥½¥ÏûÊ§£¬Á½µÎ¾§Ó¨µÄÀá»¨ ÄÇ ´ÓÈù±ß»¬Âä"},
    ...
    for (i = 0; party_data[i].verb; i++)
    {
        if (!strcmp(cmd, party_data[i].verb))
        {
            ...
            sprintf(chatbuf, "[1;36m%s [32m%s[33m %s [32m%s[37;0m",
                    users[unum].chatid,
                    party_data[i].part1_msg, party,
                    party_data[i].part2_msg);
            send_to_room(users[unum].room, chatbuf);
            return 0;
        }
    }
...
struct action condition_data[] = {
    ...
    {"cough", "¿ÈÁË¼¸Éù", NULL},
    {"die", "¿ÚÍÂ°×Ä­£¬Ë«ÑÛÒ»·­£¬ÉíÌå´¤ÁË¼¸ÏÂ£¬²»¶¯ÁË£¬ÄãÒ»¿´ËÀÁË¡£",
     NULL},
                ...
                if (users[i].sockfd == -1)
                {
                    ...
                    if (local)
                    {
                                    ...
                                    sizeof(unsigned int)); j++)
                        {
                            if (sin.sin_addr.s_addr ==
                                (unsigned int) local->h_addr_list[j])
                                break;
                        }
                        if ((j < local->h_length)
                    ...
                    else
                    {
                        hp = gethostbyaddr((char *) &sin.sin_addr,
                                           ...
                                           sin.sin_family);
                        strncpy(s,
                                hp ? hp->h_name : (char *) inet_ntoa(sin.
                                                                     sin_addr),
                                30);
                    }
                    s[29] = 0;
