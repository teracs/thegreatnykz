...
int m_ordainBM(void)
{
    ...
    if (!strncmp(bname, "BMTraining", 10))
    {
        if (district <= 0)
           sprintf(genbuf, "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n"
                   "\tÈÎÃü %s Îª %s °å%s£¡\n"
                   "\tÇë %s ÔÚÈıÌìÄÚÔÚ test °å±¨µ½¡£\n"
                   "\tÓâÆÚÎ´ÄÜ±¨µ½Õß£¬½«È¡Ïû°åÖ÷È¨ÏŞ¡£\n",
                   lookupuser.userid, bname, bm ? "°åÖ÷" : "°å¸±",
                   lookupuser.userid);
        else
           sprintf(genbuf, "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n"
                   "\tÈÎÃü %s Îª %s °å%s£¡\n"
    ...
    else if (!strncmp(bname, "District", 8))
    {
        sprintf(genbuf, "\n\t\t\t¡¾ Í¨¸æ ¡¿\n\n"
                "\tÈÎÃü %s Îª %s °å%s£¡\n"
                "\tÇë %s ÔÚÈıÌìÄÚÔÚ BMhome °å±¨µ½¡£\n"
                "\tÓâÆÚÎ´ÄÜ±¨µ½Õß£¬½«È¡Ïû°åÖ÷È¨ÏŞ¡£\n"
                "\tÉÏÈÎºóÁùÊ®ÌìÎªÊµÏ°ÆÚ£¬\n"
                "\tÆÚÂúºóÆßÈÕÄÚÇëÔÚBMhome°åÌá½»×ªÕı±¨¸æ£¬\n"
...
int m_retireBM(void)
{
    ...
    if (!gettheuserid(2, "ÊäÈëÓûÀëÖ°µÄ°åÖ÷ÕÊºÅ: ", &id))
        return -1;
    if (!gettheboardname
        (3, "ÇëÊäÈë¸Ã°åÖ÷Òª´ÇÈ¥µÄ°åÃû: ", &pos, &fh, bname))
        return -1;
    oldbm = getbnames(lookupuser.userid, bname, &find);
    if (!oldbm || !find)
    {
    ...
    else
    {
            ...
            && !(lookupuser.userlevel & PERM_SYSOP))
        {
            ...
            substitute_record(PASSFILE, &lookupuser, sizeof(struct userec),
                              id);
            sprintf(secu, "°åÖ÷Ğ¶Ö°, È¡Ïû %s µÄ°åÖ÷È¨ÏŞ",
                    lookupuser.userid);
            securityreport(secu);
            move(9, 0);
            prints(secu);
    ...
    prints("\n");
    if (askyn("Õı³£ÀëÈÎÇë°´ Enter ¼üÈ·ÈÏ£¬³·Ö°³Í·£°´ N ¼ü", YEA, NA) ==
        YEA)
        right = 1;
    else
        right = 0;
...
int chkGroupsInfo(void)
{
    ...
    prints("\n\n");
    if (askyn
        ("ÉÏÃæµÄĞÅÏ¢±íÃ÷ menu.ini ¿ÉÄÜÓĞÎó£¬ÄúÈÔÈ»Ïë¼ÌĞøÖ´ĞĞÂğ", NA,
         NA) == YEA)
        return 1;
    return 0;
}
...
int ChangeTheDir(struct boardheader *oldfh, int pos)
{
    ...
    if (oldfh)
    {
        ...
        prints
            ("Äú½«¿ªÊ¼´´½¨Ò»¸ö\033[32mĞÂ\033[mµÄÄ¿Â¼ [ENTER-->È¡Ïû´´½¨]");
    while (1)
    {
    ...
    if (askyn("±¾Ä¿Â¼ĞèÒªÉèÖÃ READ »ò POST ÏŞÖÆÂğ", NA, NA) == YEA)
    {
        ...
        prints("Éè¶¨ %s '%s' ÌÖÂÛÇøµÄÈ¨ÏŞ\n",
               newfh.level & PERM_POSTMASK ? "ÕÅÌù" : "ÔÄ¶Á",
               newfh.filename);
        newfh.level =
            setperms(newfh.level, "È¨ÏŞ", NUMPERMS, showperminfo);
        clear();
    }
    ...
    numboards = -1;
    {
        ...
        if (oldfh)
            sprintf(secu, "ĞŞ¸ÄÄ¿Â¼£º%s(%s)", oldfh->filename,
                    newfh.filename);
        else
...
int ChangeTheBoard(struct boardheader *oldfh, int pos)
{
    ...
    if (oldfh)
    {
        ...
        prints
            ("Äú½«¿ªÊ¼´´½¨Ò»¸ö\033[32mĞÂ\033[mµÄÌÖÂÛÇø [ENTER-->È¡Ïû´´½¨]");
    while (1)
    {
        struct boardheader dh;

        getdata(3, 0, "ÌÖÂÛÇøÃû³Æ(Ó¢ÎÄÃû): ", buf, BDNAME_LEN, DOECHO,
                YEA);
        if (buf[0] == '\0')
        {
            if (oldfh)
    ...
    if (gettheboardname
        (17, "ÊäÈëËùÊôÄ¿Â¼Ãû(¸ùÄ¿Â¼Ö±½Ó»Ø³µ): ", &tmppos, &tmpfh, genbuf))
        newfh.group = tmppos;
    else
        newfh.group = 0;
    ...
    if (askyn
        ("ÉèÖÃ±¾°åÎªÄäÃû°åÃæÂğ", newfh.flag & ANONY_FLAG ? YEA : NA,
         NA) == YEA)
        if (askyn
            ("ÉèÖÃ±¾°åÄäÃû·¢ÎÄÎªÄ¬ÈÏÂğ",
    ...
    if (askyn
        ("ÉèÖÃ±¾°åÎªÍÆÎÄÄ£Ê½°åÃæÂğ", (newfh.flag & PUSH_FLAG) ? YEA : NA,
         NA) == YEA)
        newfh.flag |= PUSH_FLAG;
    else
        newfh.flag &= ~PUSH_FLAG;
    ...
    if (a_mv != 0)
    {
        ...
        while (1)
        {
            ...
            if (*chs > '0' + 9)
                *chs += 7;
            getdata(j + 1, 48, "¾«»ªÇø·Ö²¼ÈçÉÏ£¬ÇëÑ¡Ôñ: ", chs, 2, DOECHO,
                    NA);
            if (chs[0] >= 'a' && chs[0] <= 'z')
                chs[0] -= 32;
            if (chs[0] >= 'A' && chs[0] <= 'Z')
    ...
    numboards = -1;
    {
        ...
        if (oldfh)
            sprintf(secu, "ĞŞ¸ÄÌÖÂÛÇø£º%s(%s)", oldfh->filename,
                    newfh.filename);
        else
...
void ShowBrdSet(struct boardheader fh)
{
    ...
    prints("ÊÇ·ñÎª¿ÉÄäÃûÌÖÂÛÇø : %s",
           ((fh.flag & ANONY_FLAG)
            || (fh.flag & POSITIVE_ANONY_FLAG)) ? "ÄäÃû" : "·ÇÄäÃû");
    move(i++, 0);
...
int m_editbrd()
{
    ...
    sprintf(buf, "ÄäÃû°å£¨Ä¬ÈÏÄäÃû£© (Y/N)? [%c]: ",
            (noidboard) ? 'Y' : 'N');
    ...
    sprintf(buf, "ÄäÃû°å£¨ÄäÃû²»Ä¬ÈÏ£© (Y/N)? [%c]: ",
            (noidboard) ? 'Y' : 'N');
    getdata(20, 0, buf, genbuf, 4, DOECHO, YEA);
    if (*genbuf == 'y' || *genbuf == 'Y' || *genbuf == 'N'
        || *genbuf == 'n')
    ...
    sprintf(buf, "ÎÄÕÂÊÇ·ñ ²»¿ÉÒÔ »Ø¸´ (Y/N)? [%c]: ",
            (noreply) ? 'Y' : 'N');
    ...
    {
        getdata(23, 0, "È·¶¨Òª¸ü¸ÄÂğ? (Y/N) [N]: ", genbuf, 4, DOECHO,
                YEA);
    }
    if (*genbuf != 'Y' && *genbuf != 'y')
    {
...
int d_board(void)
{
    ...
    mystrlcpy(binfo.title, genbuf, STRLEN);
...
int d_user(char *cid)
{
    ...
    if (num)
    {
        prints("[%s] Ä¿Ç°ÉĞµ£ÈÎÁË %d ¸ö°åµÄ°åÖ÷: ", lookupuser.userid,
               num);
        for (i = 0; i < num; i++)
            prints("%s ", bnames[i]);
...
int m_ordainQZ(void)
{
    ...
    num[0] = 0;
    getdata(j, 0, "ÈÎÃüÄÄÒ»·ÖÇø(²Î¿¼ÓÒ±ßÌáÊ¾±àºÅ)£¿: ", num, 2, DOECHO,
            NA);
...
int m_MinusTrainingBM(void)
{
    ...
    if (!oldbm || !find)
    {
        ...
        prints(" %s ²»ÊÇ %s °å°åÖ÷£¬ÈçÓĞ´íÎó£¬ÇëÍ¨Öª³ÌĞòÕ¾³¤¡£",
               lookupuser.userid, bname);
        pressanykey();
        clear();
        return -1;
...
int m_OrdainBMQZ(void)
{
    ...
    {
        move(5, 0);
        prints("Äã²»¾ßÓĞ¸Ã°åÃæËùÔÚÇøµÄÇø³¤È¨ÏŞ");
        pressanykey();
        clear();
        return -1;
    }
...
int m_RetireBMQZ(void)
{
    ...
    if (!gettheuserid(2, "ÊäÈëÓûÀëÈÎµÄ°åÖ÷ÕÊºÅ: ", &id))
        return -1;
    ...
    if (!gettheboardname
        (3, "ÊäÈë¸ÃÊ¹ÓÃÕß½«ÀëÈÎµÄÌÖÂÛÇøÃû³Æ: ", &pos, &fh, bname))
        return -1;
...
void m_EditBoardLinks(void)
{
    ...
    if (aborted != -1)
    {
        prints("%s°åÓÑÇéÁ´½Ó¸üĞÂÍê±Ï", bname);
        sprintf(secu, "¸üĞÂ%s°åÓÑÇéÁ´½Ó", bname);
        securityreport(secu);
    }
...
void m_EditConfig(void)
{
    ...
    if (aborted != -1)
    {
        prints("ÏµÍ³µµ°¸ÅäÖÃÎÄ¼şĞŞ¸ÄÍê±Ï");
        sprintf(secu, "ĞŞ¸ÄÏµÍ³µµ°¸ÅäÖÃÎÄ¼ş");
        securityreport(secu);
    }
...
void m_EditFiles(void)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            getdata(23, 0, "ÄãÒª±àĞŞÄÄÒ»ÏîÏµÍ³µµ°¸(dÏÂÒ³/uÉÏÒ³): ", ans, 4,
                    DOECHO, YEA);
            if (!isdigit(ans[0]))
            {
                if (ans[0] == 'd')
        ...
        case 0:
            ...
            if( strstr(buf, "..") 
                ...
                || strstr(buf, "/innd/")
                || strstr(buf, "/reclog/")
                || strstr(buf+strlen(BBSHOME), "/home/") )
          
            {
                aborted = -1;
            }
            else if( lstat(buf, &st) != 0)
            {
                aborted = -1;
            }
            ...
            if (aborted != -1)
            {
                prints("%s ¸üĞÂ¹ı", filename[choice]);
                sprintf(secu, "ĞŞ¸ÄÏµÍ³µµ°¸£º%s", filename[choice]);
                securityreport(secu);
