...
{
    sprintf(s, "home/%c/%c/%s/.FRIENDTEST", toupper(id[0]), toupper(id[1]),
            id);
}

void sethometop(char *s, char *id)
{
    sprintf(s, "home/%c/%c/%s/" FRIENDTOP, toupper(id[0]), toupper(id[1]),
            id);
}

void sethomestat(char *s, char *id)
{
    sprintf(s, "home/%c/%c/%s/.FRIENDSTAT", toupper(id[0]), toupper(id[1]),
            id);
}
...
void load_users(char *filename)
{
    ...
    if ((fd = open(filename, O_RDONLY, 0644)) != -1)
    {
        ...
        if (fcntl(fd, F_SETLKW, &ldata) != -1)
        {
            ...
            users =
                (struct usertype *) malloc(sizeof(struct usertype) *
                                           (userst + 1));
...
void load_quests(char *filename)
{
    ...
    if ((fd = open(filename, O_RDONLY, 0644)) != -1)
    {
        ...
        if (fcntl(fd, F_SETLKW, &ldata) != -1)
        {
            ...
            quests =
                (struct questype *) malloc(sizeof(struct questype) *
                                           TOTALNUM);
...
int usermenu(void)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        {
            move(7 + i,
                 63 - strlen(users[userst - i - 1].userid) / 2 + 7 + 7 +
                 7);
            setfcolor(GREEN, 1);
    ...
    while (1)
    {
        move(22, 20);
        clrtoeol();
        getdata(22, 26, "请选择:", buf, IDLEN + 1, true, true);
        ...
        if (toupper(buf[0]) == 'L' && !buf[1])
            return 5;
        if (HAS_PERM(PERM_SYSOP) && toupper(buf[0]) == 'D' && !buf[1])
        {
            return 6;
        }
        else if (buf[0])
        {
            ...
            if (tuid)
            {
                ...
                else
                {
                    move(23, 20);
                    clrtoeol();
                    prints("使用者%s尚未创建题目!", lookupuser.userid);
                }
            }
            else
            {
...
void new_friendtest_reset(void)
{
    resetcolor();
    clear();
    move(0, 0);
...
void new_friendtest(void)
{
    ...
    {
        empty = 0;
        getdata(1, 0, "已经存在你的账号，是否继续? [y/N] ", ans, 2, true,
                true);
        if (toupper(ans[0]) != 'Y')
            return;
    }
    getdata(1, 0, "请输入题数[1--20]", buf, 3, true, true);
    questst = atoi(buf);
    ...
    for (i = 0; i < questst; i++)
    {
      again:
        new_friendtest_reset();
        prints("第%d题(共%d题)\n", i + 1, questst);
        getdata(2, 0, "请输入题目(40个汉字内): ", quests[i].topic, 80,
                true, true);
        if (!quests[i].topic[0])
        {
            new_friendtest_reset();
            getdata(1, 0, "退出建立账号? [Y-退出/N-继续/E-不再输入新题]: ",
                    ans, 2, true, true);
            if (toupper(ans[0]) == 'Y')
                return;
        ...
        do
        {
            move(3, 0);
            clrtoeol();
            getdata(3, 0, "请输入选项个数[1--5]: ", buf, 2, true, true);
            j = atoi(buf);
        }
        while (j < 1 || j > 5);
        for (k = 0; k < j; k++)
        {
            do
            {
                ...
                getdata(4 + k * 2, 0, buf, quests[i].quest[k], 40, true,
                        true);
            }
    ...
    new_friendtest_reset();
    move(1, 0);
    prints("你的测试共有%d项，你的测试可能得到的最高分是%d", questst,
           possible_high());
    getdata(2, 0, "以上输入是否正确，选择否取消，选择是建立[Y/n]", ans, 2,
            true, true);
    if (toupper(ans[0]) == 'N')
        return;
    save_quests(direct);
...
void do_test_reset(void)
{
    ...
    prints
        ("   FRIENDTEST      友谊竞赛 %s <--> %s                                           ",
         currentuser.userid, lookupuser.userid);
    resetcolor();
    prints("\n");
}
...
void do_test(void)
{
    ...
    if (userst)
    {
        ...
        prints("其中最高分 %d ,  最低分 %d\n", users[0].tried,
               users[userst - 1].tried);
        prints("\n=======TOP 10========\n");
        for (i = 0; i < 10; i++)
            if (i < userst)
                prints(" %-12s %-3d\n", users[i].userid, users[i].tried);
        getdata(22, 0, "按d显示详细结果: ", ans, 2, true, true);
        if (toupper(ans[0]) == 'D')
        {
            do_test_reset();
            move(1, 0);
            ...
            {
                move(j, 0);
                prints(" %-12s %-3d        %-3d", users[i].userid,
                       users[i].tried, users[i].got);
                if (j >= 20 && i < userst - 1)
                {
                    getdata(22, 0, "按\033[32;1mq\033[m退出:", ans, 2,
                            true, true);
                    if (toupper(ans[0]) == 'Q')
                        break;
                    do_test_reset();
    ...
    for (i = 0; i < questst; i++)
    {
      doitagain:
        ...
        do
        {
            ...
            {
                do_test_reset();
                getdata(1, 0, "是否退出测试?[y/N]", ans, 2, true, true);
                if (toupper(ans[0]) == 'Y')
                    return;
                goto doitagain;
            }
    ...
    for (i = 0; i < questst; i++)
    {
        ...
        for (k = 0; k < j; k++)
        {
            ...
            prints("%d) %s (分值:%d  选择人次:%d)\n", k + 1,
                   quests[i].quest[k], quests[i].value[k], m);
        }
        getdata(10, 0, "按q退出:", ans, 2, true, true);
        if (toupper(ans[0]) == 'Q')
            break;
    }
}
...
void admin_stat(void)
{
    ...
    while (1)
    {
        ...
        getdata(7, 0,
                "选择[q-退出,d-删除,e-修改,a-添加,r-排行重置,s-察看,l-列表] ",
                ans, 2, true, true);
        ...
        case 'A':
          ...
          againa:
            ...
            {
                new_friendtest_reset();
                getdata(1, 0, "取消? [Y-退出/N-继续]: ", ans, 2, true,
                        true);
                if (toupper(ans[0]) == 'Y')
                    break;
                goto againa;
            ...
            do
            {
                move(3, 0);
                clrtoeol();
                getdata(3, 0, "请输入选项个数[1--5]: ", buf, 2, true,
                        true);
                j = atoi(buf);
            }
            while (j < 1 || j > 5);
            for (k = 0; k < j; k++)
            {
                do
                {
                    ...
                    clrtoeol();
                    sprintf(buf, "请输入第%d个选择内容:", k + 1);
                    getdata(4 + k * 2, 0, buf, quests[i].quest[k], 40,
                            true, true);
                }
                while (!quests[i].quest[k][0]);
                do
        ...
        case 'E':
          ...
          againb:
            ...
            break;
        case 'R':
            getdata(8, 0, "一旦重置之后，排行榜将被抹去，确认吗(y/N)", ans,
                    2, true, true);
            if (toupper(ans[0]) != 'Y')
                break;
            load_users(FRIENDTOP);
...
void sys_list(void)
{
    ...
    for (i = 0; i < userst; i++)
    {
        ...
        if (k > 20 && i < userst - 1)
        {
            getdata(22, 0, "按\033[32;1mq\033[m退出:", ans, 2, true, true);
            if (toupper(ans[0]) == 'Q')
                break;
            sys_reset();
            move(1, 0);
...
int admin_menu(void)
{
    ...
    getdata(3, 0,
            "选择(0-删题,1-禁止开题,2-允许开题,3-禁止做题,4-允许做题,5-列表,其他-离开) ",
            ans, 2, true, true);
    if (ans[0] >= '0' && ans[0] <= '4')
    {
        getdata(5, 0, "请输入id:", buf, IDLEN + 1, true, true);
        tuid = getuser(buf);
        if (!tuid)
        {
            move(7, 0);
