...
void ApplyToNameList(int (*fptr) (char *))
{
    struct word *p;
...
int namecomplete_zh(char *prompt, char *data)
{
    char *temp;
    int ch;
    int count = 0;
...
int UserMaxLen(char (*cwlist)[13], int cwnum, int morenum, int count)
{
    int len, max = 0;
...
int
UserSubArray(char (*cwbuf)[13], char (*cwlist)[13], int cwnum, int key,
             int pos)
{
    int key2, num = 0;
    int n, ch;
...
int usercomplete(char *prompt, char *data)
{
    ...
    temp = data;
    cwlist = u_namearray((char (*)[13]) cwbuf, &cwnum, "");
    getyx(&y, &x);
    getyx(&origy, &origx);
    while ((ch = igetkey()) != EOF)
    {
        ...
        if (ch == ' ')
        {
            ...
            for (i = strlen(data); i && i < IDLEN; i++)
            {
                ...
                *temp++ = ch;
                *temp = '\0';
                n = UserSubArray((char (*)[13]) cwbuf,
                                 (char (*)[13]) cwlist, cwnum, ch, count);
                if (n == 0)
                {
                    temp--;
                    *temp = '\0';
            ...
            col = 0;
            len =
                UserMaxLen((char (*)[13]) cwlist, cwnum, morenum,
                           NUMLINES);
            move(origy + 1, 0);
            clrtobot();
            ...
            while (len + col < 79)
            {
                ...
                for (i = 0; morenum < cwnum && i < NUMLINES - origy + 1;
                     i++)
                {
        ...
        if (count < STRLEN)
        {
            ...
            *temp++ = ch;
            *temp = '\0';
            n = UserSubArray((char (*)[13]) cwbuf, (char (*)[13]) cwlist,
                             cwnum, ch, count);
