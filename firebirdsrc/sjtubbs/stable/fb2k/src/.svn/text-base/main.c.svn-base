...
void login_query(void)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            getdata(0, 0, "[1;37mÇëÊäÈëÃÜÂë: [m", passbuf, 9,
                    NOECHO, YEA);
            passbuf[8] = '\0';
...
int mailto_real_name_mail(char *userid)
{
    ...
    fclose(code);
    return_no = bbs_sendmail(fname, (char *) title, email, NA, YEA);
    unlink(fname);
    return return_no;
}
...
void user_login(void)
{
    ...
    if (currentuser.numlogins < 1)
    {
        ...
        getdatestring(time(0), NA);
        prints
            ("\033[1;36m¡î ÕâÊÇÄúµÚ \033[33m1\033[36m ´Î°Ý·Ã±¾Õ¾£¬Çë¼Ç×¡½ñÌì°É¡£\n");
        prints("¡î ÄúµÚÒ»´ÎÁ¬Èë±¾Õ¾µÄÊ±¼äÎª \033[33m%s\033[m ",
               datestring);
    }
    else
    {
    ...
    {
        getdata(t_lines - 1, 0,"ÃÜÂëÊäÈë´íÎó¼ÇÂ¼  (Y) É¾³ý¼ÇÂ¼ (N) ±£Áô (M) ¼Ä»ØÐÅÏä£¿[N]£º", genbuf, 4, DOECHO, YEA);
        if (genbuf[0] == 'M' || genbuf[0] == 'm')
        {
            mail_file(fname, currentuser.userid, "ÃÜÂëÊäÈë´íÎó¼ÇÂ¼");
            unlink(fname);
        }
            ...
            if (namep != NULL && ci_strcmp(namep, currentuser.userid) == 0)
            {
                ...
                if (flag == 'a' && (strncmp(datestring, date, 16) >= 0))
                {
                    if (del_from_file(filename, currentuser.userid))
                    {
						...
						{
							sprintf(repbuf, "»Ö¸´%sÔÚÈ«Õ¾µÄ»ù±¾È¨ÏÞ",
									currentuser.userid);
							sprintf(msgbuf, "\n  %s ÍøÓÑ£º\n\n"
									"\tÒò·â½ûÊ±¼äÒÑ¹ý£¬ÏÖ»Ö¸´ÄúÔÚÈ«Õ¾µÄ»ù±¾È¨Á¦¡£\n\n"
									"\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tBy %s",
									currentuser.userid, BBSID);
						}
...
char *boardmargin(void)
{
    ...
    if (selboard)
        sprintf(buf, " %cÇø [%s] ", ch, currboard);
    else
    {
