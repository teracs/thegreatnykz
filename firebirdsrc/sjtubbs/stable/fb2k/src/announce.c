...
int bmonly = 0;
void a_menu(char *maintitle, char *path, char *xpath, int lastlevel, int lastbmonly, int depth);
...
int a_loadnames(MENU * pm)
{
    ...
    while (fgets(buf, sizeof(buf), fn) != NULL)
    {
        ...
        else if (strncmp(buf, "Path=", 5) == 0)
        {
            ...
            mystrlcpy(pm->mtitle, buf + 8, STRLEN);
...
void a_forward(char *path, ITEM * pitem, int mode)
{
    ...
    if (dashf(fname))
    {
        mystrlcpy(fhdr.title, pitem->title, STRLEN);
...
void a_menu(char *maintitle, char *path, char *xpath, int lastlevel, int lastbmonly, int depth)
{
    MENU me;
    FILE *fp;
    char fname[PATHLEN], tmp[STRLEN], *bmstr, buf[STRLEN], pathnum[80];
    ...
    memset(buf, 0, STRLEN);
    mystrlcpy(buf, me.mtitle, STRLEN);
      ...
      EXPRESS:
        ...
        case KEY_RIGHT:
			...
			break;
                    ...
                    now = time(0);
                    sprintf(pathnum, "%s-%d", me.xpath, (me.now + 1));
                    pathnum[70] = 0;
                    a_menu(me.item[me.now]->title, fname, pathnum, me.level,
                           bmonly, depth + 1);
                    timetotal += time(0) - now;
                    now = time(0);
                }
