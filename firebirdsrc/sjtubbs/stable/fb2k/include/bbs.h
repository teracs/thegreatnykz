...
#define RECOMMEND_DIGEST_BOARDNAME   "Digest"
...
#define NA  (0)
#define YEA_FOR_ONCE (1<<1)     /* Special Booleans, If user confirm immediately, is true otherwise false */
...
#define MAXVOTEPEREMAIL     1      /*同一个email不同账号最多投票数 */
...
#define FILE_NOREPLY2       0x1         /*No Reply By BM, fileheader->access[1]*/
...
// 如需取消，只需关闭对应的宏定义
#define NEW_VCODE
#define NEW_VCODE_FILE "etc/vcodefile"
...
/*Nodejs service*/
#define SERVICE_HANDLER

#ifdef SERVICE_HANDLER
#define REPLY_SERVICE_HANDLER
//#define WWW_LOGIN_SERVICE_HANDLER 
typedef void (*buf_callback)(char* buf, int len);
int send_msg_to_service(const char* msg, buf_callback callback);
#endif

#ifdef REPLY_SERVICE_HANDLER
#define REPLY_SERVICE_HOST "127.0.0.1"
#define REPLY_SERVICE_PORT 50000
...
int www_login_service_handle(const char*, const int, const int);
int www_logout_service_handle(const char*);
#endif
