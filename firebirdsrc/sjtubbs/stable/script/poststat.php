...
function _e($str){
	...
	$s = htmlspecialchars($str, ENT_QUOTES, 'GB2312');
	if($s=="") $s = htmlspecialchars(
		substr($str,0,strlen($str)-1), ENT_QUOTES, 'GB2312');
...
$line = sprintf("%s","          \033[1;34m-----\033[37m=====\033[41m 本日10大热门话题 \033[40m=====\033[34m-----\033[0m\n\n");
...
/*	$query_junk = "select * from junk where time >
	...
	$line = sprintf("%s\033[1;37m第\033[1;31m%2d\033[37m名 \033[37m信区:\033[33m%-12s\033[37m【\033[32m%s\033[37m】\033[36mP:%4d U:%3d I:%3d \033[35m%+12s\n",$line,$i+1,$post_sum_array_row[board],date("Md H:i",$post_sum_array_row[reid]),$post_sum_array_row[sum],$post_sum_array_row[user_sum],$post_sum_array_row[ip_sum],$post_sum_array_row[post_master]);
	$line_web = sprintf("%s<tr><td width=110>[<a
		href=\"/bbsdoc,board,%s.html\"
		target=\"_self\">%12s</a>]</td><td><a
		href=\"/bbstcon,board,%s,reid,%d.html\"
		target=\"_self\">%s
		</a></td><td
		width=110>%s</td></tr>",$line_web,htmlspecialchars($post_sum_array_row[board]),$post_sum_array_row[board],$post_sum_array_row[board],$post_sum_array_row[reid],_e($post_sum_array_row[title]),$post_sum_array_row[post_master]);
	$line = sprintf("%s\033[37m标题 : \033[1;44;37m%-60.60s\033[40m\n",$line,$post_sum_array_row[title]);
	$i++;
	}
}
fwrite($filehandle,$line);
...
while ($row = mysql_fetch_assoc($result) ) {
    ...
    $line = sprintf("%s\n<tr><td><a href=\"/bbsdoc,board,%s.html\"
...
while ($row = mysql_fetch_assoc($result) ) {
    $line = sprintf("%s
	    <tr><td width=110>
	    [<a href=\"/bbsdoc,board,%s.html\"
	    target=\"_self\">%s</a>]
	    </td><td>
	    <a href=\"/bbstcon,board,%s,reid,%d,file,%s.html\"
	    target=\"_self\">%s</a>
	    </td><td>
	    %s
	    </td>
	    ...
	    $line,$row['board'],$row['board'],$row['board'],$row['reid'],$row['filename'],
	    _e($row['title']),date("Md H:i:s",$row['reid']),$row['user_id']);
}
$line = $line."</table>";
